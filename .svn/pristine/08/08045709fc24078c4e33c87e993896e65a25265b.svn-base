...
function onOrgChanged(){
	...
	case '1':
		$.ajax({
			url:'${path}/baseInfo/statisticManage/getBaseInfoStatisticList.action?orgId='+getCurrentOrgId()+"&year="+$("#year").val()+"&month="+$("#month").val()+"&type=<@s.property value='#parameters.type'/>",
	...
	case '2':
		$.ajax({
			url:'${path}/baseInfo/statisticManage/getBaseInfoStatisticList.action?orgLevelDistance=2&orgId='+getCurrentOrgId()+"&year="+$("#year").val()+"&month="+$("#month").val()+"&type=<@s.property value='#parameters.type'/>",
			success:function(data){
				grid.bindData(data);
			}
		})
...
$(document).ready(function(){
	...
	$("#createStatistic").click(function(){
		$.ajax({
			url:'/baseInfo/statisticManage/createHistoryStatistic.action?orgId='+getCurrentOrgId()+"&year="+$("#year").val()+"&month="+$("#month").val()+"&type=<@s.property value='#parameters.type'/>",
	...
	$(".print").click(function(){
		...
		var url = '${path}/statAnalyse/baseInfoStat/common/commonStatisticPrint.ftl?parentOrgId='+getCurrentOrgId()+"&year="+$("#year").val()+"&month="+$("#month").val()+"&type=<@s.property value='#parameters.type'/>&moduleName="+document.title;
		if($("#type").val() == '2'){
