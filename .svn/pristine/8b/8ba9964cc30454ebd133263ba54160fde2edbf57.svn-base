...
public class CalendarUtil {
	 ...
	 * @return
	 */
	public static boolean isMothStartTime() {
		Calendar calendar = Calendar.getInstance();
		//日历翻到当前
		...
		//日历翻到当前月第一天开始
		calendar.setTime(getMonthStart(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH)+1));
		int dayStart=calendar.get(Calendar.DAY_OF_MONTH);
		int hourStart=calendar.get(Calendar.HOUR);
		//比较
		if(day==dayStart&&hour==hourStart){
			return true;
		}
		return false;
	}
