...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#helpPersonnelList").jqGridFunction({
			...
			colModel:[
				...
				{name:"name",sortable:false ,index:"name", label: '监管人姓名'},
				{name:"identity", sortable:false, label: '监管人员身份'},
			...
			ondblClickRow: viewWardHelpPrecord,
		...
		$("#addHelp").click(function(){
			...
			$("#helpPersonnelDialog").createDialog({
				...
				height: 250,
				title:'新增监管人员信息',
				url:'${path}/baseinfo/helpPersonnel/dispatch.action?personnelId='+domainId+'&personnelType=<s:property value="#parameters.keyType"/>&mode=add&personType=ward',
				buttons: {
					"保存" : function(event){
		...
		$("#updateHelp").click(function(){
			...
			$("#helpPersonnelDialog").createDialog({
				...
				height: 250,
				title:'修改监管人员信息',
				url:'${path}/baseinfo/helpPersonnel/dispatch.action?helpPersonnel.id='+id[0]+'&mode=edit&personType=ward',
				buttons: {
					"保存" : function(event){
				      $("#helpPersonnelForm").submit();
				   },
		...
		$("#logoutHelp").click(function(){
			...
			$.confirm({
				title:"确认注销",
				message:"该监管人信息注销后，将无法恢复,您确认注销该人员信息吗?",
				width: 300,
				okFunc: function(){
				$.ajax({
	...
	function viewWardHelpPrecord(){
		...
		$("#helpPersonnelDialog").createDialog({
			...
			height: 250,
			title:'查看监管人员信息',
			url:'${path}/baseinfo/helpPersonnel/dispatch.action?helpPersonnel.id='+id[0]+'&mode=view&personType=ward',
			buttons: {
			   "关闭" : function(){
			        $(this).dialog("close");
			   }
