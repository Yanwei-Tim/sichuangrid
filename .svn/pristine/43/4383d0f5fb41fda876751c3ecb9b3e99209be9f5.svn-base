...
function getmonth(){
	$.ajax({
		async: false,
		url: "${path }/stat/currentTime/getCurrentTimeForMonthToSpecial.action?currenYear="+$("#pyear").val(),
...
function onOrgChanged(){
	...
	$("#importantPlacePie").pieChart({
		url:"${path}/baseInfoStat/companyPlaceStanalsManage/getStatisticsCompanyPlacePie.action?orgId="+getCurrentOrgId()+"&year="+$("#pyear").val()+"&month="+$("#pmonth").val()+"&moduleType="+moduleType,
