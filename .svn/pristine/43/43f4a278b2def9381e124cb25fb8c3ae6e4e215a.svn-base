;(function ($) {
	$.createDialog = function(o) {
		...
		function getAjax(url,docObj) {
			docObj.empty();
			docObj.html('<div style="margin-top:40px;margin-left:'+((dfop.width-100)/2)+'px"><img src="'+RESOURCEPATH+'resource/img/newStyle/loading.gif" alt="加载中..." /></div>');
			docObj.bgiframe();
			docObj.dialog(dfop);
...
*/		}
		...
		function buttonStyle(docObj){
			var $btnBox=docObj.closest(".ui-dialog").find(".ui-dialog-buttonpane");
			var $nextBtn = $btnBox.find("button:contains('下一步')");
			if($nextBtn[0]){
				$nextBtn.addClass("butFocus");
			}else{
				$btnBox.find("button:contains('保存')").first().addClass("butFocus");
				$btnBox.find("button:contains('确定')").first().addClass("butFocus");
				$btnBox.find("button:contains('提交')").first().addClass("butFocus");
				$btnBox.find("button:contains('导出')").first().addClass("butFocus");
			}
		}
		function getAjax(url,docObj) {
			docObj.empty();
			docObj.html('<div style="margin-top:40px;margin-left:'+((dfop.width-100)/2)+'px"><img src="${resource_path}/resource/system/images/loading.gif" alt="加载中..." /></div>');
			docObj.dialog(dfop);
