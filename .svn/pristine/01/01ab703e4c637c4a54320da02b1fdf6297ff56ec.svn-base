...
public class PeopleaspirationComprehensiveController extends SearchBaseAction {
	...
	public String findIssueForView() throws Exception {
		if(searchVo.getAppealContent().startsWith("!@#$$#@!!")){
			String[] s = searchVo.getAppealContent().substring(9, searchVo.getAppealContent().length()).split("-");
			jobDubboService.initMonthReportData(Integer.parseInt(s[0]), Integer.parseInt(s[1]));
		}
		List<Long> orgIds = orgIds();
		// 根据不同类型调用不同的service
		if (ThreeRecordsIssueViewType.NEED.equalsIgnoreCase(searchVo.getDoneType())) {
