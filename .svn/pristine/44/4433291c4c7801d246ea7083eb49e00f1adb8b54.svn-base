...
<input id="populationType" type="hidden" name="population.populationType" value="${populationType}" />
	<div id="tabs">
		<ul>
			<li id="populationBaseInfo"><a href="${path}/baseinfo/elderlyPeopleManage/viewCommonPopulation.action?population.id=${population.encryptId}">基本信息</a> </li>
			<li><a href="${path}/baseinfo/elderlyPeopleManage/viewElderlyPeople.action?population.id=${population.encryptId}">业务信息</a> </li>
			<pop:JugePermissionTag ename="serviceTeamMemberManagement">
			<li><a href="/plugin/serviceTeam/router/routerManage/maintainServiceMemberForPopulation.action?module=<s:property value='@com.tianque.service.util.PopulationType@ELDERLY_PEOPLE'/>&mode=view&id=${population.encryptId}">服务人员</a></li>
...
// 	$.ajax({
// 		url:'${path}/baseinfo/populationTypeManage/getPopulationTypeByPopulationIdAndType.action?orgInternalCode=${population.orgInternalCode}&populationId=${population.encryptId}&populationType=<s:property value="@com.tianque.service.util.PopulationType@ELDERLY_PEOPLE"/>',
// 		async:false,
...
// 				<c:if test='${population.isHaveHouse!=false && null!=population.isHaveHouse}'>
