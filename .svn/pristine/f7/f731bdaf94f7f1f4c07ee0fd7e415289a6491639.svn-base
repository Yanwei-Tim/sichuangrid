package com.tianque.baseInfo.unsettledPopulation.validator;
...
@Service("unsettledPopulationValidator")
public class UnsettledPopulationValidatorImpl extends
		AbstractCountrymenValidator<UnsettledPopulation> {
	...
	public ValidateResult validateBaseInfo(Countrymen domain) {
		...
		if (validateHelper.emptyString(domain.getIdCardNo())) {
			if (domain.getGender() == null
					|| domain.getGender().getId() == null) {
				result.addErrorMessage(getColumNo("gender") + "请选择性别");
			}
		}
		// 省市县验证
	...
	@Override
	public ValidateResult validateSpecializedInfo(UnsettledPopulation domain) {
		ValidateResult result = new ValidateResult();
