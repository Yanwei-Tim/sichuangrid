...
@Service("baseinfoStatisticService")
public class BaseinfoStatisticServiceImpl extends AbstractBaseService implements
		BaseinfoStatisticService {
	private static Logger logger = LoggerFactory
			.getLogger(BaseinfoStatisticServiceImpl.class);
	@Autowired
	...
	public List<BaseinfoStatisticVo> getStatisticInfoForList(long orgId,
		...
		if (year == Calendar.getInstance().get(Calendar.YEAR)
			...
			if (list != null && list.size() > 0) {
				// 总况的情况
				lists = baseInfoStatisticsDao.getTotalStatisticList(
			...
			if (!PopulationCatalog.ALL_YOUTH_POPULATION.equals(type)
				...
				// 总况的历史记录列表
				lists = baseInfoStatisticsDao.getTotalStatisticListFromHistory(
						getStatisticSearchVoList(list, orgId, year, month,
								orginternalcode), orgLevelDistance,
						getAdminsOrgType());
				return createBaseinfoStatisticVoByResultList(lists,
						new StatisticSearchVo());
			}
	...
	private List<BaseinfoStatisticVo> sortByOrgSeq(
		...
		List<BaseinfoStatisticVo> sortedBaseinfoStatisticVo = new ArrayList<BaseinfoStatisticVo>();
	...
	private List<BaseinfoStatisticVo> getRealData(
		...
		if (isIdleYouth(statisticSearchVo.getType())) {
			...
			for (int i = 0; i < dicts.size(); i++) {
				...
				if (PropertyTypes.IDLEYOUTH_STAFF_TYPE.equals(statisticSearchVo
						.getDomainName())) {
					helpList = baseInfoStatisticsDao
		...
		if (isPositiveinfos(statisticSearchVo)) {
			...
			for (int i = 0; i < dicts.size(); i++) {
				statisticSearchVo.setDisplayName(dicts.get(i).getDisplayName());
				statisticSearchVo.setPropertyDict(dicts.get(i));
				List<Map<String, Object>> helpList = baseInfoStatisticsDao
	...
	private HighchartColumnVo getArealDistributionList(
		...
		if (isIdleYouth(statisticSearchVo.getType())) {
			...
			List<Map<String, Object>> list = baseInfoStatisticsDao
	...
	private HighchartColumnVo createHighchartColumnVoByCurrent(
		...
		// 查询帮教情况，并组装数据
		statisticSearchVo.setOrgType(getOrgType(statisticSearchVo));
		List<Map<String, Object>> helpLists = baseInfoStatisticsDao
	...
	private HighchartColumnVo getTotalArealDistributionList(
		...
		List<Map<String, Object>> resultList = baseInfoStatisticsDao
	...
	private HighchartColumnVo findHelpData(List<Map<String, Object>> list,
		...
		String[] propertyDictList = AnalyseUtil.groupMap.get(statisticSearchVo
				.getTableDisplayName());
		int DictSize = 0;
	...
	private void deleteOldDatasFromTable(int year, int month, String type,
			String orgInternalCode) {
		baseInfoStatisticsDao.deleteHistoryStatistic(orgInternalCode, year,
				month, type);
	}
	...
	private boolean checkHistoryStatisticTable(int year, int month) {
		boolean isCreate = tableService.createAnalyseTable(
				AnalyseUtil.IMPORTPERSONTABLENAME, AnalyseUtil.IMPORTPERSONSQL,
				year, month);
	...
	public List<Object[]> getStatisticInfo(int year, int month, String type,
			String orgInternalCode, String domainName) {
		...
		if (year == Calendar.getInstance().get(Calendar.YEAR)
			...
			if (!PopulationCatalog.ALL_YOUTH_POPULATION.equals(type)) {
				List<PopulationCatalog> list = getPopulationCatalogListByType(type);
				if (list != null && list.size() > 0) {
					return getTotalTablePieStatisticList(list, orgInternalCode);
				}
		...
		// 单个表类型分布图
		StatisticSearchVo statisticSearchVo = createStatisticSearchVo(type, 0,
				domainName);
		statisticSearchVo.setYear(year);
		statisticSearchVo.setMonth(month);
		statisticSearchVo.setOrginternalcode(orgInternalCode);
	...
	private List<Object[]> getRealTimeCountByTypeName(
		...
		if (isIdleYouth(statisticSearchVo.getType())) {
			if (PropertyTypes.IDLEYOUTH_STAFF_TYPE.equals(statisticSearchVo
				...
				for (int i = 0; i < dicts.size(); i++) {
					statisticSearchVo.setPropertyDict(dicts.get(i));
					mapList.addAll(baseInfoStatisticsDao
							.getIdleYouthRealTimeCountGroupByType(statisticSearchVo));
				}
			} else {
				mapList = baseInfoStatisticsDao
			...
			if (mapList != null && !mapList.isEmpty()) {
				...
				double total = Double
						.parseDouble(String.valueOf(baseInfoStatisticsDao
		...
		double sum = Double.parseDouble(String.valueOf(baseInfoStatisticsDao
	...
	private List<Object[]> getTotalTablePieStatisticListFromHistory(
			List<PopulationCatalog> list, String orgInternalCode, int year,
			int month) {
		...
		for (PopulationCatalog catalog : list) {
			...
			sum = (Integer) cacheService.get(MemCacheConstant
					.getHistoryStaCachKey(orgInternalCode, year, month,
							catalog.getCatalog()));
			if (sum == null) {
				sum = baseInfoStatisticsDao.countByMapFromHistorySta(queryMap);
				cacheService.set(MemCacheConstant.getHistoryStaCachKey(
						orgInternalCode, year, month, catalog.getCatalog()),
						sum);
			}
	...
	private List<Object[]> getTotalTablePieStatisticList(
		...
		for (PopulationCatalog catalog : list) {
			...
			map = new HashMap<String, Object>();
			sum = baseInfoStatisticsDao.getTotalFromTableByOrgCode(catalog
	...
	private List<Object[]> getStatisticInfoFromHistory(
		...
		if (PopulationCatalog.ALL_YOUTH_POPULATION.equals(statisticSearchVo
				.getType())) {
			total = Double.parseDouble(String.valueOf(baseInfoStatisticsDao
		...
		for (PropertyDict propertyDict : propertyDictList) {
			...
			int sum = baseInfoStatisticsDao.countByMapFromHistorySta(map);
	...
	private HighchartColumnVo getArealDistributionListFromHistory(
		...
		if (null != statisticSearchVo.getDomainName()) {
			...
			// 没有分组的情况
			List<Map<String, Object>> lists = baseInfoStatisticsDao
	...
	private void generateStatisticByType(int year, int month, String orgCode,
		...
		if (isIdleYouth(type)) {
			...
			/** 之前的方法有问题total、monthcreate、attentionnum没有用值sum在用 先不生成这三个字段 */
	...
	private StatisticSearchVo constructionStatisticSerchVo(int year, int month,
	...
	private void generatePositiveInfoHistoryStatistic(int year, int month,
			String type, String orgCode) {
		baseInfoStatisticsDao.generatePositiveInfoHistoryStatistic(year, month,
	...
	private void generateActualHouseHistoryStatistic(int year, int month,
		...
		for (Long countryOrgId : countryList) {
			...
			if (!BaseInfoTables.HOUSEINFO_KEY.equals(tableName)) {
				baseInfoStatisticsDao.generateActualHouseHistoryStatistic(year,
						month, CalendarUtil.getMonthStart(year, month),
						CalendarUtil.getNextMonthStart(year, month), type,
						PopulationCatalog.parse(type).getStatisticListSetting()
								.getTableName(), null);
	...
	private void generateIdleYouthHistoryStatistic(int year, int month,
			String type, String orgCode) {
		baseInfoStatisticsDao.generateIdleYouthHistoryStatisticByType(year,
				month, CalendarUtil.getMonthStart(year, month),
				CalendarUtil.getNextMonthStart(year, month), type,
				PopulationCatalog.parse(type).getStatisticListSetting()
						.getTableName());

		baseInfoStatisticsDao.generateIdleYouthHistoryStatisticByAge(year,
				month, CalendarUtil.getMonthStart(year, month),
				CalendarUtil.getNextMonthStart(year, month), type,
				PopulationCatalog.parse(type).getStatisticListSetting()
						.getTableName());

		// baseInfoStatisticsDao.generateIdleYouthByAgeAndLeaderCount(year,
		// month,
		// CalendarUtil.getMonthStart(year, month),
		// CalendarUtil.getNextMonthStart(year, month), type,
		// PopulationCatalog.parse(type).getStatisticListSetting()
	...
	private void generateHistoryStatisticByType(int year, int month,
			String type, String orgCode) {
		baseInfoStatisticsDao.generateHistoryStatisticByType(year, month,
				CalendarUtil.getMonthStart(year, month),
				CalendarUtil.getNextMonthStart(year, month), type,
				PopulationCatalog.parse(type).getStatisticListSetting()
						.getTableName(), PopulationCatalog.parse(type)
	...
	public int getImportPersonCount(Long orgId) {
		...
		HighchartColumnVo highchartColumnVo = baseInfoStatisticsDao
	...
	public void createHistoryStatisticByType(int year, int month, String type,
		...
		// 清除数据
		if (!isCreateTable) {
			baseInfoStatisticsDao.deleteHistoryStatistic(null, year, month,
					type);
		}
	...
	private List getYouthByType(StatisticSearchVo statisticSearchVo) {
		...
		for (int i = 0; i < dicts.size(); i++) {
			...
			statisticSearchVo.setOrgType(getOrgType(statisticSearchVo));
			List<Map<String, Object>> subList = new ArrayList<Map<String, Object>>();
			subList = baseInfoStatisticsDao.countYouthByType(statisticSearchVo);
			baseLists.addAll(subList);
		}
	...
	private List getYouthListByPoliticalBackGround(
		...
		int total = Integer.parseInt(String.valueOf(baseInfoStatisticsDao
				.getTotalYouthByOrgCode(statisticSearchVo)));
		int exceptOthers = 0;
		for (Map map : mapList) {
	...
	@Override
	public void manualAllYouthPopulationCompletion(
		...
		for (int year = packetStatisticsVo.getStartYear(); year <= (Integer) packetStatisticsVo
			...
			for (int month = startMonth; month <= endMonth; month++) {
				...
				createHistoryStatisticByType(year, month,
						packetStatisticsVo.getTypeName(), null);
				logger.error("执行" + packetStatisticsVo.getTypeName() + "_"
