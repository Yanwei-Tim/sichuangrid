...
<div id="dialog-form" title="" class="container container_24">
	...
	<pop:token />
		...
		<input type="hidden" id="" name="helpPrecord.personnelType" value="${personnelType}"/>
		...
		<div class="grid_6 lable-right">
			<label class="form-lbl">走访时间：</label>
		</div>
		<div class="grid_15">
		...
		<div class='clearLine'>&nbsp;</div>
		<div class="grid_6 lable-right">
			<label class="form-lbl">走访地点：</label>
		</div>
		<div class="grid_15">
		...
		<div class='clearLine'>&nbsp;</div>
		<div class="grid_6 lable-right">
			<label class="form-lbl">走访情况：</label>
		</div>
		<div class="grid_15">
...
<script type="text/javascript">
	$(document).ready(function(){
		$("#personTypeNames").val($("#superiorPersonTypeName").val());
	...
	<s:if test='"edit".equals(mode)'>
		...
		$("#helpPrecordForm").formValidate({
			...
			messages:{
				...
				"helpPrecord.address":{
					required:"走访地址不能为空",
					maxlength:$.format("走访地址最多输入{0}个字符")
				}
				,
				"helpPrecord.events":{
					maxlength:$.format("走访情况最多输入{0}个字符")
				}
		...
		$("#helpPersonName").personnelComplete({
			...
			postDataFunction: function(){
			    var personnelId=$("#domainId").val();
			   	var  personnelType = $("#superiorKeyType").val();
