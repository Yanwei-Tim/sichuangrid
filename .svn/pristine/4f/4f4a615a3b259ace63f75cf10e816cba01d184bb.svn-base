	...
	<div id="personSelectBox">
		<div class="personSelectMenus" id="personSelectMenus">
			<ul>
				...
				<li><a href="javascript:;"><span></span>按岗位发送</a></li>
			</ul>
		</div>
		<%-- 站内联系人 --%>
		...
		<div class="personSelectPanel" >
	    	<div class="grid_8 lable-right">
		  	...
		  	<div class="grid_16">
				<input type="text" id="roleOrgTree" class="form-txt"/>
	    	</div>
		    <div class="grid_24 departmentBox">
			    <div class="grid_8 lable-right">
					<label>范围：</label>
			  	...
			  	<div class="grid_16">
					<select id="roleScope" class="form-txt">
						<option selected="selected" value=<s:property value="@com.tianque.platformMessage.constants.RoleScope@ONLY_SELF"/>>仅限本级</option>
						<option value=<s:property value="@com.tianque.platformMessage.constants.RoleScope@DIRECTLY_CHILD"/>>直属下辖</option>
						<option value=<s:property value="@com.tianque.platformMessage.constants.RoleScope@ALL_CHILD"/>>所有下辖</option>
					</select>
		    	</div>
	    	</div>
	    	<div class='clearLine'>&nbsp;</div>
	    	<div>
				<ul id="roleList" class="orgLevelBox"></ul>
			</div>
		</div>
	...
	if((data!=null && data.length > 0) || (userReciverIds!='' && userReciverIds.length > 0) ){
		...
		for(var j = 0 ; j < userReciverIds.length ; j++){
			...
			if(id !=''&& id != '-1' && id !='-2' && id.lastIndexOf('-levelList')==-1 && id.lastIndexOf('-roleList')==-1 && $("#orgLevelContactersList ul li[id='"+id+"']").length==0){
				var name = $("#userReceivers").data(id);
				if(name==undefined){
					//通过输入框输入的
...
function clickWorkcontactOrGroupCheckbox(checkbox) {
	if (checkbox.attr("checked")) {
		...
		$("#userReceivers").data(checkbox.val(),checkbox.attr("name"));
		...
		$("#userReceivers").removeData(cval);
		$("#userReceivers").removePersonnelCompeleteValue(cval);
...
function clickWorkcontacterCheckbox(checkbox){
	if(checkbox.attr("checked")){
	    ...
	    $("#viewMemberListDlg").createDialog({
			...
			height: 300,
			title:'选择群组成员',
			url:'${path}/contact/myGroupManage/findMyGroupHasContactersToSelectPersion.action?myGroup.id='+id+'&myGroup.belongClass=workContact',
			buttons: {
				"确定" : function(event){
...
function setOrgLevelCheckboxState(level){
	...
	//已选择层级部门收件人的id
	var orgReceiversArray = $('#orgReceivers').val().split(',');
    for(var i = 0 ; i < orgLevelCheckboxs.length ; i++ ){
    	...
    	if(orgReceiversArray.contains(id)){
    		orgLevelCheckboxs[i].checked=true;
        }
    }
    isSelectedAllOrgLevel(level);
    //对非当前层级选中的层级联系人遍历 并添加到已选择对象中去
    for(var j = 0 ; j < orgReceiversArray.length; j++){

    	var id = orgReceiversArray[j];
        if(id==null||id==''){
        	continue;
        }
        ...
        if(id!='' && $("#orgLevelContactersList>ul li[id='"+id+"']").length==0){
        	var nameAndLevel= $("#orgReceivers").data(id);
...
//     	 $("#selectAllLevelAdministrativeOrg,#selectAllLevelFunctionalOrg").attr("disabled",true);
//     }else{
//    	   	$("#selectAllLevelAdministrativeOrg").attr("disabled",false);
//    	   	if(level < <s:property value="@com.tianque.domain.property.OrganizationLevel@DISTRICT"/>){
//    	   		$("#selectAllLevelFunctionalOrg").attr("disabled",true);
//    	   	}else{
//    	   		$("#selectAllLevelFunctionalOrg").attr("disabled",false);
//    	   	}
//     }
}
...
function clickOrgLevelCheckbox(checkbox,level){
		...
		if(checkbox.attr("checked")){
		   ...
		   var orgReceiversArray = $('#orgReceivers').val().split(',');
		   if(!orgReceiversArray.contains(id)){
			   $("#orgReceivers").val($("#orgReceivers").val()+','+id);
		   }
		   $("#userReceivers").setPersonnelCompleteVal({
		   ...
		   $("#orgReceivers").data(id,[$("#areaOrgTree").val(),checkbox.val()]);
		}else{
			$("#orgReceivers").val($("#orgReceivers").val().replace(','+id , ''));
			...
			$("#"+id).remove();
			$("#userReceivers").removePersonnelCompeleteValue(checkbox.val()+"-levelList");
			$("#orgReceivers").removeData(id);
		}
}
...
//按岗位发送组织树
var roleOrgTree=$("#roleOrgTree").treeSelect({
	width:100,
	allOrg:false,
	nodeUrl:"/sysadmin/orgManage/ajaxOrgsForExtTree.action?directlySupervisor=true",
	rootId:provienceId,
	...
	showFunctionalOrg:true,
	directlySupervisor:true,
	changeFun:function(){ return getRoleData($("#roleOrgId").val(),$("#roleScope").val());}
});

//选择组织树时加载加载该组织下的岗位
function getRoleData(orgId,roleScope){
	$.ajax({
		async:false,
		url:"${path}/interactive/outboxPlatformMessageManage/findRoles.action",
		data:{
			"orgId":orgId,
			"roleScope":roleScope
		},
...
function showRoleList(data){
	var userLeavel = '<s:property value="@com.tianque.core.util.ThreadVariable@getUser().getOrganization().getOrgLevel().getInternalId()"/>';
	if($.getSelectedNode(roleOrgTree) != null){
		var roleOrgTreeLevel = $.getSelectedNode(roleOrgTree).attributes.orgLevelInternalId;
		if(roleOrgTreeLevel > userLeavel){
			$("#roleScope").attr("disabled",true);
		}else{
	...
	$("#roleList").empty();
	var roleReceiversArray = $('#roleReceivers').val().split(',');
	if(data!=null && data.length > 0 ){
		
		$("#roleList").append('<li><label><input type="checkbox" id="selecAllRole" />全选</label></li>');
		
		for(var i=0; i < data.length ; i++){
			$("#roleList").append(
				'<li>' +
					'<label><input type="checkbox" key="role" name="'+data[i].roleName+'" id="role_'+data[i].id+'" value="'+data[i].id+'"/></label> '+data[i].roleName+
				'</li>');
			var id = data[i].id +'_'+$("#roleOrgId").val()+'_'+$("#roleScope").val();
			if(roleReceiversArray.contains(id)){
				$('#role_'+data[i].id).attr('checked',true);
				if($("#orgLevelContactersList ul li[id='"+id+"']").length==0){
					$("#orgLevelContactersList > ul").prepend("<li key='role' id='"+id+"'><label>"+$("#roleOrgTree").val()+"下的全部"+data[i].roleName+"</label><a href='javascript:;' class='delete'>删除</a></li>");
				}
				roleReceiversArray.remove(id);
			}
		}
	}

	for(var j = 0 ; j < roleReceiversArray.length ; j++){
		var id = roleReceiversArray[j];
		if(id != ''){
			var orgNameAndRoleName =  $("#roleReceivers").data(id);
			if(orgNameAndRoleName[0]!= null && orgNameAndRoleName[0].indexOf("下的全部") >=0){
				if( $("#orgLevelContactersList ul li[id='"+id+"']").length==0){
					$("#orgLevelContactersList > ul").prepend("<li key='role' id='"+id+"'><label>"+orgNameAndRoleName[0]+"</label><a href='javascript:;' class='delete'>删除</a></li>");
				}
			}else{
				if( $("#orgLevelContactersList ul li[id='"+id+"']").length==0){
					$("#orgLevelContactersList > ul").prepend("<li key='role' id='"+id+"'><label>"+orgNameAndRoleName[0]+"下的全部"+orgNameAndRoleName[1]+"</label><a href='javascript:;' class='delete'>删除</a></li>");
				}
			}
		}
	}
...
//点击岗位checkbox
function clickRoleCheckbox(checkbox){
	var id = checkbox.val()+'_'+$("#roleOrgId").val()+"_"+$("#roleScope").val();
	if(checkbox.attr("checked")){
		isSelectAllRole();
		if($("#orgLevelContactersList>ul li[id='"+id+"']").length==0){
			$("#orgLevelContactersList > ul").prepend(
					'<li key="role" id="'+id+'">' +
							'<label>'+$("#roleOrgTree").val()+'下的全部'+checkbox.attr('name')+'</label>'+
							'<a href="javascript:;"  class="delete">删除</a>'+
					'</li>');
		}
	    var roleReceiversArray = $('#roleReceivers').val().split(',');
	    if(!roleReceiversArray.contains(id)){
		   $("#roleReceivers").val($("#roleReceivers").val()+','+id);
	    }
	    $("#userReceivers").setPersonnelCompleteVal({
			//value : "-2",
			value : checkbox.val()+"-roleList",
			label : $("#roleOrgTree").val()+"下的全部"+checkbox.attr('name'),
			desc  : ""
		});
	    $("#roleReceivers").data(id,[$("#roleOrgTree").val(),checkbox.attr('name'),$("#roleScope").val()]);
	}else{
		$("#roleReceivers").val($("#roleReceivers").val().replace(','+id , ''));
		$("#selecAllRole").attr("checked",false);
		$("#"+id).remove();
		$("#userReceivers").removePersonnelCompeleteValue(checkbox.val()+"-roleList");
		$("#roleReceivers").removeData(id);
...
$(function(){
	...
	//加载rootOrg下的岗位
	roleOrgTree.getLoader().on("load" , function(){
		if($("#roleOrgId").val() == "" || $("#roleScope").val() == ""){
			getRoleData(roleOrgTree.root.firstChild.id,$("#roleScope").val());
		}else{
			getRoleData($("#roleOrgId").val(),$("#roleScope").val());
		}
	});
	...
	//全选岗位
	$("#roleList").delegate("#selecAllRole","click",function(){
		var isChecked=$(this).attr("checked");
		$("#roleList li input[id!='selecAllRole']").each(function(){
			if(isChecked){
				 if(!$(this).attr("checked")){
			     	 $(this).attr("checked",true);
			     	 clickRoleCheckbox($(this));
			     }
			}else{
				if($(this).attr("checked")){
			     	 $(this).attr("checked",false);
	...
	//选择单个岗位
	$("#roleList").delegate("li input[key='role']","click",function(){
		clickRoleCheckbox($(this));
	});

	//切换岗位范围重新加载该部门下的岗位
	$("#roleScope").change(function(){
		getRoleData($("#roleOrgId").val(),$(this).val());
	});
