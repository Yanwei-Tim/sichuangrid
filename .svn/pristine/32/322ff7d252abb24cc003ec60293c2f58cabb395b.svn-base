...
@Service("issueSerialKeyGenerator")
@Transactional
public class IssueSerialKeyGenerator implements KeyGenerator, IssueSerialNumberGenerator {
	...
	public String getNextKey(String prefix) {
		...
		SystemKeyInfo systemKeyInfo = systemKeyInfoService.getSimpleSystemKeyInfoByName(prefix);
		if (systemKeyInfo == null)
			systemKeyInfo = addNewSystemKeyInfo(prefix);
		systemKeyInfo.setCurNum(systemKeyInfo.getCurNum() + systemKeyInfo.getStep());
		...
		if (curNum.length() > 4)
			...
			curNum = DEFAULT_FIX_CONTEXT.substring(0, 4 - curNum.length()) + curNum;
	...
	private SystemKeyInfo addNewSystemKeyInfo(String prefix) {
		return systemKeyInfoService.addSystemKeyInfo(createSystemKeyInfoInstance(prefix));
	}

	private SystemKeyInfo addNewSystemKeyInfo() {
		return systemKeyInfoService.addSystemKeyInfo(createSystemKeyInfoInstance());
	}
	...
	@Override
	public String nextValue(IssueNew issue) {
		String prefix = getCurrentIssuePrefix(issue);
		return getNextKey(prefix);
	}

	private String getCurrentIssuePrefix(IssueNew issue) {
		Organization district = organizationDubboService.findSomeLevelParentOrg(issue.getOccurOrg()
				.getId(), OrganizationLevel.DISTRICT);
		if (district == null) {
			district = organizationDubboService.findOrganizationsByParentId(null).get(0);
		}
		return CalendarUtil.format("yyMMdd", new Date()) + district.getDepartmentNo();
	}
}
