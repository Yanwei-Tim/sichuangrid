...
<table id='tb' class="tb" border=0 cellpadding=0 cellspacing=0 style='margin:10px 10px;border-collapse:collapse;table-layout:fixed;width:780pt'>
	<s:if test='"add".equals(mode)'>
	<tr>
		<td colspan=7 height=44 id="reportName" class=head style='text-align:center;border:0px;height:33.0pt;font-size:25px; font-weight:bold;'>${dailyYear.name}年<span id="selectedMonthForvill"></span>${organization.orgName}矛盾纠纷排查情况表</td>
	</tr>
	</s:if>
	<s:else>
	<tr>
		<td colspan=7 height=44 id="reportName" class=head style='text-align:center;border:0px;height:33.0pt;font-size:25px; font-weight:bold;'>${disputEsexamine.name}</td>
	</tr>
	</s:else>
	<tr>
		<td colspan=4 class=head style='height:32.25pt;border:0px;border-bottom:1px solid gray;' id="areacode">
	...
	<tr id='c_429_' >
		...
		<td class=label >调处数</td>
		<td name="imptVilImptDealcount" yearCountTarget="imptYearImptDealcount" class="toDataJosn" countName="imptUnImptDealcount-data" countTarget="imptUnImptDealcount">${disputEsexamine.imptVilImptDealcount}</td>
	...
	<tr id='c_429_' >
		...
		<td class=label >成功数</td>
		<td name="imptVilImptFinishcount" yearCountTarget="imptYearImptFinishcount" class="toDataJosn" countName="imptUnImptFinishcount-data" countTarget="imptUnImptFinishcount">${disputEsexamine.imptVilImptFinishcount}</td>
	...
	<tr id='c_430_'>
		...
		<td name="concentrateVilFinishcount" yearCountTarget="concentrateYearFinishcount" yearCountTarget="concentrateYearFinishcount" class="toDataJosn" countName="concentrateUnFinishcount-data" countTarget="concentrateUnFinishcount">${disputEsexamine.concentrateVilFinishcount}</td>
	...
	<tr height=37 >
		<td colspan=2 height=37 class=bottom>签发人：<input <s:if test='"view".equals(mode)'>readonly="readonly"</s:if> id="checkUser" type= "text"   maxLength= "12 " value="${disputEsexamine.dealPerson}"   name="disputEsexamine.dealPerson"  size= "12 "   style= "BORDER-BOTTOM:   #000000   1px solid; BORDER-LEFT:   #ffffff   0px   solid;   BORDER-RIGHT:   #ffffff   0px   solid;   BORDER-TOP:   #ffffff   0px   solid "/>  </td>
	...
	function paddingTdData(data){
		...
		$.each(toDataJosnTds,function(i){
			if(data==null){
				...
				if($(toDataJosnTds[i]).attr("name")=="totalYearDealrate"){
					//alert(data[$(toDataJosnTds[i]).attr("name")]);
				}
