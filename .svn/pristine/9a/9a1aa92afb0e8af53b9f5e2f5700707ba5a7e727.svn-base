...
$(document).ready(function(){
	var gridoption = {
		url:"${path}/workingRecord/seriesSecurityManage/seriesSecurityList.action?organization.id="+currentOrgId+"&dailyDirectoryId="+dailyId,
		datatype: "json",
		colNames:['id','活动时间','活动名称','创建情况','附件','录入时间'],
	   	colModel:[
	...
	$("#add").click(function(event){
		...
		if(!selectedNode.attributes.leaf){
			...
			modal : true,
			url:"${path}/workingRecord/seriesSecurityManage/dispatch.action?mode=add"+"&dailyDirectoryId="+dailyDirectoryId,
			buttons :{
				"保存" : function(){
	...
	$("#update").click(function(event){
		...
		$("#dailyLogMaintanceDialog").createDialog({
			...
			url:"${path}/workingRecord/seriesSecurityManage/dispatch.action?mode=edit&seriesSecurity.id="+selectedId,
			buttons : {
				"保存" : function(){
					$("#dailyLog-form").submit();
				},
	...
	$("#reload").click(function(){
		$("#dailyLogList").setGridParam({
			url:"${path}/workingRecord/seriesSecurityManage/seriesSecurityList.action?organization.id="+currentOrgId,
			postData:{
...
function doubleClickTable(selectedId){
	$("#dailyLogMaintanceDialog").createDialog({
		...
		modal : true,
		url:"${path}/workingRecord/seriesSecurityManage/dispatch.action?mode=view&seriesSecurity.id="+selectedId,
		buttons :{
			"关闭" : function(){
				$(this).dialog("close");
			}
...
function showDailyLogAttachFile(){
	$.each($(".popUpMore"), function(i, n){
		$.ajax({
			url:"${path}/workingRecord/seriesSecurityManage/getSeriesSecurityById.action?seriesSecurity.id="+$(n).attr("seriesSecurityId")+"&organization.id="+currentOrgId,
			success:function(dailylog){
				var popMoreData = [];
				for(var j = 0; j < dailylog.dailyAttachFiles.length; j++){
					popMoreData[j]={id:dailylog.dailyAttachFiles[j].id, url:'${path}/workingRecord/seriesSecurityManage/downloadDailyAttachFile.action?dailyAttachFile.id='+dailylog.dailyAttachFiles[j].id, text:dailylog.dailyAttachFiles[j].fileName,clickFun:function(){}};
				}
...
function formatterAttachFile(el,options,rowData){
	if(rowData.dailyAttachFiles.length>0){
		return "<div style='clear:both' align='center'><a href='javascript:;' id='seriesSecurity_"+rowData.id+"' style='color:#666666' class='popUpMore' seriesSecurityId='"+rowData.id+"' >附件>></a></div>";
	}
	return "";
}
...
function deleteDailyLog(selectedIds){
	$.ajax({
		url:'${path}/workingRecord/seriesSecurityManage/deleteSeriesSecurity.action?seriesSecurityIds='+selectedIds+"&organization.id="+currentOrgId,
