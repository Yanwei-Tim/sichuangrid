...
@Component("positiveInfoRecordValidator")
public class PositiveInfoRecordValidatorImpl implements DomainValidator<PositiveInfoRecord> {
	@Autowired
	public ValidateHelper validateHelper;

	@Override
	public ValidateResult validate(PositiveInfoRecord domain) {
		ValidateResult result = new ValidateResult();
		if (validateHelper.nullObj(domain.getRecordDate())) {
		...
		if (validateHelper.emptyString(domain.getFamilyPhone())) {
