package com.tianque.plugin.dataManage.population.elderlyPeopleTemp.validator;
...
@Component("elderlyPeopleTempValidator")
public class ElderlyPeopleTempValidatorImpl extends
		AbstactDataManageValidator<ElderlyPeopleTemp> {

	@Override
	public ValidateResult validateSpecializedInfo(ElderlyPeopleTemp domain) {
		...
		if (domain.getPension() != null && domain.getPension() > 999999999) {
			result.addErrorMessage(getColumNo("pension") + "退休金不能大于999999999元");
		}
		if (domain.getPension() != null && domain.getPension() <= 0) {
			result.addErrorMessage(getColumNo("pension") + "退休金不能小于0元");
		}
