...
<form id="maintainShiftForm" method="post" action="${path}/baseinfo/druggyManage/shiftDruggy.action">
<pop:token />
	...
	<input name="druggy.organization.id" id="shiftOrgId" value="" type="text" style="width:0px;border:0px;height:0px;overflow:hidden;position:absolute;top:25px;left:70px;">
	<input name="druggyIds" id="druggyIds" value="${druggyIds}" type="hidden">
</form >
...
$(document).ready(function(){
	...
	$("#maintainShiftForm").formValidate({
		...
		rules:{"druggy.organization.id":{isGridOrganization:true,isModernOrg:true}},
		messages:{"druggy.organization.id":{isGridOrganization:"数据只能转移得到网格",isModernOrg:"数据不能转移到当前网格"}}
	});
});
</script>