...
<pop:JugePermissionTag ename="addLedgerSteadyWorkRecord">
	<a id="add" href="javascript:;"><span>新增</span></a>
</pop:JugePermissionTag> <pop:JugePermissionTag ename="updateLedgerSteadyWorkRecord">
	<a id="update" href="javascript:void(0)"><span>修改</span></a>
</pop:JugePermissionTag> <pop:JugePermissionTag ename="deleteLedgerSteadyWorkRecord">
	<a id="delete" href="javascript:void(0)"><span>删除</span></a>
</pop:JugePermissionTag> 
    <pop:JugePermissionTag ename="queryLedgerSteadyWorkRecord">
...
var issueButtons={
		...
		showButtons:function(showBtns){
			...
			//if("completed" == $("#jurisdictionsViewType").val()){
...
$(function(){
	...
	$("#add").click(function(event){
		$("#steadyWorkDialog").createDialog({
			...
			height:500,
			title:'新增稳定台账信息',
			url:'${path}/threeRecordsIssue/ledgerSteadyWorkManage/dispatchOperate.action?mode=add&ledgerSteadyWork.organization.id='+$("#currentOrgId").val(),
			buttons: {
		   		"保存" : function(event){
		   			$("#maintainForm").submit();
		   			//$("#maintainForm").submit();
	...
	$("#search").click(function(event){
		$("#issueDialog").createDialog({
			...
			height: 300,
			title: "稳定查询-请输入查询条件",
			url: "${path}/threeRecordsIssue/ledgerSteadyWorkManage/dispatchOperate.action?mode=search&keyId="+$("#currentOrgId").val()+'&tag='+viewType.value+"&selectedIssueType=",
			buttons: {
		   		"查询" : function(event){
		   				searchIssue();
			        	$(this).dialog("close");
	...
	$("#import").click(function(event){
        $("#steadyWorkDialog").createDialog({
            ...
            height: 230,
            title:'数据导入',
            url:PATH+'/account/dataTrans/import.jsp?isNew=1&dataType=SteadyWorkData&dialog=steadyWorkDialog&startRow=1&templates=STEADYWORKDATA&listName=ledgerSteadyWorkRecordViewList',
            buttons:{
                "导入" : function(event){
                    $("#mForm").submit();
                },
    ...
    $("#printIssue").click(function (event) {
        ...
        $("#issueDialog").createDialog({
            ...
            height: 600,
            title: '稳定工作信息打印',
            url: '${path}/threeRecordsIssue/ledgerSteadyWorkManage/dispatchOperate.action?mode=print&keyId=' + selectedIssue.encryptIdByIssueId,
            buttons: {
                "打印": function (event) {
                    $("#steadyWorkPrint").printArea();
                },
                "关闭": function () {
                    $(this).dialog("close");
                }
    ...
    $("#exportIssue").click(function (event) {
        ...
        var selectedIssue = getSelectedData();
        var url='${path}/threeRecordsIssue/ledgerSteadyWorkManage/exportExcel.action?keyId=' + selectedIssue.encryptIdByIssueId;
	...
	$("#conceptTop,#conceptBottom").click(function(event){
		...
		if(currentOrgId !=selectedIssue['targetOrg.id']){
	   	  $.messageBox({level: 'warn', message: "该稳定台账不可受理，请进行其它操作！"}); 
	   	  return;
	   	}
	...
	$("#dealTop,#dealBottom").die().live("click",function(event){
		...
		if(selectedIssue.stateCode!=null && '办理中' == selectedIssue.stateCode){
			...
			$.messageBox({level:'warn',message:"该条稳定类信息不可办理，请先受理后再作办理操作！"});
		}
	});
...
function removePoorAndStep(keyId) {
    $.ajax({
        url: "/threeRecordsIssue/ledgerSteadyWorkManage/deleteLedgerSteadyWork.action",
        data: {
            "keyId": keyId
        },
        success: function (responseData) {
            if (responseData == true) {
                $.messageBox({message: "已经成功删除该稳定信息处理信息!"});
                //onLoadDelay();
                searchIsssueById();
                getMessageByUser();
            } else {
                $.messageBox({level: 'error', message: "删除稳定信息失败!"});
            }
        }
    });
}
...
            searchVo[data[i].name] = data[i].value;
	...
	$("#ledgerSteadyWorkList").setPostData(pageData);
	$("#ledgerSteadyWorkList").trigger("reloadGrid");
...
function dealIssue(issueStepId){
	...
	$("#issueDialog").createDialog({
		...
		height:550,
		title:'办理',
		url:'${path}/threeRecordsIssue/ledgerSteadyWorkIssueManage/dispatchOperate.action?mode=deal&keyId='+issueStepId+'&issuesKeyId='+issuesKeyId,
		buttons: {
			"确定" : function(event){
...
function simpleDealIssue(issueStepId,dealType){
	...
	$("#issueDialog").createDialog({
		...
		height:200,
		title:dealTitil,
		url:'${path}/threeRecordsIssue/ledgerSteadyWorkIssueManage/dispatchOperate.action?dealCode='+dealType+'&keyId='+issueStepId,
		buttons: {
			"确定" : function(event){
				$("#issueDealForm").submit();
			},
...
function printAcceptForm(id){
	$("#issueDialog").createDialog({
	    ...
	    height: 400,
	    title: "受理单",
	    url: '${path}/threeRecords/acceptForm/dispatch.action?mode=add&keyId='+id+'&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@STEADYWORK"/>',
	    ajaxType:'post',
	    buttons: {
	        "保存打印": function (event) {
	            $("#accept_form").submit();
...
function editThreeRecords(id){
	$("#steadyWorkDialog").createDialog({
		...
		height:500,
		title:'修改台账信息',
		url:'${path}/threeRecordsIssue/ledgerSteadyWorkManage/dispatchOperate.action?mode=edit&id='+id,
		buttons: {
	   		"保存" : function(event){
	   			$("#maintainForm").submit();
	   			searchIsssueById();
...
$("#feedBack").click(function(event){
	...
	$.post('${path}/threeRecords/feedBack/isCanFeedBack.action?&ledgerId='+ selectedIssue.id+'&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@STEADYWORK"/>', function (data) {
		if(!data){
			$.messageBox({level:'warn',message: "该 稳定信息不可反馈，请进行其它操作！"});
			return;
		}
		$("#feedBackDialog").createDialog({
			...
			height: 400,
			title: "反馈",
			url: '${path}/threeRecords/feedBack/dispatch.action?keyId=' + selectedIssue.stepId+'&ledgerId='+ selectedIssue.id+'&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@STEADYWORK"/>',
			buttons: {
		   		"反馈" : function(event){
...
$("#replyFormTop").click(function (event) {
    ...
    var selectedIssue = getSelectedData();
     $("#feedBackDialog").createDialog({
         ...
         height: 400,
         title: "回复单",
         url: '${path}/threeRecords/replyForm/dispatch.action?keyId=' + selectedIssue.stepId+'&serialNumber='+ selectedIssue.serialNumber + '&ledgerId=' + selectedIssue.id + '&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@STEADYWORK"/>',
         buttons: {
             "回复": function (event) {
