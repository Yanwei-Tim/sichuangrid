...
<input type="hidden" id="orgType" value="<s:property value='#parameters.orgType'/>"/>
 ...
 <div style="margin-left: 5px">
   ...
   <br/><br/><br/><br/>
   <jsp:include page="/account/informationAnalysis/mould/currentYearCollectDoneRateMouldDetail.jsp"  flush="true"/>
...
function initYearCollect(){
	var statGridCollect = initCurrentYearCollectGridCollect("当年建账办结情况详细",false);
...
function getPrimaryOrgStat(statGridCollect){
	$.ajax({
		...
		data:{
			...
			"accountReport.reportType":<s:property value="@com.tianque.plugin.account.constants.LedgerReportType@YEAR_COLLECT_DONE_DETAIL"/>,
