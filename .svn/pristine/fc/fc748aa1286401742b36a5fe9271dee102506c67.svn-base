...
import com.tianque.plugin.analysisNew.dao.SearchPrimaryOrganizationDataColumnNewDao;
 ...
 * 统计队伍信息
 */
@Service("searchPrimaryOrganizationDataColumnNewService")
public class SearchPrimaryOrganizationDataColumnNewServiceImpl implements
		SearchPrimaryOrganizationDataColumnNewService {
	@Autowired
	private OrganizationDubboService organizationDubboService;
	@Autowired
	private SearchPrimaryOrganizationDataColumnNewDao searchPrimaryOrganizationDataColumnNewDao;
	@Autowired
	 ...
	 * @return
	 */
	private List<PrimaryOrganizationDataColumnTwoNewVo> dealDataColumnVoListForHj(
			List<PrimaryOrganizationDataColumnTwoNewVo> list) {
		int teamCount = 0;
		int teamMemberCount = 0;
		...
		PrimaryOrganizationDataColumnTwoNewVo primaryOrganizationDataColumnVo = new PrimaryOrganizationDataColumnTwoNewVo();
		if (list != null && list.size() > 0) {
			for (PrimaryOrganizationDataColumnTwoNewVo columnVo : list) {
	 ...
	 * @return
	 */
	private List<PrimaryOrganizationDataColumnTwoNewVo> dealDataColumnVoListForZk(
			List<PrimaryOrganizationDataColumnTwoNewVo> list) {
		int teamCount = 0;
		int teamMemberCount = 0;
		if (list != null && list.size() > 0) {
	...
	@Override
	public List<PrimaryOrganizationDataColumnTwoNewVo> getPrimaryOrganizationDataColumnVoList(
			Long parentOrgId) {
		...
		List<PrimaryOrganizationDataColumnTwoNewVo> listData = null;
		if (key != null) {
			listData = (List<PrimaryOrganizationDataColumnTwoNewVo>) cacheService
					.get(MemCacheConstant.ANALYSE_GRASSROOTS_NAMESPACE, key);
			if (listData != null) {
				return listData;
			}
		}
		List<PrimaryOrganizationDataColumnTwoNewVo> list = searchPrimaryOrganizationDataColumnNewDao
				.searchPrimaryOrganizationDataColumnByOrgInternalCode(parentOrgId);

		List<PrimaryOrganizationDataColumnTwoNewVo> dealListForZk = dealDataColumnVoListForZk(list);
		List<PrimaryOrganizationDataColumnTwoNewVo> dealFinal = dealDataColumnVoListForHj(dealListForZk);
		if (key != null) {
	...
	//
	// List<SearchPrimaryOrganizationDataColumnVo> columnVoList =
	// searchPrimaryOrganizationDataColumnNewDao
