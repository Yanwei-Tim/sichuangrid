...
public class CompletedIssueServiceImpl implements CompletedIssueService {
	...
	public CompletedIssue addCompletedIssue(CompletedIssue completedIssue) {
		Organization currentOrg=organizationDubboService.getSimpleOrgById(completedIssue.getCurrentOrg().getId());
		if(currentOrg != null){
			completedIssue.setCurrentOrgFunctionalType(currentOrg.getFunctionalOrgType() == null ? 0L:currentOrg.getFunctionalOrgType().getId());
			completedIssue.setCurrentOrginternalCode(currentOrg.getOrgInternalCode());
			completedIssue.setCurrentOrgLevel(currentOrg.getOrgLevel().getId());
		}
