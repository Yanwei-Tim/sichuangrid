...
<@pop.formatterProperty name="position" domainName="@com.tianque.plugin.serviceTeam.util.Constants$ServiceTeamPositionPropertyTypes@TEAMPOSITION" />
	...
	$("#addServiceTeamMember").click(function(event){
		$("#_serviceTeamMemberDialog").createDialog({
			...
			width: 700,
			height: 480,
			url:'${path}/baseinfo/routerManage/maintainServiceMemberForLocation.action?dailogName=_serviceTeamMemberDialog&mode=add&serviceTeamMemberVo.org.id='+getCurrentOrgId(),
			buttons: {			
				"关闭" : function(){
		        	$(this).dialog("close");
	...
	$("#addGuardersForObject").click(function(event){
		$("#_serviceTeamGuardersDialog").createDialog({
			...
			width: 600,
			height: 280,
			url:'${path}/baseinfo/routerManage/maintainServiceMemberForPopulation.action?mode=add&dailogName=_serviceTeamGuardersDialog&serviceTeamMemberVo.org.id='+getCurrentOrgId(),
			buttons: {			
	...
	//var personData = $("#"+$('#module').val()+"List").getRowData($("#objectId").val());	
		var listParam = null;
			listParam = {
				...
				'serviceMemberVo.memberName':$("#objectName").val()
			};
		loadData(listParam);
	}
	//加载服务成员数据
	function loadData(listParam) {
		$("#serviceTeamMemberList").setGridParam({
			url:'${path}/baseinfo/routerManage/findServiceMembersByServiceMemberVo.action',
			datatype: "json",
			page:1
		});
 ...
 function positionOrRelationFormatter(el,options,rowData){
	...
	var data = $("#serviceTeamMemberList").getRowData(id);
		$.ajax({
				async: false,
				url:"${path}/baseinfo/routerManage/leaveOrBackOnDuty.action",
			   	data:{
	...
	function updataGuarder(memberId){
		$("#_serviceTeamGuardersDialog").createDialog({ 
			...
			width: 600,
			height: 260,
			url:'${path}/baseinfo/routerManage/maintainServiceMemberForPopulation.action?mode=edit&dailogName=_serviceTeamGuardersDialog&serviceTeamGuarders.id='+memberId,
			buttons: {			
				"保存并关闭" : function(event){
					$("#isSubmit").val("true");
					$("#serviceTeamGuarderForm").submit();
	...
	function delMemberforObject(id){
		...
		$.ajax({
				async: false,
				url:"${path}/baseinfo/routerManage/deleteServiceMemberWithObject.action",
			   	data:{
			   		"selectedIds":id
		        },
				success:function(responseData){
	...
	function delGuarder(id){
		...
		okFunc: function(){
			$.ajax({
				async: false,
				url:"${path}/baseinfo/routerManage/deleteServiceGuardersWithObject.action",
			   	data:{
			   		"selectedIds":id
		        },
				success:function(responseData){
