...
<div id="dialog-form" title="新增" class="container container_24">
	<div id="tabs" style="height:400px;">
	   	<div id="positiveInfoDiv" class="container container_24">
	  		<form id="positiveInfoForm" method="post" action="${path}/baseinfo/goodSamaritanManage/addGoodSamaritan.action" >
	  		<pop:token />
			...
			<jsp:include page="/baseinfo/commonPopulation/populationInfo/goodSamaritan.jsp"></jsp:include>	
			</form>
	  	</div>
	</div>
</div>
			...
			$(form).ajaxSubmit({
				...
				success:function(data){
					...
					if("add"=="<s:property value='#parameters.mode[0]'/>"){
						if($("#isLock").val()!='1'){
	    				 	$("#goodSamaritanList").addRowData(data.id,data,"first");
		                	$("#goodSamaritanList").setSelection(data.id);
		                }
		                $("#positiveInfoList").trigger("reloadGrid");
	                	$.messageBox({message:"见义勇为新增成功！"});
	                }
	                if("edit"=="<s:property value='#parameters.mode[0]'/>"){
	                	 $("#positiveInfoList").setRowData(data.id,data);
	                	 if(data.death){
               				if($("#isLock").val()=='0') {
               					$("#goodSamaritanList").delRowData(data.id);
							}else {
								 $("#goodSamaritanList").setRowData(data.id,data);
								 $("#"+data.id+"").css('color','#778899');
							}
						}else{
							$("#goodSamaritanList").setRowData(data.id,data);
						}
						 $("#goodSamaritanList").trigger("reloadGrid");
