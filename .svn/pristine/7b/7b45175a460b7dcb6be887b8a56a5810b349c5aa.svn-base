...
<div class="content">
	<jsp:include page="${path}/account/peopleAspiration/trafficForViewButtons.jsp"/>
...
function initIssueCount(){
	$.ajax({
        url: "/threeRecords/traffic/getIssueCount.action",
        data: {
...
function viewIssue(rowid){
	...
	//if (hasRowSelected() && $("#view").isButtonEnabled()){
		...
		$("#issueDialog").createDialog({
			...
			height: 500,
			title: "查看交通信息详情",
			url: "${path}/threeRecords/traffic/dispatch.action?mode=viewNew&keyId="+selectedIssue.encryptIdByIssueId,
			buttons: {
		   		"关闭" : function(){
		        	$(this).dialog("close");
		   		}
