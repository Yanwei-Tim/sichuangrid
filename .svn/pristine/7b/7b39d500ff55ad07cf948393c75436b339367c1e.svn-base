...
<div >
	...
	<input type="hidden" id="domainId" name="helpPersonnel.personId" value="${personnelId}"/>
		<input type="hidden" id="keyType" name="helpPersonnel.personType" value="${personnelType}"/>
		...
		<div class="grid_14">
			<input type="text" id="helpPersonnelName" maxlength="15" name="helpPersonnel.name" value="${helpPersonnel.name}" class="form-txt" <s:if test='"view".equals(mode)'>disabled='true'</s:if>/>
		</div>
		<div class="grid_1"><s:if test='!"view".equals(mode)'><em class="form-req">*</em></s:if></div>

		<div class='clearLine'>&nbsp;</div>
		<div class="grid_9 lable-right">
			<label class="form-lbl">监护人身份：</label>
		</div>
		<div class="grid_14">
			<input type="text" maxlength="15" id="helpPersonnelIdentity" name="helpPersonnel.identity" value="${helpPersonnel.identity}" class="form-txt" <s:if test='"view".equals(mode)'>disabled='true'</s:if>/>
		</div>


		<div class='clearLine'>&nbsp;</div>
		<div class="grid_9 lable-right">
			<label class="form-lbl">监护人手机：</label>
		</div>
		<div class="grid_14">
...
<script type="text/javascript">
	...
	<s:if test='"add".equals(mode)'>
		$("#helpPersonnelForm").attr("action","${path}/baseinfo/helpPersonnel/handicapped/addHelpPersonnel.action");
	</s:if>
	<s:if test='"edit".equals(mode)'>
		$("#helpPersonnelForm").attr("action","${path}/baseinfo/helpPersonnel/handicapped/updateHelpPersonnel.action");
	</s:if>
