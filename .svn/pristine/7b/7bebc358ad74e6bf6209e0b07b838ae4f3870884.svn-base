package com.tianque.plugin.transfer.controller;
...
import com.tianque.plugin.transfer.service.TransferService;
...
public class TransferController extends BaseAction {
	...
	public String transfer() throws Exception {
		...
		resultList = transferService.validate(type, moveIds, toOrgId,
				getOrgId());
		bigType = TransferUtil.getBigType(type);
		return SUCCESS;
	}

	@Action(value = "isSameCity", results = {
			@Result(name = "success", type = "json", params = { "root", "true" }),
			@Result(name = "error", type = "json", params = { "root", "false" }) })
	public String isSameCity() throws Exception {
		if (orgId == null || toOrgId == null) {
			return ERROR;
		}
		String oldOrgCityDepNo = shardConversion.getShardCode(orgId);
		String toOrgCityDepNo = shardConversion.getShardCode(toOrgId);
		return oldOrgCityDepNo.equals(toOrgCityDepNo) ? SUCCESS : ERROR;
	}

	// @Action(value = "downLoad")
	// public String downLoad() {
	// try {
	// for (ErrorMessageVo vo : list) {
	// return vo.getMessage();
	// }
	// //inputStream =
	// ExcelExportHelper.exportDateToExcel(TransferUtil.downLoadData(), list);
	// downloadFileName = new String("错误信息清单".getBytes("gbk"), "ISO8859-1") +
	// ".xls";
	// } catch (Exception e) {
	// e.printStackTrace();
	// }
	...
	@EncryptAnnotation
	@Action(value = "transferDispatchByEncryptId", results = {
			@Result(name = "success", location = "/common/shiftTree.jsp"),
			@Result(name = "error", type = "json", params = { "root",
					"errorMessage" }) })
	public String transferDispatchByEncryptId() throws Exception {
		if (orgId == null) {
			errorMessage = "组织机构不能为空";
			return ERROR;
		}
