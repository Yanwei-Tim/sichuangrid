...
public class UnemployedPeopleServiceImpl extends BaseInfoPopulationTemplateImpl
	...
	//隐藏身份证中间4位
		private PageInfo<UnemployedPeople> hiddenIdCard(PageInfo<UnemployedPeople> pageInfo){
						//判断权限，有权限不隐藏
						if(permissionDubboService.
								isUserHasPermission(ThreadVariable.getUser().getId(), "isUnemployedPeopleManagementNotHidCard")){
							return pageInfo;
						}
						List<UnemployedPeople> list = pageInfo.getResult();
						int index=0;
						for (UnemployedPeople verification:list) {
