...
public class MentalPatientTaskServiceImpl implements MentalPatientTaskService {
	...
	public PageInfo getMentalPatientTaskList(MentalPatientTask mentalPatientTask, Integer page,
		...
		try {
			...
			PageInfo<MentalPatientTask>pageInfo=mentalPatientTaskDao.getMentalPatientTaskList(mentalPatientTask, page, rows,
					sidx, sord);
			//隐藏身份证中间4位
	...
	public MentalPatientTask getMentalPatientTaskById(Long id) {
		...
		try {
			...
			mentalPatientTask.setIdCard(IdCardUtil.hiddenIdCard(mentalPatientTask.getIdCard()));
			return mentalPatientTask;
		} catch (Exception e) {
	...
	public PageInfo searchMentalPatient(MentalPatientTask mentalPatientTask, Integer page,
		...
		try {
			fillOrganization(mentalPatientTask);
			mentalPatientTask = fillOrgInternalCode(mentalPatientTask);
			PageInfo<MentalPatientTask>pageInfo=mentalPatientTaskDao.searchMentalPatient(mentalPatientTask, page, rows, sidx,
					sord);
			//隐藏身份证中间4位
			pageInfo=hiddenIdCard(pageInfo);
			return pageInfo;
	...
	//隐藏身份证中间4位
	private PageInfo<MentalPatientTask> hiddenIdCard(PageInfo<MentalPatientTask> pageInfo){
		//判断权限，有权限不隐藏
		if(permissionDubboService.
				isUserHasPermission(ThreadVariable.getUser().getId(), "isMentalTaskNotHidCard")){
			return pageInfo;
		}
		List<MentalPatientTask>list=pageInfo.getResult();
		int index=0;
		for (MentalPatientTask mentalPatientTask:list) {
			mentalPatientTask.setIdCard(IdCardUtil.hiddenIdCard(mentalPatientTask.getIdCard()));
			list.set(index, mentalPatientTask);
			index++;
		}
