...
@Scope("prototype")
@Namespace("/judgmentAnalysis/floatingPopulation")
@Controller("floatingPopulationAnalysisController")
public class FloatingPopulationAnalysisController extends BaseAction {
	private static Logger logger = LoggerFactory
			.getLogger(FloatingPopulationAnalysisController.class);
	@Autowired
	...
	// 自定义显示省份
	private static final int cutNum = 10;
	private String proNameShow;
	private String proValueShow;
	...
	public String getArrayHistoryCycles() {
		try {
			getYearData();
			String modelKeyName = "flpop";
			String mode = DimensionCombinationMode.HIS.value;
			historyCyclesHb = historyCycleDubboService.countUnderHistoryCycle(
					orgId, modelKeyName, mode, year, month);
			// 环比
			...
			List<HistoryCycle> historyCyclesOldHb = historyCycleDubboService
					.countUnderHistoryCycle(orgId, modelKeyName, mode,
			...
			List<HistoryCycle> historyCyclesTb = historyCycleDubboService
					.countUnderHistoryCycle(orgId, modelKeyName, mode, year,
							month);
			...
			List<HistoryCycle> historyCyclesOldTb = historyCycleDubboService
					.countUnderHistoryCycle(orgId, modelKeyName, mode, yearOld,
							month);
	...
	private boolean checkHbData(ChartPageShowVo chartPageShowVo) {
		...
		if (amountHb == null || amountHb.length == 0
	...
	private boolean checkTbData(ChartPageShowVo chartPageShowVo) {
		...
		if (amountTb == null || amountTb.length == 0
				|| chartPageShowVo.getOrgName() == null
				|| chartPageShowVo.getOrgName().length == 0
				|| chartPageShowVo.getOrgId() == null
				|| chartPageShowVo.getOrgId().length == 0) {
	...
	private void setDataList() {
		...
		if (checkHbData(chartPageShowVo)) {
			...
			for (int i = 0; i < amountHb.length; i++) {
				if (chartPageShowVo.getRateHb() == null
					...
					dataObject += "{ 'AreaDate':'"
							...
							+ chartPageShowVo.getOrgId()[i] + "' },";
					amountOldTotalHb += Double.parseDouble(chartPageShowVo
							.getAmountOldHb()[i]);
				}
			...
			if (amountOldTotalHb > 0) {
				rateTotalHbTemp = (amountTotalHb - amountOldTotalHb) > 0 ? ((amountTotalHb - amountOldTotalHb) / amountOldTotalHb) * 100
						: 0.0d;
			}
		...
		if (checkTbData(chartPageShowVo)) {
			...
			if (amountOldTotalTb > 0) {
				rateTotalTbTemp = (amountTotalTb - amountOldTotalTb) > 0 ? ((amountTotalTb - amountOldTotalTb) / amountOldTotalTb) * 100
						: 0.0d;
			}
	...
	private void setPieData(List<DimensionCombinationCycle> combinationCycles,
		...
		String barChartName = "[";
		String pieData = "";
		if (ChartPageConstant.DIMENSIONNAME_PRO.equals(dimensionKeyName)) {
			// 省份只显示10个
			if (combinationCycles.size() > cutNum) {
				for (int i = 0; i < cutNum; i++) {
					String dimenName = combinationCycles.get(i)
							.getDimensionName1();
	...
	private String setProData(String barChartName, String proName,
			String barChartAmount) {
		String[] proNameStr = proName.split(",");
		String[] barChartAmountStr = barChartAmount.split(",");
		String proNameTemp = "";
		String barChartAmountTemp = "";
		String barChartAmountAfter = "";
		if (combinationCycles.size() > cutNum) {// 大于8个 需要自定义显示
			for (int i = 0; i < cutNum; i++) {
				if (i != cutNum - 1) {
					barChartAmountTemp += barChartAmountStr[i] + ",";
					barChartName += "'" + proNameStr[i] + "',";
					proNameTemp += proNameStr[i] + ",";
				} else {
					barChartAmountTemp += barChartAmountStr[i];
					barChartName += "'" + proNameStr[i] + "'";
					proNameTemp += proNameStr[i];
			...
			String orgNameAfter = "";
			for (int i = cutNum; i < proNameStr.length; i++) {
				if (i != proNameStr.length - 1) {
					barChartAmountAfter += barChartAmountStr[i] + ",";
					orgNameAfter += proNameStr[i] + ",";
				} else {
					barChartAmountAfter += barChartAmountStr[i];
					orgNameAfter += proNameStr[i];
				}
			...
			chartPageShowVo
					.setOrgNameAfterValue(barChartAmountAfter.split(","));
			chartPageShowVo.setOrgNameAfter(orgNameAfter.split(","));
		} else {// 不需要自定义显示
			for (int i = 0; i < proNameStr.length; i++) {
				if (i != proNameStr.length - 1) {
					barChartAmountTemp += barChartAmountStr[i] + ",";
					barChartName += "'" + proNameStr[i] + "',";
				} else {
		...
		String proNameShowStr = "";
		String[] proNameShowTemp = null;
		String[] proValueShowTemp = null;
		if (StringUtil.isStringAvaliable(proNameShow)) {
			proNameShowTemp = proNameShow.split(",");
			proValueShowTemp = proValueShow.split(",");
		}
		if (proValueShowTemp != null && proValueShowTemp.length > 0) {
			for (int i = 0; i < proValueShowTemp.length; i++) {
				if (i != proValueShowTemp.length - 1) {
					barChartAmountTemp += "," + proValueShowTemp[i];
					barChartName += ",'" + proNameShowTemp[i] + "'";
					proNameShowStr += proNameShowTemp[i] + ",";
				} else {
					barChartAmountTemp += "," + proValueShowTemp[i];
					barChartName += ",'" + proNameShowTemp[i] + "'";
					proNameShowStr += proNameShowTemp[i];
		...
		return barChartAmountTemp + "#" + barChartName;
	}
	 ...
	 * barChartName = "["; String proTemp = ""; String barChartAmountTemp = "";
	 * if (ChartPageConstant.DIMENSIONNAME_PRO.equals(dimensionKeyName)) {// 省份
	 * for (int i = 0; i < combinationCycles.size(); i++) { String dimenName =
	 * combinationCycles.get(i).getDimensionName1(); if (i !=
	 * combinationCycles.size() - 1) { barChartAmountTemp +=
	 * combinationCycles.get(i).getAmount() + ","; proTemp += dimenName + ","; }
	 * else { barChartAmountTemp += combinationCycles.get(i).getAmount();
	 * proTemp += dimenName; } } if (combinationCycles.size() != 0) { // 设置自定义省份
	 * String str = setProData(barChartName, proTemp, barChartAmountTemp);
	 * barChartAmount += str.split("#")[0]; barChartName = str.split("#")[1]; }
	 * } else if (ChartPageConstant.DIMENSIONNAME_CIT.equals(dimensionKeyName))
	 * {// 城市 if (combinationCycles.size() > cutNum) {// 大于10个，只取前10个 for (int i
	 * = 0; i < cutNum; i++) { String dimenName = combinationCycles.get(i)
	 * .getDimensionName1(); if (i != cutNum - 1) { barChartAmount +=
	 ...
	 * "'"; } } } else { for (int i = 0; i < combinationCycles.size(); i++) {
	 ...
	 * "'"; } } }
	 * 
	 * } else { for (int i = 0; i < combinationCycles.size(); i++) { String
	 * dimenName = combinationCycles.get(i).getDimensionName1(); if (i !=
	 * combinationCycles.size() - 1) { barChartAmount +=
	 * combinationCycles.get(i).getAmount() + ","; barChartName += "'" +
	 * dimenName + "',"; } else { barChartAmount +=
	 * combinationCycles.get(i).getAmount(); barChartName += "'" + dimenName +
	...
	private String selectMoudleName(String dimensionKeyName) {
		...
		if (ChartPageConstant.DIMENSIONNAME_SEX.equals(dimensionKeyName)) {
			...
			mName = "文化程度";
				...
				.equals(dimensionKeyName)) {
			mName = "婚姻状况";
		} else if (ChartPageConstant.DIMENSIONNAME_INFLOW
				.equals(dimensionKeyName)) {
			mName = "流入原因";
		} else if (ChartPageConstant.DIMENSIONNAME_PRO.equals(dimensionKeyName)) {
			mName = "按省份排名";
		} else if (ChartPageConstant.DIMENSIONNAME_CIT.equals(dimensionKeyName)) {
			mName = "按城市排名";
		}
		...
		try {
			...
			if (amounts != null && amounts.length != 0 && amountOlds != null
				...
				for (int i = 0; i < amounts.length; i++) {
					...
					if (amounOld > 0) {
						rateHbTemp = (amoun - amounOld) > 0 ? ((amoun - amounOld) / amounOld) * 100
								: 0.0f;
					}
		...
		try {
			...
			if (amounts != null && amounts.length != 0 && amountOlds != null
				...
				for (int i = 0; i < amounts.length; i++) {
					...
					if (amounOld > 0) {
						rateTbTemp = (amoun - amounOld) > 0 ? ((amoun - amounOld) / amounOld) * 100
								: 0.0f;
					}
					if (i != amounts.length - 1) {
	...
	public List<HistoryCycle> getHistoryCyclesHb() {
		return historyCyclesHb;
	}

	public void setHistoryCyclesHb(List<HistoryCycle> historyCyclesHb) {
		this.historyCyclesHb = historyCyclesHb;
	}
	...
	public String getProNameShow() {
		return proNameShow;
	}

	public void setProNameShow(String proNameShow) {
		this.proNameShow = proNameShow;
	}

	public String getProValueShow() {
		return proValueShow;
	}

	public void setProValueShow(String proValueShow) {
		this.proValueShow = proValueShow;
	}
