...
@Service("statisticsPopulationNewService")
public class StatisticsPopulationNewServiceImpl extends LogableService
		implements StatisticsPopulationNewService {
	...
	public void addpopulationStat() {
		...
		tableService.createAnalyseTable(AnalyseUtilNew.ACTUALPERSONTABLENAME,
				AnalyseUtilNew.ACTUALPERSONSQL, year, month);
		populationStatTypeNewDao.deletePopulationStatType(null, year, month,
				null);
		for (Entry<String, String> entry : BaseInfoTables.personnelTables
			...
			for (Long countryOrgId : countryList) {
				...
				if (!BaseInfoTables.personnelTables.get(
						...
						entry.getValue())) {
					populationStatTypeNewDao.addpopulationStatType(
							populationStat, entry.getValue(),
		...
		packetStatisticsService.createPacketStatisticsByTypeAndTime(
				PacketStatisticsTables.POPULATIONSTATTYPE_KEY, null, year,
				month);
	}
	...
	public void addpopulationStat(Integer year, Integer month, Long orgId,
		...
		// 累加统计没有按照org累加（现改为默认中国层级生成）
		Organization rootOrg = organizationDubboService.getRootOrganization();
		orgId = rootOrg.getId();
		String orgCode = rootOrg.getOrgInternalCode();
		// if (orgId != null) {
		...
		tableService.createAnalyseTable(AnalyseUtilNew.ACTUALPERSONTABLENAME,
				AnalyseUtilNew.ACTUALPERSONSQL, year, month);
		if (populationType == null || "".equals(populationType)) {
			populationStatTypeNewDao.deletePopulationStatType(orgCode, year,
					month, null);
			for (Entry<String, String> entry : BaseInfoTables.personnelTables
					.entrySet()) {
				PopulationStatType populationStat = new PopulationStatType();
			...
			packetStatisticsService
					.packetStatisticsByTableAndGroupType(PacketStatisticsHelper
							.createPacketStatisticsVo(year, month,
			...
			packetStatisticsService.createPacketStatisticsByTypeAndTime(
					AnalyseUtilNew.ACTUALPERSONTABLENAME, populationType, year,
					month);
		}
		...
		if (ShardTables.SHARD_HOUSEHOLDSTAFFS.equalsIgnoreCase(tableName)) {
			...
			if (shardConversion.isOverCity(organization)) {
				...
				for (Organization org : orgs) {
					...
					populationStatTypeNewDao.addpopulationStatType(
							populationType, tempTableName, createDate,
							nextMonthStart, orgCode);
				}
			} else {
				shardCode = shardConversion.getShardCode(orgId);
				tempTableName = tableName + "_" + shardCode;
				populationStatTypeNewDao.addpopulationStatType(populationType,
	...
	public List<PopulationAreaDataVo> getPopulationAreaDataByOrgIdAndMonth(
		...
		if (orgLevelDistance == null) {
			list = populationStatTypeNewDao.findPopulationStatTypeForOrg(orgId,
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (!orginternalId.equals(oldOrgId)) {
				...
				areaDate = createPopulationAreaDatas(orgId, orgCode, orgName);
	...
	public List<PopulationAreaDataVo> getPopulationAreaDataByOrgIdAndMonthAndPopulationType(
		...
		List<Map<String, Object>> list = populationStatTypeNewDao
	...
	private Map<String, Integer> getPopulationCountSumByOrgInternalCode(
		...
		for (Entry<String, String> entry : BaseInfoTables.personnelTables
				.entrySet()) {
			int sum = populationStatTypeNewDao.getCountByTableName(
	...
	public List<Object[]> getPopulationPieByOrgIdAndMonth(Long orgId,
		...
		Map<String, Integer> map = getPopulationCountSum(
				organizationDubboService.getSimpleOrgById(orgId).getId(), year,
				month);
		double total = map.get("total");
		for (Entry<String, String> entry : BaseInfoTables.personnelTables
				.entrySet()) {
	...
	private Map<String, Integer> getPopulationCountSum(Long orgId, int year,
			int month) {
		Map<String, Integer> map = new HashMap<String, Integer>();
		int total = 0;
		for (Entry<String, String> entry : BaseInfoTables.personnelTables
				.entrySet()) {
			int sum = populationStatTypeNewDao.findTendencyChart(
					entry.getKey(), orgId, year, month);
			map.put(entry.getKey(), sum);
			total = total + sum;
		}
		map.put("total", total);
	...
	public int getCountByOrgId(Long orgId, String tableType) {
		...
		List<Map<String, Object>> list = populationStatTypeNewDao
				.getCountByOrgInternalCodeAndTableNameAndMap(tableName, orgId,
						null, null, getOrgType());
		int countNum = countAlldata(list, "SUM");
		// return populationStatTypeNewDao.getCountByOrgId(tableName, orgId);
		return countNum;
	}
	...
	public List<Object[]> getPopulationPieInfo(int year, int month, Long orgId,
		...
		if (statisticSearchPopulationVo.getDisplayName() != null
			...
			tableService.createAnalyseTable(
					AnalyseUtilNew.ACTUALPERSONCATEGORYTABLENAME,
					AnalyseUtilNew.ACTUALPERSONCATEGORYSQL, year, month);
		}
		// 单个表类型分布图
		...
		statisticSearchPopulationVo
				.setType(AnalyseUtilNew.ACTUALPERSONCATEGORYTABLENAME);
	...
	private List<Object[]> getStatisticInfoFromHistory(
		...
		map.put("orgId", statisticSearchPopulationVo.getOrgId());
		map.put("domainName", statisticSearchPopulationVo.getDomainName());
		double total = Double
				.parseDouble(String.valueOf(populationStatTypeNewDao
		...
		for (PropertyDict propertyDict : propertyDictList) {
			...
			Long sum = populationStatTypeNewDao.countByMapFromHistorySta(map);
	...
	private Map<String, Integer> getHouseholdStaffPopulationCountSumMapByStatisticSearchPopulationVo(
		...
		if (propertyDicts != null && propertyDicts.size() > 0) {
			...
			for (PropertyDict propertyDict : propertyDicts) {
				int sum = populationStatTypeNewDao
						.getHouseholdStaffCountByTypes(
								statisticSearchPopulationVo
										.getOrginternalcode(), propertyDict
										.getId(), statisticSearchPopulationVo
										.getStartDate(),
	...
	public void addHouseholdStaffPopulationStat() {
		...
		packetStatisticsService.createPacketStatisticsByTypeAndTime(
				PacketStatisticsTables.HOUSEHOLDSTAFFSTAT_KEY, null, year,
				month);
	}
	...
	private void createHouseholdStaffHistoryStatistic(int year, int month,
		...
		// 删除原有记录
		populationStatTypeNewDao.deleteHouseholdStaffHistoryStatistic(
	...
	public void addpopulationStatForYearAndMonth(Integer year, Integer month) {
		...
		for (Entry<String, String> entry : BaseInfoTables.personnelTables
			...
			populationStat.setEndDate(CalendarUtil.getNextMonthStart(year,
					month));
			populationStatTypeNewDao.addpopulationStatType(populationStat,
	...
	public void addYouthPopulationStat(int year, int month) {
		...
		tableService.createAnalyseTable(AnalyseUtilNew.YOUTH_TABLE_NAME,
				AnalyseUtilNew.ACTUALPERSONSQL, year, month);
		populationStatTypeNewDao.deleteYouthStatType(year, month);
		...
		for (Map.Entry<String, String> youthsTypeEntry : YouthsType.youthsTypes
			...
			populationStatTypeNewDao.addYouthStatType(populationStat);
			// 未落户人口
			populationStat.getSearchYouthsVo().setTableName(
		...
		packetStatisticsService.createPacketStatisticsByTypeAndTime(
				PacketStatisticsTables.YOUTHSTATTYPE_KEY, null, year, month);
	}
