...
<form id="maintainShiftForm" method="post" action="${path}/baseinfo/lettingHouseManage/shiftLettingHouse.action">
	<input name="orgId" id="orgId" value="${lettingHouse.organization.id}" type="hidden">
	<input name="orgPangtId" id="orgPangtId" value="${lettingHouse.organization.parentOrg.id}" type="hidden">
	<input name="lettingHouse.organization.id" id="shiftOrgId" value="" type="text" style="width:0px;border:0px;height:0px;overflow:hidden;position:absolute;top:25px;left:70px;">
	<input name="lettingHouseIds" id="lettingHouseIds" value="${lettingHouseIds}" type="hidden">
</form >
...
$(document).ready(function(){
	...
	$("#maintainShiftForm").formValidate({
		...
		rules:{"lettingHouse.organization.id":{isGridOrganization:true,isModernOrg:true}},
		messages:{"lettingHouse.organization.id":{isGridOrganization:"数据只能转移得到网格",isModernOrg:"数据不能转移到当前网格"}}
	});
});
</script>