...
<div id="commonPopulation" class="container container_24">
	...
	<pop:token />
		...
		<input id="baseInfoId" name="population.baseInfoId" type="hidden" value="${population.baseInfoId}"/>
		...
		<div class="grid_4 lable-right">
   		...
   		<div class="grid_7">
   			<input type="text" <c:if test="${population.id!=null || dailogName=='unsettledPopulationMaintanceDialog'}">readonly</c:if> name="population.idCardNo" id="idCardNo" maxlength="18" value="${population.idCardNo}" style="width: 99%"
		...
		<div class="grid_7">
   		...
   		<div class="grid_7">
   			 <input type="text" name="population.workUnit" id="workUnit" maxlength="50" value="${population.workUnit }" 
   			   class="form-txt {exculdeParticalChar:true,messages:{exculdeParticalChar:'工作单位或就读学校不能输入特殊字符'}}" />
   		</div>
   		<div class="clearLine">&nbsp;</div>
   		<div class="grid_4 lable-right">
		...
		<div class="grid_7">
   		...
   		<div class="grid_20">
   			<input type="text" name="population.nativePlaceAddress" id="nativePlaceAddress" maxlength="50" value="${population.nativePlaceAddress }"  style="width: 99%"
   			 class="form-txt {exculdeParticalChar:true,messages:{exculdeParticalChar:'户籍地详址不能输入特殊字符'}}"
			/>
		</div>
		<div class='clearLine'>&nbsp;</div>
		...
		<div class="grid_4 lable-right">
	   		...
	   		<div class="grid_20">
	   			<input type="text" name="population.nativePoliceStation" id="nativePoliceStation" maxlength="50" value="${population.nativePoliceStation }" class="form-txt {exculdeParticalChar:true,messages:{exculdeParticalChar:'户籍派出所不能输入特殊字符'}}" style="width: 99%"
		...
		<div class='clearLine'>&nbsp;</div>
        	<s:if test='#request.fromClaim.equals("true")'></s:if><s:else>
				<jsp:include page="/baseinfo/commonPopulation/commonSearchAddress.jsp">
					<jsp:param value="householdStaff" name="name"/>
				</jsp:include>
			</s:else>
   		<div class='clearLine'>&nbsp;</div>
   		<div class="grid_4 lable-right">
   		...
   		<div class="grid_20">
   			<input type="text" name="population.otherAddress" id="populationOtherAddress"  maxlength="50" value="${population.otherAddress }" style="width: 99%"
   			 class="form-txt {exculdeParticalChar:true,messages:{exculdeParticalChar:'临时居所不能输入特殊字符'}}"
   			/>
   		</div>
   		<div class='clearLine'>&nbsp;</div>
   		<div class="grid_4 lable-right" >
...
$(document).ready(function(){
	...
	function isSettled (){
		...
		$.ajax({
			...
			success:function(responseData){
	    	   if(null!=responseData && null!=responseData.id){
	    		   ...
	    		   $.confirm({
	   					...
	   					okFunc: function(){
	   						...
	   						$("#baseInfoId").val(responseData.baseInfoId);
			   				...
			   				if(responseData.isHaveHouse == null){
								$("#populationHasHouseInfo").val('null');
			   				}
			   				if($("#houseId").val()==""||$("#houseId").val()==null){
	   							populationHasHouseInfoChanged();
			   				}
	   						$("#noHouseReason").val(responseData.noHouseReason == null ? '' : responseData.noHouseReason);
				...
				$("#_imgUrl").val($("#imgUrl").val());
	         	$(form).ajaxSubmit({
             		...
             		success: function(data){
	                     ...
	                     $("#<s:property value='#parameters.dailogName[0]'/>").proccessTypeSuccess(data.actualPopulationType,data);
		      	   },
...
$(window).resize(function(){
	setTimeout(function(){
		$(".ui-jqgrid-bdiv").height($(".ui-layout-center").height()-$(".thisCrumbs").height()-120)
	},500)
})
