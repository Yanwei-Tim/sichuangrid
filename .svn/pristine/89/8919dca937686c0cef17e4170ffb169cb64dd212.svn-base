...
public class CompanyPlaceImportValidatorImpl implements
	...
	private final int TELEPHONEMAX_LENGTH = 13;
	/** IP字段长度 */
	...
	public ValidateResult validate(CompanyPlace domain) {
		...
		if (!validateHelper.emptyString(domain.getTelePhone())) {
			...
			if (validateHelper.illegalStringLength(TELEPHONE_LENGTH_MIN,
					TELEPHONEMAX_LENGTH, domain.getTelePhone())) {
				result.addErrorMessage(getColumNo("telePhone") + "电话号码不能小于"
						+ TELEPHONE_LENGTH_MIN + "个字符,不能大于" + TELEPHONEMAX_LENGTH
						+ "个字符");
			}
		}
		// 传真号码验证
		...
		//寄递物流点的验证
		result=logisticsValidate(result,domain);
		return result;

	}

	//寄递物流点的验证
	private ValidateResult logisticsValidate(ValidateResult result,CompanyPlace domain){
		if(domain.getClassifiCation()==null||!domain.getClassifiCation().getDisplayName().equals(BaseInfoTables.NEW_LOGISTICS_DISPLAYNAME)){
			return result;
		}
		if(validateHelper.emptyString(domain.getUserName())){
			result.addErrorMessage(getColumNo("userName")
		...
		if(domain.getOperatingBrand()==null){
			result.addErrorMessage(getColumNo("operatingBrand")
					+ "经营品牌:不能为空");
		}
		if(domain.getOperatingMode()==null){
			result.addErrorMessage(getColumNo("operatingMode")
					+ "经营方式:不能为空");
		}
		return result;
	}
