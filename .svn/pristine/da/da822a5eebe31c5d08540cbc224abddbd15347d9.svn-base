...
public class UserAccountLoginDetailDaoImpl extends AbstractBaseDao implements UserAccountLoginDetailDao {
	...
	public PageInfo<UserAccountLoginDetail> findUserAccountLoginDetailForPageResult(
			Organization organization, Integer searchType,boolean isFunOrgType,List<Long> funOrgIdList, Integer page,
		...
		if(funOrgIdList!=null && funOrgIdList.size()!=0){
			map.put("funOrgIdList",funOrgIdList);
		}
		if(isFunOrgType){
			map.put("orgType", 2);
		}else{
