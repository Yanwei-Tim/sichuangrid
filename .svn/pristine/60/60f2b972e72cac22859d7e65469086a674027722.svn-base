...
<div id="dialog-form"  class="container container_24">
	...
	<div class="grid_4 lable-right">
   		<label class="form-lb1">缓存Key：</label>
  	</div>
  	<div class="grid_6">
  		<input name="cacheKey" class="form-txt" type="text" id="cacheKey" >
   	</div>
   	<div class="grid_4">
...
<script type="text/javascript">
	...
	if($("#cacheKey").val()=="" || $("#cacheKey").val()==null ){
		$.messageBox({ level:"warn",message:"缓存Key不能为空"});
		return;
	}
		$.dialogLoading("open");
		$.ajax({
			url:PATH+'/sysadmin/jobMonitor/cleanCacheByKey.action',
			type:'post',
			data:{
