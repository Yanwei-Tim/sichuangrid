...
<form id="maintainShiftForm" method="post" action="${path}/baseinfo/constructionUnitManage/shiftConstructionUnit.action">
	...
	<input name="constructionUnit.organization.id" id="shiftOrgId" value="" type="text" style="width:0px;border:0px;height:0px;overflow:hidden;position:absolute;top:25px;left:70px;">
	<input name="constructionUnitIds" id="constructionUnitIds" value="${constructionUnitIds}" type="hidden">
</form >
...
$(document).ready(function(){
	...
	$("#maintainShiftForm").formValidate({
		...
		rules:{"constructionUnit.organization.id":{isGridOrganization:true,isModernOrg:true}},
		messages:{"constructionUnit.organization.id":{isGridOrganization:"数据只能转移得到网格",isModernOrg:"数据不能转移到当前网格"}}
	});
});
</script>