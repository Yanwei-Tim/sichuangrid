...
<div class="ui-corner-all nav-buttons" id="nav" style="float:left">
	...
	<pop:JugePermissionTag ename="gridServiceTeamEventSearchIssue">
...
<div class="ui-corner-all nav-buttons">
	<select onchange="onStatusChanged(this.value)" id="grid_doneTypeSelect" class="form-txt" style="display: none;width: 85px; float: right;">
...
var issueButtons={
		...
		showButtons:function(showBtns){
			...
			if("done" == $("#jurisdictionsViewType").val() || "completed" == $("#jurisdictionsViewType").val()){
				$("#grid_doneTypeSelect").show();
			}else{
				$("#grid_doneTypeSelect").hide();
			}
			$("#seachValue").show();
			if(isGrid()){
...
            searchIssueVo[data[i].name] = data[i].value;
	...
	var pageData = $.extend(
			...
			"searchIssueVo.fourTeamsType":"<s:property value='@com.tianque.domain.property.FourTeamsType@GRIDSERVICETEAM'/>",
