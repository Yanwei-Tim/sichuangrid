...
<div class="ui-corner-all nav-buttons"  id="nav" style="float:left">
	...
	<pop:JugePermissionTag ename="communityVolunteerServiceTeamEventSearchIssue">
...
<div class="ui-corner-all nav-buttons">
	<select onchange="onStatusChanged(this.value)" id="commun_doneTypeSelect" class="form-txt" style="display: none;width: 85px; float: right;">
...
var issueButtons={
		...
		showButtons:function(showBtns){
			...
			if("done" == $("#jurisdictionsViewType").val() || "completed" == $("#jurisdictionsViewType").val()){
				$("#commun_doneTypeSelect").show();
			}else{
				$("#commun_doneTypeSelect").hide();
			}
			$("#seachValue").show();
			if(isGrid()){
...
            searchIssueVo[data[i].name] = data[i].value;
	...
	var pageData = $.extend(
			...
			"searchIssueVo.fourTeamsType":"<s:property value='@com.tianque.domain.property.FourTeamsType@COMMUNITYVOLUNTEERSERVICETEAM'/>",
