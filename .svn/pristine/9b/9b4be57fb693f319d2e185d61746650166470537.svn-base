	...
	<form id="maintainForm" method="post" action="">
	 <input type="hidden" id="isCloseWindow" value="true"/>
      <pop:token></pop:token>
		...
		<input type="hidden" name="preciseInbox.toUserName" value="${preciseInbox.toUserName}"><!-- 用于获取微信号 -->
...
$(document).ready(function(){
	...
	$('#custom_image_upload').uploadFile({
		...
		queueSizeLimit :1,
		multi:false,
		removeAction:"${path}/weChat/preciseInbox/delelteAttachFile.action",
		fileExt        : "*.jpg",
		fileDesc       : '图片',
		chinese        :false,
		sizeLimit:1048576
	 ...
	 <s:if test='"replyNewsMessage".equals(mode)'>
	   $("#maintainForm").attr("action","/weChat/preciseInbox/replyNewsMessage.action");
	</s:if>
	<s:else>
