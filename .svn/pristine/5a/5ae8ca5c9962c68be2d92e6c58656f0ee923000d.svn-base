...
<@pop.formatterProperty name="position" domainName="@com.tianque.plugin.serviceTeam.util.Constants$ServiceTeamPositionPropertyTypes@TEAMPOSITION" />
	...
	$("#addServiceTeamMember").click(function(event){
		$("#_serviceTeamMemberDialog").createDialog({
			...
			width: 700,
			height: 480,
			url:'${path}/plugin/serviceTeam/router/routerManage/maintainServiceMemberForHouse.action?dailogName=_serviceTeamMemberDialog&id=${(id)! }&populationType=${(populationType)! }&name=${(name)!}&mode=view&serviceTeamMemberVo.org.id='+getCurrentOrgId(),
			buttons: {			
				"关闭" : function(){
		        	$(this).dialog("close");
		...
		$.ajax({
				...
				success:function(responseData){
					if(responseData>0){
						...
						$.messageBox({level:'error',message:"无法定位要卸任的治安负责人！"});
					}
				}
			});
	}
