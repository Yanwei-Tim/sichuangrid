...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#helpPersonnelList").jqGridFunction({
			...
			colModel:[
				...
				{name:"name", sortable:false, label: '监护人姓名'},
				{name:"identity", sortable:false, label: '监护人身份'},
			...
			ondblClickRow: viewhandicappedHelp,
		...
		$("#addHelp").click(function(){
			...
			$("#helpPersonnelDialog").createDialog({
				...
				height: 230,
				title:'新增监护人员信息',
				url:'${path}/baseinfo/helpPersonnel/dispatch.action?personnelId='+domainId+'&personnelType=<s:property value="#parameters.keyType"/>&mode=add&personType=handicappedHelp',
				buttons: {
					"保存" : function(event){
		...
		$("#updateHelp").click(function(){
			...
			$("#helpPersonnelDialog").createDialog({
				...
				height: 250,
				title:'修改监护人信息',
				url:'${path}/baseinfo/helpPersonnel/dispatch.action?helpPersonnel.id='+id[0]+'&mode=edit&personType=handicappedHelp',
				buttons: {
					"保存" : function(event){
				      $("#helpPersonnelForm").submit();
				   },
	...
	function viewhandicappedHelp(){
		...
		$("#helpPersonnelDialog").createDialog({
			...
			height: 250,
			title:'查看监护人信息',
			url:'${path}/baseinfo/helpPersonnel/dispatch.action?helpPersonnel.id='+id[0]+'&mode=view&personType=handicappedHelp',
			buttons: {
			   "关闭" : function(){
			        $(this).dialog("close");
			   }
