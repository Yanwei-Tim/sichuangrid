...
function getmonth(){
	$.ajax({
		async: false,
		url: "${path }/stat/currentTimeNew/getCurrentTimeForMonthToSpecial.action?currenYear="+$("#pyear").val(),
...
function onOrgChanged(){
	...
	$("#importantPlacePie").pieChart({
		url:"${path}/baseInfoStat/statisticsPopulationNew/getStatisticsPopulationPie.action?orgId="+getCurrentOrgId()+"&year="+$("#pyear").val()+"&month="+$("#pmonth").val()+"&populationType="+populationType,
