			...
			<div class="grid_4 lable-right">
	  		...
	  		<div class="grid_7 form-left">
	  			<select name="workDiary.diaryType.id" id="diaryType" class="dialogtip form-txt" title=""
	   				<s:if test='"add".equals(mode)'></s:if>
	   				<s:else>disabled='true'</s:else>
	   				>
	   				<s:iterator value="workDiaryTypes">
	   					...
	   					<s:else>
	   						<option  value="<s:property value="id"/>" <s:if test='workDiary.diaryType.id == id'>selected="selected"</s:if> > <s:property value="displayName"/></option>
	   					</s:else>
	   				</s:iterator>
				</select>
	  		</div>
...
$(document).ready(function(){
	...
	<s:if test='"edit".equals(mode)'>
		$("#maintainForm").attr("action","${path}/dailyLog/workDiaryManage/editWorkDiary.action");
	</s:if>
             ...
             success: function(data){
                     ...
                     data["organization.orgName"]=data.organization.orgName;
        	   		 <s:if test='"add".equals(mode) || "copy".equals(mode) '>
		   		 		$("#workDiaryList").addRowData(data.id,data,"first");
					    $.messageBox({message:"成功保存新工作日志信息!"});
						$("#workDiaryDialog").dialog("close");
						$("#workDiaryList").trigger("reloadGrid");
				     </s:if>
				     <s:if test='"edit".equals(mode)'>
				        $("#workDiaryList").setRowData(data.id,data);
					    $.messageBox({message:"成功保存工作日志修改信息!"});
					    $("#workDiaryDialog").dialog("close");
					    $("#workDiaryList").trigger("reloadGrid");
				     </s:if>
      	   },
