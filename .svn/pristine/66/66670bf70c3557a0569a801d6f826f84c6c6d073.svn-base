		...
		<div id="enterpriseInfo" class="container container_24">
			...
			<div class="grid_7">
				<select name="enterprise.type.id" class="form-txt dialogtip" id="enterprise-type-id"
					<s:if test='"view".equals(mode)'>disabled='true'</s:if>>
					<pop:OptionTag
						name="@com.tianque.domain.property.PropertyTypes@FIRESAFETY_TYPE"
...
var keyTypeName='';
	...
	$(document).ready(function(){
		...
		initTabText();
		<s:if test='"add".equals(mode)'>
			$("#maintainForm").attr("action", "${path}/baseinfo/fireSafetyEnterpriseManage/addFireSafetyEnterprise.action");
		</s:if>
		<s:if test='"edit".equals(mode)'>
			$("#maintainForm").attr("action", "${path}/baseinfo/fireSafetyEnterpriseManage/updateFireSafetyEnterprise.action");
		</s:if>
		...
		jQuery.validator.addMethod("exsistedCompanyName", function(value, element){
			...
			$.ajax({
				async:false,
				url:'/baseinfo/fireSafetyEnterpriseManage/hasDuplicateFireSafetyEnterpriseNameAndKeyType.action',
			   	data:{
