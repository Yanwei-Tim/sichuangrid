...
var title = "出租房";	
		jQuery(document).ready(function() {		
			...
			$("#searchHighA").click(function(){
				$("#statisticsDialog").createDialog({
					...
					modal:true,
					url: '${path}/baseinfo/siteinfo/lettingHouse/searchLettingHouseDlg.jsp',
					buttons: {
						"查询" : function(event){
			...
			$("#searchByIdcardA").click(function(){
				if($("#searchByLandlordName").val()=="请输入房东姓名"){
					$.notice({
						level: 'alert',
					    okbtnName: "确定",
					    title:'提示',
			...
			$("#refreshOrgTree1").click(function(){
				$("#searchByLandlordName").attr("value","");
				$("#searchByLandlordName").attr("value","请输入房东姓名");
			});
		});
		function getDialogParam(){
			return {
				...
				title: title+'信息',
				modal:true,
				url: '${path}/baseinfo/siteinfo/lettingHouse/lettingHouseListForStatistics.jsp?isSearch=1',
				buttons: {
			   		"关闭" : function(){
			        	$(this).dialog("close");
			   		}
...
<div class="btnbanner">
	<div class="ui-widget autosearch">
		<input type="text" value="请输入房东姓名" class="searchtxt" id="searchByLandlordName" maxlength="18" onfocus="if(this.value == '请输入房东姓名') this.value = '';" onblur="if(this.value == '') this.value = '请输入房东姓名';">
