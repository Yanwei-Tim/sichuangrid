...
<div id="positiveinfoPie" class="SigmaReport" style="height:270px;width:100%;"></div>

<script type="text/javascript">
if(<%=AnalyseUtilNew.IS_NEWANALYSE_JOB%>){
	var data_table_url = "${path}/baseInfo/statisticManageNew/getStatisticPie.action?orgId=";
	...
	var data_table_url = "${path}/baseInfo/statisticManage/getStatisticPie.action?orgId=";
...
function onOrgChanged(){
	$("#positiveinfoPie").pieChart({
		url:data_table_url+<%=orgId%>+"&year="+$("#queryYear").val()+"&month="+$("#queryMonth").val()+"&type=${type}",
		moduleName:"特殊人群",
...
function getmonth(){
	$.ajax({
		async: false,
		url: data_month_url+$("#queryYear").val(),
		success:function(responseData){
			for(var i = 0;i<responseData.length;i++){
