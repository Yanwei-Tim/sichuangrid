...
<div class="container container_24">
	...
	<@pop.token />
 		<input name="serviceTeamMemberVo.org.id" type="hidden" id="orgInternalCodeVo" value="${(serviceTeamMemberVo.org.id)!}"/>
 		<input name="serviceTeamGuarders.serviceGuardersWithObject.objectType" type="hidden" id="GuardersObjectType" value=""/>
 		<input name="serviceTeamGuarders.serviceGuardersWithObject.objectName" type="hidden" id="GuardersObjectName" value=""/>
 		<input name="serviceTeamGuarders.serviceGuardersWithObject.objectId" type="hidden" id="GuardersObjectId" value=""/>
 		<input name="serviceTeamGuarders.serviceGuardersWithObject.teamMember" type="hidden" id="GuardersTeamMember" value="0"/>
	...
	<div class='clearLine'>&nbsp;</div>
		...
		<div class="grid_8">
			<input type="text" name="serviceTeamGuarders.idCardNo" id="guarderIdCardNo" value="" maxlength="18"	class="form-txt {idCard:true,messages:{idCard:'请输入一个合法的身份证号码'}}" style="width:96%"/>
		</div>
		<div class='clearLine'>&nbsp;</div>
		<div class="grid_4 lable-right">
...
function setPersonData(){
	var personData = $("#"+$('#moduleForMember').val()+"List").getRowData($("#objectId").val());
	var personName = $(personData.name).text();
	var personType = $("#_populationType_").val();
	$("#GuardersObjectId").val(personData.id);
	$("#GuardersObjectType").val(personType);
	$("#GuardersObjectName").val(personName);
}
setPersonData();
//表单验证
