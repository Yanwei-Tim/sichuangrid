...
<input name="serviceTeamMemberVo.memberName" type="hidden" id="objectName" value="${(name)!}"/>
<input name="serviceTeamMemberVo.objectType" type="hidden" id="MemberObject_Type" value="<#if populationType??>${(populationType)!}<#else>${(module)!}</#if>"/>
<div class="content">
	<div style="width:100%;" >
...
<@pop.formatterProperty name="position" domainName="@com.tianque.plugin.serviceTeam.util.Constants$ServiceTeamPositionPropertyTypes@TEAMPOSITION" />
	...
	function loadData(listParam) {
		$("#serviceTeamMemberList").setGridParam({
			url:'${path}/plugin/serviceTeam/router/routerManage/findServiceMembersByServiceMemberVo.action?serviceMemberVo.onDuty=1',
			datatype: "json",
			page:1
		});
