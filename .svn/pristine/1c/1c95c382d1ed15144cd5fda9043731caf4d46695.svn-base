...
import com.tianque.plugin.analysisNew.dao.UserActivateReportNewDAO;
import com.tianque.plugin.analysisNew.domain.UserActivateReport;
 ...
 */
@Service("userActivateReportNewService")
@Transactional
public class UserActivateReportNewServiceImpl implements
		UserActivateReportNewService {
	...
	@Autowired
	private UserActivateReportNewDAO userActivateReportNewDAO;
	@Autowired
	...
	private List<UserActivateReport> getUserActivateReportListByMap(int year,
		...
		list = calculationPercentage(userActivateReportNewDAO
	...
	public List<UserActivateReport> getUserActivateReportList(int year,
		...
		tableManageService.createAnalyseTable(
				AnalyseUtilNew.USER_ACTIVATE_REPORT_TABLE_NAME,
				AnalyseUtilNew.USER_ACTIVATE_REPORT_TABLE_NAME_SQL,
	...
	private void createUserActivateReportDataByYearAndMonth(int year, int month) {
		...
		if (!isCreat) {
			...
			map.put("year", year);
			map.put("month", month);
			userActivateReportNewDAO.deleteAllDataByYearAndMonth(map);
		}
		// 上月月末 用于统计事件
		Date lastMonthDate = CalendarUtil.getLastMonthStart(year, month);
		// 本月月初
		Date startDate = CalendarUtil.getBeforeMonthFirstDayByTime(year, month);
		// 下一个月月初
	...
	private void querySimpleUserActivateReportForList(Map<String, Object> map,
		...
		if (statistics == 0) {
			key = MemCacheConstant
					.getUserActivateReportCachKey(
							MemCacheConstant.ANALYSE_USERACTIVATEREPORT_NAMESPACE,
							year, month, CITY_VAL,
			...
			key = MemCacheConstant.getUserActivateReportCachKey(
					MemCacheConstant.ANALYSE_USERACTIVATEREPORT_NAMESPACE,
					year, month, DISTRICT_VAL,
					MemCacheConstant.STATISTICSTABLELIST);
		}
		List<UserActivateReport> list = userActivateReportNewDAO
		...
		if (list != null) {
			for (UserActivateReport userActivateReport : list) {
				...
				userActivateReport.setCommunityActivateCount(userActivateReport
						...
						- userActivateReport.getVillageActivateCount());
				userActivateReportNewDAO
						.addUserActivateReport(userActivateReport);
			}
