...
@Component("threeValidateHelper")
public class ThreeValidateHelper {
	...
	public String[] cellValueTrim(String[] cellValues) {
		for (int i = 0; i < cellValues.length; i++) {
			cellValues[i] = cellValues[i] == null ? "" : cellValues[i].trim();
	...
	public boolean illegalPropertyDictDisplayNameById(Long propertyDictId,
			String dictDisplayName) {
		PropertyDict propertyDict = propertyService
	...
	public boolean existsSamePropertyTypes(List<PropertyDict> dicts,
		...
		for (String propertyType : propertyTypes) {
			List<PropertyDict> list = propertyService
		...
		if (vlaue != -1) {
			...
			if (results != null && results.size() == 1) {
				results = orgService.findOrganizationsByOrgNameAndParentId(
						null, gridOrgName, results.get(0).getId());
			}
		}
	...
	public boolean isUbordinateRelationship(Long downOrgId, Long upOrgId) {
		Organization downOrg = orgService.getSimpleOrgById(downOrgId);
		Organization upOrg = orgService.getSimpleOrgById(upOrgId);
		if (upOrg != null && upOrg.getOrgType() != null) {
			upOrg.setOrgType(propertyService.getPropertyDictById(upOrg
					.getOrgType().getId()));
		}
		...
		if (upOrg != null
			...
			upOrg = orgService.getParentOrgByOrgTypeAndChildOrgId(
