...
<div id="nav" class="ui-corner-all">
		...
		<select style="float:left;" name="searchType" id="searchType">
			<option value="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_MONTH'/>">按月份</option>
			<option value="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_QUARTER'/>">按季度</option>
			<option value="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_YEAR'/>">按年度</option>
		</select>
		<div id="yearDiv">
...
$(document).ready(function() {
	...
	$("#searchType").change(function(){
		if($("#issueTendecyType").val()!="issueCount"&&$("#searchType").val()=="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_MONTH'/>"){
			$("#yearDiv").show();
			$("#quarterDiv").hide();
			$("#monthDiv").show();
		}else if($("#issueTendecyType").val()!="issueCount"&&$("#searchType").val()=="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_QUARTER'/>"){
			$("#yearDiv").show();
			$("#quarterDiv").show();
			$("#monthDiv").hide();
		}else if($("#issueTendecyType").val()!="issueCount"&&$("#searchType").val()=="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_YEAR'/>"){
			$("#yearDiv").show();
			$("#quarterDiv").hide();
			$("#monthDiv").hide();
		}
...
function onOrgChanged(){
	if($("#issueTendecyType").val()=="comparedSame" || $("#issueTendecyType").val()=="sequential"){
		if($("#searchType").val()=="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_MONTH'/>"){
			if($("#cyear").val()==$("#cyear option:first").val()&&$("#cmonth").val()==$("#cmonth option:first").val()){
				$.messageBox({level:'warn',message:"本月数据还未生成,您可以查看其它月份"});
				return;
			}
		}
		if($("#searchType").val()=="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_QUARTER'/>"){
		...
		if($("#searchType").val()=="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_YEAR'/>"){
	...
	var issueLineUrl;
	if($("#issueTendecyType").val()=="issueCount"){	
		issueLineUrl="${path}/issueAnalysisChartManageNew/getIssueTendencyChart.action?orgId="+getCurrentOrgId()+"&searchType="+$("#searchType").val();
	}
	if($("#issueTendecyType").val()=="comparedSame"){			
		issueLineUrl="${path}/issueAnalysisChartManageNew/getIssueComparedSameChart.action?orgId="+getCurrentOrgId()+"&year="+$("#cyear").val()+"&month="+$("#cmonth").val()+"&searchType="+$("#searchType").val()+"&quarter="+$("#quarter").val();
	}
	if($("#issueTendecyType").val()=="sequential"){
		issueLineUrl="${path}/issueAnalysisChartManageNew/getIssueSequentialChart.action?orgId="+getCurrentOrgId()+"&year="+$("#cyear").val()+"&month="+$("#cmonth").val()+"&searchType="+$("#searchType").val()+"&quarter="+$("#quarter").val();
	}
	$("#issueLine").lineChart({
		url: issueLineUrl,
		moduleName:"",
...
function getQuarter(){
	$.ajax({
		async: false,
		url: "${path }/stat/currentTimeNew/getCurrentTimeForQuarter.action?currenYear="+$("#cyear").val(),
		success:function(responseData){
			for(var i = 0;i<responseData.length;i++){
