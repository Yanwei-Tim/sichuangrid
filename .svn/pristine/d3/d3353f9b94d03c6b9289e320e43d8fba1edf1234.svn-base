...
public class WorkCalendarDaoImpl extends AbstractBaseDao implements
	...
	/**
	 * @see com.tianque.dao.WorkCalendarDao#addDefaultCalendar(com.tianque.domain.WorkCalendar)
	 */
	@Override
	public boolean addDefaultCalendar(WorkCalendar workCalendar) {
		List<List<WorkCalendar>> worksList = new ArrayList<List<WorkCalendar>>();
		// 获取默认日历，若缓存中木有去数据库查
		if (cacheService.get("WorkCalendarList_" + workCalendar.getYear()) != null) {
			worksList = (List<List<WorkCalendar>>) cacheService
					.get("WorkCalendarList_" + workCalendar.getYear());
		} else {
			List<WorkCalendar> workList = new ArrayList<WorkCalendar>();
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("year", workCalendar.getYear());
			map.put("calendarType", 0);
			List<WorkCalendar> list = getSqlMapClientTemplate().queryForList(
					"workCalendar.findWorkClaendars", map);
			if (list == null || list.isEmpty()) {
				worksList.add(workList);
		...
		// 将默认日历复制给地区特色日历
		for (int k = 0; k < worksList.size(); k++) {
			{
				List<WorkCalendar> workCalendars = worksList.get(k);
				for (int m = 0; m < workCalendars.size(); m++) {
					WorkCalendar calendar = workCalendars.get(m);
					calendar.setEndWorkTimeAM(workCalendar.getEndWorkTimeAM());
					calendar.setEndWorkTimePM(workCalendar.getEndWorkTimePM());
					calendar.setStartWorkTimeAM(workCalendar
							.getStartWorkTimeAM());
					calendar.setStartWorkTimePM(workCalendar
							.getStartWorkTimePM());
					calendar.setCalendarType(workCalendar.getCalendarType());
					calendar.setOrgId(workCalendar.getOrgId());
					getSqlMapClientTemplate().insert(
							"workCalendar.addWorkClaendars", calendar);
				}
			}
