...
<table width="200" class="tablelist">
	 <tr>
	    ...
	    <s:if test='null!=enterprise.imgUrl && !"".equals(enterprise.imgUrl)'>
			<img id="entermg" name="enterprise.imgUrl" src="${path }${enterprise.imgUrl}" width="148px"/>
		</s:if>
		<s:else>
	...
	<tr>
		...
		<td class="content"><span><pop:PropertyDictViewTag
			...
			defaultValue="${enterprise.type.id}"/></span>
		</td>
...
$(function (){
	if(null!=$("#_imgUrl").val() && $("#_imgUrl").val()!=""){//添加随机数保证IE下的图片查看没问题
		$("#entermg").attr("src",$("#_imgUrl").val()+"?r="+Math.random());
	}
});
</script>