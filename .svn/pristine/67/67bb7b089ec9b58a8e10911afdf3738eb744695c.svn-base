...
<pop:JugePermissionTag ename="enterpriseDownManagement">
	<span id="enterpriseDownKey"><s:property value="#request.name" /></span>
</pop:JugePermissionTag>
<pop:JugePermissionTag ename="protectionKeyManagement">
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#enterpriseList").jqGridFunction({
			...
			colModel:[
				...
				{name:"name", index:"name", label: '企业名称',formatter:nameFormatter,sortable:false,width:180},
				{name:"type", index:"type", label:'企业类型',sortable:false,formatter:typeFormatter,width:90,align:"center"},
				{name:"businessLicense",index:"businessLicense", sortable:false, label: '工商执照号码',width:100},
				{name:'organization.orgName',index:'orgInternalCode',label:'所属网格',sortable:false,width:150, hidden:true},
				{name:"legalPerson", index:"legalPerson", label:'法人代表',width:100},
				{name:"industry", index:"industry", label:'所属行业', formatter:industryFormatter,width:100},
				{name:"riskEnterprise", sortable:false, label:'是否危化企业', formatter:isRiskEnterprise,width:100,align:"center"},
				...
				{name:"hiddenCases", sortable:false, label:'隐患情况',hidden:true,width:100},
				...
				{name:"telephone",sortable:false,label:"法人联系电话",hidden:true,width:100},
				{name:"mobileNumber",sortable:false,label:"法人手机号码",hidden:true,width:100},
				{name:"address",sortable:false,label:"企业地址",hidden:true,width:200},
	...
	function isEmphasisFormatter(){
		...
		idCollection=$("#enterpriseList").getDataIDs();
			for(var i=0;i<idCollection.length;i++){
				...
				if(ent.isEmphasis==1 || ent.isEmphasis == "true"){
	...
	function searchEnterprise(){
		...
		var postData = $("#searchEnterpriseForm").serializeObject();
		//console.log(postData);
		//fateson add   ..."orgId":getOrgIdForStat(),修复高级收索 不到的bug
		$.extend(postData,{"ownerOrg.id":getCurrentOrgId(),"orgId":getOrgIdForStat(),"enterpriseSearchCondition.keyType":$("#keyType").val()});
		/* var searchCondition={
		...
		/** 是危险企业（默认为否） */
		if($("#conditionIsRiskEnterprise").val() != ""){
			$.extend(postData,{"enterpriseSearchCondition.isRiskEnterprise":1});
		}
		$("#enterpriseList").setPostData(postData);
		//console.log(postData);
		$("#enterpriseList").trigger("reloadGrid");
		$("#safetyProductionKeyDialog").dialog("close");
	}
	...
	function viewEnterpriseInfo(selectedId){
		...
		$("#enterpriseDialog").createDialog({
			...
			title: '查看'+title+'信息',
			modal: true,
			url: '${path}/baseinfo/safetyProductionKeyManage/dispatchOperateByEncrypt.action?mode=view&locationType='+enterpriseTemplates+'&keyType='+keyType+'&location.id='+id,
			buttons: {
			   "关闭" : function(){
			        $(this).dialog("close");
			   }
