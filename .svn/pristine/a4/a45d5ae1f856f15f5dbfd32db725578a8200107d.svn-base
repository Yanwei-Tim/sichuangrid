...
function featureClickMapEnd(feature){
	...
	if($currentPos.length==1){
		if($currentPos.find("label").attr("name")=='redCuffTeamMember'){
			// 在红袖套下面进行画图，该功能为选中区域内的红袖套成员发送微信或短信
			var markersArray = getMarkersInPolygonPoints(polygonPoints);
			chooseRedCuffByDraw(markersArray,lon,lat);//detailsListCommonTwoDimensionMap.jsp
			return ;
		}
	}
