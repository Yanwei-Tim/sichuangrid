...
import com.tianque.plugin.analysisNew.dao.BaseInfoStatTypeNewDao;
...
import com.tianque.plugin.analysisNew.domain.PersonnelDetailDataVo;
...
@Service("statisticsPositiveinfoNewService")
public class StatisticsPersonnelNewServiceImpl implements
		StatisticsPersonnelNewService {
	...
	@Autowired
	private BaseInfoStatTypeNewDao baseInfoStatTypeNewDao;
	...
	private int getImportantPersonelCountByOrgInternalCode(
		...
		return statisticsBaseInfoNewDao
	...
	public List<Object[]> getImportantPersonlPieByOrgIdAndMonth(Long orgId,
		...
		List<Object[]> importantPersonlPieDatas = new ArrayList<Object[]>();
		List<Map<String, Object>> maps = baseInfoStatTypeNewDao
				.findBaseInfoStatTypeForOrg(orgId, typeName, year, month);
		double sum = 0L;
	...
	private PersonnelAreaDataVo getImportantPersonelAreaDataVoByOrgIdAndMonth(
		...
		PersonnelAreaDataVo personnelAreaDataVo = new PersonnelAreaDataVo();
		List<Map<String, Object>> maps = baseInfoStatTypeNewDao
				.findBaseInfoStatTypeForOrg(organization.getId(), typeName,
						year, month);
		...
		personnelAreaDataVo
				.setPersonnelDetailDatas(getImportantPersonelDetailDatasByOrgInternalCodeAndMonth(
						maps, typeName, year, month, organization));
		personnelAreaDataVo.setAmount(getBaseinfoStatSum(organization,
				typeName, year, month));
		return personnelAreaDataVo;
	}

	private int getBaseinfoStatSum(Organization organization, String typeName,
			int year, int month) {
		List<Map<String, Object>> maps = baseInfoStatTypeNewDao
				.findBaseInfoStatTypeForOrg(organization.getId(), typeName,
						year, month);
	...
	private List<PersonnelDetailDataVo> getImportantPersonelDetailDatasByOrgInternalCodeAndMonth(
			...
			int month, Organization organization) {
		int helpSum = 0;
		int nohelpSum = 0;
		int resitedSum = 0;
		int recidivismSum = 0;
		int noresitedSum = 0;
		List<PersonnelDetailDataVo> personnelDetailDataVos = new ArrayList<PersonnelDetailDataVo>();
		int sum = getBaseinfoStatSum(organization, typeName, year, month);
