...
<div id="dialog-form" title="" class="container container_24">
	...
	<pop:token />
		...
		<div class="grid_15 form-left " id="selectPerson" style="line-height:20px;">
		...
		<div class="grid_6 lable-right">
			<label class="form-lbl">监护时间：</label>
		</div>
		<div class="grid_15">
		...
		<div class='clearLine'>&nbsp;</div>
		<div class="grid_6 lable-right">
			<label class="form-lbl">监护地点：</label>
		</div>
		<div class="grid_15">
		...
		<div class='clearLine'>&nbsp;</div>
		<div class="grid_6 lable-right">
			<label class="form-lbl">监护情况：</label>
		</div>
		<div class="grid_15">
...
<script type="text/javascript">
	...
	<s:if test='"add".equals(mode)'>
		$("#helpPrecordForm").attr("action","${path}/baseinfo/handicappedPrecord/addHelpPrecord.action");
	</s:if>
	<s:if test='"edit".equals(mode)'>
		$("#helpPrecordForm").attr("action","${path}/baseinfo/handicappedPrecord/updateHelpPrecord.action");
	</s:if>
		$("#helpPrecordForm").formValidate({
			...
			messages:{
				...
				"helpPrecord.address":{
					required:"监护地址不能为空",
					maxlength:$.format("监护地址最多输入{0}个字符")
				}
				,
				"helpPrecord.events":{
					maxlength:$.format("监护情况最多输入{0}个字符")
				}
