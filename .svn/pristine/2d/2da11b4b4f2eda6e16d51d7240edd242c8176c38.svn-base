...
<div class="content">
	...
	<div style="width: 100%">
		<table id="drugsSaftyList"></table>
		<div id="drugsSaftyListPager"></div>
	</div>
...
<script type="text/javascript">
	...
	function fastSearch(){
		...
		var initParam ={
			 'drugsSafty.address':name,
			 "policyPropaganda.mode":"gridConfigService",
			 "drugsSafty.organization.id":selectConfigTaskOrg()
		};
	    $('#drugsSaftyList').setPostData(initParam);
	...
	function signFormatter(el, options, rowData){
		var flag = "<@pop.JugePermissionTag ename='signDrugysSafty'>true</@pop.JugePermissionTag>";
	...
	<pop:formatterProperty name="category" domainName="@com.tianque.domain.property.PropertyTypes@DRUGS_SAFTY_CATEGORY" />
	//初始化表单
	$(document).ready(function(){
		...
		if(isConfigTaskSelect()){
			$.extend(postData,{"drugsSafty.mode":"gridConfigService","drugsSafty.funOrgId": $("#funOrgId").val()})
		}
		//表格 jqGrid初始化
		$('#drugsSaftyList').jqGridFunction({
			url:'${path}/serviceList/drugsSaftyManage/getDrugsSaftyList.action',
			datatype: "json",
			postData:postData,
		   	colModel:[
...
// 			       {name:'supervise',label:"督办",index:'supervise',formatter:superviseFormatter,sortable:false,width:80},
		...
		$('#add').click(function(){
			...
			$('#serviceListDialog').createDialog({
				...
				height:dialogHeight,
				title:'新增药品安全协管',
				url:'${path}/serviceList/drugsSaftyManage/dispatchOperate.action?mode=add&drugsSafty.organization.id='+selectConfigTaskOrg(),
				buttons:{
					'确定':function(){
						$('#maintainForm').submit();
					},
		...
		$('#update').click(function(){
			var id = $("#drugsSaftyList").jqGrid("getGridParam", "selarrrow");
			 ...
			 updateDrugsSaftyById(id);
		});
		
		//批量删除任务
		$('#delete').click(function(){
			var ids = $('#drugsSaftyList').getSelectedRowIds();
			if(ids.length <=0 ){
				 $.messageBox({
						level:'warn',
						message:'请先选择一个任务'
			...
			$.confirm({
				...
				okFunc: function(){
					$.ajax({
						url:'${path}/serviceList/drugsSaftyManage/deleteDrugsSafty.action?'+idStr,
						type:'get',
						dataType:'json',
		...
		$("#import").click(function(event){
			serviceListCommonImport("drugsSafty","DRUGSSAFTY","drugsSaftyList");
		});
		...
		$('#reload').click(function(){
			...
			$('#drugsSaftyList').setPostData(postData);
			$('#drugsSaftyList').trigger('reloadGrid');
		});
	});
	//根据id修改
	function updateDrugsSaftyById(id){
// 		var id =  $('#drugsSaftyList').getRowData(id);
		$('#serviceListDialog').createDialog({
			width:dialogWidth,
			height:dialogHeight,
			title:'修改药品安全协管',
			url:'${path}/serviceList/drugsSaftyManage/dispatchOperate.action?mode=edit&drugsSafty.id='+id,
			buttons:{
				'确定':function(){
					$("#maintainForm").attr("action","${path}/serviceList/drugsSaftyManage/updateDrugsSafty.action?");
					$('#maintainForm').submit();
				},
				'关闭':function(){
					$(this).dialog('close');
	...
	function addressFormatter(el, options, rowData){
		return "<a href='javascript:' onclick='viewdrugsSafty("+rowData.id+")'>"+rowData.address+"</a>"
	}
	
	//根据id查看
	function viewdrugsSafty(id){
		$("#serviceListDialog").createDialog({
			width:dialogWidth,
			height:dialogHeight,
			title:"查看药品安全协管",
			url:"${path}/serviceList/drugsSaftyManage/viewDrugsSaftyDetail.action?drugsSafty.id="+id,
			buttons:{
				"关闭":function(){
					$(this).dialog("close");
				}
	...
	function selectRow(){
	    var selectedIds = $("#drugsSaftyList").jqGrid("getGridParam", "selarrrow");
	...
	function updateOperator(id){
		...
		var builddata=$("#drugsSaftyList").getRowData(id);
		...
		$("#serviceListDialog").createDialog({
			...
			height:470,
			title: '签收',
			url:'${path}/serviceList/drugsSaftyManage/dispatchOperate.action?mode=sign&drugsSafty.organization.id='+ selectConfigTaskOrg()+'&drugsSafty.id='+id,
			buttons: {
				"签收" : function(){
					$("#maintainForm").submit();
				},
	...
	function doReply(id){
		$("#serviceListDialog").createDialog({
			...
			height:470,
			title: '回复',
			url:'${path}/serviceList/drugsSaftyManage/dispatchOperate.action?mode=reply&drugsSafty.organization.id='+ selectConfigTaskOrg()+'&drugsSafty.id='+id,
			buttons: {
				"回复" : function(){
					$("#maintainForm").submit();
				},
