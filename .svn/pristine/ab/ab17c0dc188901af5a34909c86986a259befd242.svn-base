...
<%@ include file="/includes/baseInclude.jsp" %>
	...
	<input id="populationType" type="hidden" name="population.populationType" value="${locationType}" />
		<div id=tabs>
			<ul>
				<li><a href="${path}/baseinfo/publicPlaceManage/viewPublicPlaceByEncrypt.action?location.id=${location.encryptId}">基本信息</a> </li>
	...
	$(function() {
		$( "#tabs" ).tabs();
		/*$.ajax({
			url:"${path}/baseinfo/publicPlaceManage/viewPublicPlace.action?location.id=${location.id}",
			success:function(data){
				$("#publicPlaceInfo").html("");
				$("#publicPlaceInfo").html(data);	
			}
			});	*/
	});
	</script>