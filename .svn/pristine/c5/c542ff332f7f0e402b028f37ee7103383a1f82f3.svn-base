...
<%
 request.setAttribute("houseAddress",java.net.URLDecoder.decode(request.getParameter("houseAddress"),"utf-8"));
 request.setAttribute("hsAddress",java.net.URLDecoder.decode(request.getParameter("hsAddress"),"utf-8"));
 request.setAttribute("hsType",java.net.URLDecoder.decode(request.getParameter("hsType"),"utf-8"));
...
<input type="hidden" id="houseId_search" name="houseId" value="<s:property value="#parameters.houseId"/>"/>
<input type="hidden" id="houseAddress_search" name="houseAddress" value="${houseAddress }"/>
<%-- <input type="hidden" id="houseInfoId_search" name="houseInfoId" value="<s:property value="#parameters.houseInfoId"/>"/> --%>
<input type="hidden" id="houseOpertyType_search" name="houseOpertyType" value="<s:property value="#parameters.houseOpertyType"/>"/>
<input type="hidden" id="hsType" name="hsType" value="${hsType}"/>
<input type="hidden" id="hsAddress" name="hsAddress" value="${hsAddress}"></input>
<div class="content">
	<div style="width: 100%;">
...
$(document).ready(function(){
		...
		postData: {
	        "systemOperateLogVo.searchType": $("#houseOpertyType_search").val(),
	        "systemOperateLogVo.dataKeyword" : $("#houseId_search").val()
        },
		datatype: "json",
		colModel:[
	    	{name:"id", index:"id", hidden:true},
	    	{name:"operateDate", index:"operateDate",label:"时间",width:130},
	    	{name:"operatePerson", index:"operatePerson",label:"操作账号",width:135,align:"center",formatter:dealOperatePerson},
...
function dealOperatePerson(el,options,rowData){
	var userName = rowData.operatePerson;
	if(userName!=null && typeof(userName)!='undefined' && userName=='admin' ){
		return "系统消息";
	}else{
		return userName;
	}
