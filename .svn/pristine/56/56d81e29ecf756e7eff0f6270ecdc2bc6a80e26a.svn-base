...
$(document).ready(function(){
	var gridoption = {
		url:"${path}/workingRecord/societySecurityManage/societySecurityList.action?organization.id="+currentOrgId+"&dailyDirectoryId="+dailyId,
		datatype: "json",
		colNames:['id','建设情况','附件','录入时间'],
	   	colModel:[
	...
	$("#add").click(function(event){
		...
		if(!selectedNode.attributes.leaf){
			...
			modal : true,
			url:"${path}/workingRecord/societySecurityManage/dispatch.action?mode=add"+"&dailyDirectoryId="+dailyDirectoryId,
			buttons :{
				"保存" : function(){
	...
	$("#update").click(function(event){
		...
		$("#dailyLogMaintanceDialog").createDialog({
			...
			url:"${path}/workingRecord/societySecurityManage/dispatch.action?mode=edit&societySecurity.id="+selectedId,
			buttons : {
				"保存" : function(){
					$("#dailyLog-form").submit();
				},
	...
	$("#reload").click(function(){
		$("#dailyLogList").setGridParam({
			url:"${path}/workingRecord/societySecurityManage/societySecurityList.action?organization.id="+currentOrgId,
			postData:{
...
function doubleClickTable(selectedId){
	$("#dailyLogMaintanceDialog").createDialog({
		...
		modal : true,
		url:"${path}/workingRecord/societySecurityManage/dispatch.action?mode=view&societySecurity.id="+selectedId,
		buttons :{
			"关闭" : function(){
				$(this).dialog("close");
			}
...
function showDailyLogAttachFile(){
	$.each($(".popUpMore"), function(i, n){
		$.ajax({
			url:"${path}/workingRecord/societySecurityManage/getSocietySecurityById.action?societySecurity.id="+$(n).attr("societySecurityId")+"&organization.id="+currentOrgId,
			success:function(dailylog){
				var popMoreData = [];
				for(var j = 0; j < dailylog.dailyAttachFiles.length; j++){
					popMoreData[j]={id:dailylog.dailyAttachFiles[j].id, url:'${path}/workingRecord/societySecurityManage/downloadDailyAttachFile.action?dailyAttachFile.id='+dailylog.dailyAttachFiles[j].id, text:dailylog.dailyAttachFiles[j].fileName,clickFun:function(){}};
				}
...
function formatterAttachFile(el,options,rowData){
	if(rowData.dailyAttachFiles.length>0){
		return "<div style='clear:both' align='center'><a href='javascript:;' id='societySecurity_"+rowData.id+"' style='color:#666666' class='popUpMore' societySecurityId='"+rowData.id+"' >附件>></a></div>";
	}
	return "";
}
...
function deleteDailyLog(selectedIds){
	$.ajax({
		url:'${path}/workingRecord/societySecurityManage/deleteSocietySecurity.action?societySecurityIds='+selectedIds+"&organization.id="+currentOrgId,
