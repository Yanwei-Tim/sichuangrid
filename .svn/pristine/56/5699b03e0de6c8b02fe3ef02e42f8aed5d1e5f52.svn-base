 ...
 */
@Namespace("/baseInfoStat/companyPlaceStanalsManage")
@Controller("statisticsCompanyPlaceController")
@Scope("prototype")
public class StatisticsCompanyPlaceController extends BaseAction {
	private static Logger logger = LoggerFactory.getLogger(StatisticsCompanyPlaceController.class);
	/** 区别单位场所是哪个单位场所 */
	...
	public String getStatisticsCompanyPlaceColumn() throws Exception {
		...
		if (year == Calendar.getInstance().get(Calendar.YEAR)
			...
			companyPlaceColumn = companyPlaceLeaderViewService.getCompanyPlaceColumnByOrgIdAndType(
					orgId, moduleType);

		} else {
			// 加载历史数据
			companyPlaceColumn = companyPlaceLeaderViewService.getCompanyPlaceColumnByTime(orgId,
					moduleType, year, month);
		}
		return SUCCESS;
	}
	...
	public String getStatisticsCompanyPlaceList() throws Exception {
		...
		if (year == Calendar.getInstance().get(Calendar.YEAR)
				&& month == (Calendar.getInstance().get(Calendar.MONTH) + 1)) {
			populationAreaData = companyPlaceLeaderViewService.getCurrentAreaDate(orgId,
					moduleType, orgLevelDistance);
		} else {
			populationAreaData = companyPlaceLeaderViewService.getAreaDateByDate(orgId, moduleType,
	...
	public String createStatisticsCompanyPlaceList() throws Exception {
		...
		companyPlaceLeaderViewService.addStatisticsCompanyPlace(year, month, orgId, moduleType);
		return SUCCESS;
	}
	...
	public String showTendencyChartForCompanyPlace() throws Exception {
		...
		companyPlaceColumn = companyPlaceLeaderViewService.findTendencyChart(moduleType, orgId);
		return SUCCESS;
	}
	...
	public String getStatisticsCompanyPlacePie() throws Exception {
		...
		companyPlacePie = companyPlaceLeaderViewService.getCompanyPlacePieInfo(year, month, orgId,
				moduleType);
		return SUCCESS;
	}
	...
	public void setPopulationAreaData(List<PopulationAreaDataVo> populationAreaData) {
