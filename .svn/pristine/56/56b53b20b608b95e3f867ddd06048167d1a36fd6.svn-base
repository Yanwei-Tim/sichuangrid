...
<div class="content">
	<jsp:include page="${path}/account/peopleAspiration/energyForViewButtons.jsp"/>
...
$(document).ready(function(){
	...
	$("#issueList").jqGridFunction({
		...
		colModel:[
			...
			{name:'targetOrg.id',index:'targeOrg.id',frozen:true,hidden:true,hidedlg:true},
...
function initIssueCount(){
	$.ajax({
        url: "/threeRecords/energy/getIssueCount.action",
        data: {
...
function viewIssue(rowid){
	...
	//if (hasRowSelected() && $("#view").isButtonEnabled()){
		...
		$("#issueDialog").createDialog({
			...
			height: 500,
			title: "查看能源信息详情",
			url: "${path}/threeRecords/energy/dispatch.action?mode=viewNew&keyId="+selectedIssue.encryptIdByIssueId,
			buttons: {
		   		"关闭" : function(){
		        	$(this).dialog("close");
		   		}
