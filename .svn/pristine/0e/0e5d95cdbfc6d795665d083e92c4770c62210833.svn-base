...
function SigmaReport(box,context,columns,printClassName,printTable_title,tableTitle,printBtn,sortFun){
	...
	if (element.printClassName == "SigmaReport") {
		$('.SigmaReport').height($('body').height() - topVal - 15)
		//$('.SigmaReport .body').height($('body').height()- $('.SigmaReport .head').height() - topVal -50)
		$(".SigmaReport #tableWrap")
				.height(
						$('.SigmaReport .body').height()
								+ ($('body').height()
										- $('.SigmaReport .head').height()
										- topVal - 50));
		$(".SigmaReport .body,.SigmaReport #body_gridbox table").width(
				$("#tHead_gridbox").width());
	}
	//this.layout();
	$.loadingComp("close");
