...
@Component("enterpriseDataConverter")
public class EnterpriseDataConverter extends AbstractDataConverter<Enterprise> {
	...
	public Enterprise persistenceDomain(Enterprise domain) {
		if (domain.getType() == null) {
			if (domain.getKeyType() != null && domain.getKeyType().equals("enterpriseKey")) {
				...
				for (int i = 0; i < propertyDictList.size(); i++) {
					...
					if (domain1.getDisplayName().equals("规上企业")) {
	...
	public Enterprise convertToDomain(String[] cellValues, ValidateResult result,
		...
		ExcelImportHelper.newProcImportDate(beanDatas, cellValues, getUploadOrganization(),
				enterprise, result);
		enterprise.setKeyType("enterpriseKey");
		return enterprise;
	}
