...
@Controller("fourTeamsIOrgToolsController")
@Scope("prototype")
@Namespace("/fourTeams/tools/org")
public class FourTeamsOrgToolsController extends ActionSupport {
	/** 要比较的部门id */
	private Long orgId;
	/** 目标层级 */
	...
	public String compareOrgLevel() {
		if (null != orgId) {
			...
			numResult = OrganizationLevel.levelCompare(org.getOrgLevel()
					.getInternalId(), orgLevel);
		} else {
			numResult = 1;
		}
		return "success";
