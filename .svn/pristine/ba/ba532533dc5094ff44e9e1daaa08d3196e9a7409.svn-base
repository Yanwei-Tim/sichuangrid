...
<div class="content">
	...
	<a id="addServiceRecord1" href="javascript:void(0)"><span>新增</span></a>
	</@pop.JugePermissionTag>
...
$(document).ready(function(){
	...
	$("#serviceRecordList").jqGridFunction({
		...
		colModel:[
		    ...
		    {name:'occurDate',index:"occurDate",label:'走访时间',sortable:true,width:80,align:"center"},
		    {name:'serviceJoiners',index:'serviceJoiners',label:'走访人',sortable:false,width:80,align:"center"},
		    {name:'serviceJoiners',index:'serviceJoiners',label:'帮扶人员',sortable:false,width:80,align:"center"},
		    {name:'serviceRecordAttachments',label:'有无异常',sortable:false, width:200,formatter:formatterAttachFile}
		],
	...
	//新增按钮事件
	$("#addServiceRecord1").click(function(event){
		var objectNames=$("#name").val();
		$("#serviceRecordDialog").createDialog({
			title:"新增服务记录",
			width: 600,
			height: 400,
			url:'${path}/plugin/serviceTeam/serviceRecord/dispatch1.action?mode=add&dailogName=serviceRecordDialog&objectIds=${(id)!}&populationType=${(populationType)!}&objectNames='+encodeURIComponent(objectNames)+'&showRecordType=+${(showRecordType)!}',
			buttons: {
	...
	$("#searchServiceRecord").click(function(event){
		$("#serviceRecordDialog").createDialog({
			...
			width: 600,
			height: 300,
			url:'${path}/plugin/serviceTeam/serviceRecord/dispatch.action?showRecordType=${(showRecordType)}&mode=search&dailogName=serviceRecordDialog&objectIds=${(id)!}&populationType=${(populationType)!}',
			buttons: {
				"查询" : function(event){
	...
	function deleteRecordOperator(selectedIds){
		...
		if(serviceRecord.internalId>USER_ORG_LEVEL){
			$.messageBox({level:'warn',message:"选中的记录层级高于当前登录层级，无法对该记录进行删除操作！"});
			return;
		}
		$.confirm({
			title:"确认删除",
	...
	function updateRecordOperator(selectedId){
		...
		if(serviceRecord.internalId>USER_ORG_LEVEL){
			$.messageBox({level:'warn',message:"选中的记录层级高于当前登录层级，无法对该记录进行修改操作！"});
			return;
		}
		$("#serviceRecordDialog").createDialog({
			title:'修改服务记录',
			width: 600,
			height: 400,
			url:'${path}/plugin/serviceTeam/serviceRecord/dispatch.action?mode=edit&dailogName=serviceRecordDialog&objectNames=${(name)!}&objectIds=${(id)!}&serviceRecord.id='+selectedId+'&showRecordType=+${(showRecordType)!}&serviceObjectId='+$("#id").val(),
			buttons: {
				"保存并关闭" : function(event){
					$("#serviceRecordForm").submit();
	   			},
