...
<script type="text/javascript">
	...
	function loadList(){
		$("#helpPrecordList").setGridParam({
			url:'${path}/baseinfo/handicappedPrecord/findHelpPrecord.action',
			datatype: "json",
			page:1
		});
	...
	$(document).ready(function(){
		...
		$("#helpPrecordList").jqGridFunction({
			...
			colModel:[
				...
				{name:"helpTime",sortable:false ,index:"floorpersonsTime", label: '监护人时间',width:100},
				{name:"address", sortable:false, label:'监护地点', width:100},
				{name:"helpPersonnelName", sortable:false, label: '监护人',width:200},
			...
			ondblClickRow: viewhandicappedHelpPre,
		...
		$("#addHelpPrecord").click(function(){
			...
			$("#helpPrecordDialog").createDialog({
				...
				height: 360,
				title:'新增监护情况',
				url:'${path}/baseinfo/handicappedPrecord/dispatch.action?personnelId='+domainId+'&personnelType=<s:property value="#parameters.keyType"/>&mode=add&precordType=handicappedHelp',
				buttons: {
					"保存" : function(event){
		...
		$("#updateHelpPrecord").click(function(){
			...
			$("#helpPrecordDialog").createDialog({
				...
				height: 400,
				title:'修改监护情况',
				url:'${path}/baseinfo/handicappedPrecord/dispatch.action?helpPrecord.id='+selectedId+'&mode=edit&precordType=handicappedHelp',
				buttons: {
					"保存" : function(event){
				      $("#helpPrecordForm").submit();
				   },
	...
	function viewhandicappedHelpPre(){
		var selectedId = $("#helpPrecordList").getGridParam("selrow");
		if(selectedId==null){
		  return;
		}
		$("#helpPrecordDialog").createDialog({
			...
			height: 400,
			title:'查看监护情况',
			url:'${path}/baseinfo/handicappedPrecord/dispatch.action?helpPrecord.id='+selectedId+'&mode=view&precordType=handicappedHelp',
			buttons: {
			   "关闭" : function(){
			        $(this).dialog("close");
			   }
	...
	function deleteHelpPrecord(){
		...
		$.ajax({
			url:"${path}/baseinfo/handicappedPrecord/deleteHelpPrecordById.action",
			data:{
