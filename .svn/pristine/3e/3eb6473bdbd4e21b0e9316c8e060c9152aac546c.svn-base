...
<form id="searchCommentLogForm">
	<div class="container container_24">
	    <div class="grid_4 lable-right">
			<label class="form-lbl">点评时间从：</label>
		</div>
		<div class="grid_7 " >
	    	<input type="text" name="searchPeopleLogVo.commentStartDate" id="commentStartDate"
		...
		<div class="grid_7" >
	    	<input type="text" name="searchPeopleLogVo.commentEndDate" id="commentEndDate" readonly class="form-txt " />
		</div>
		<div class="grid_4 lable-right">
			<label class="form-lbl">日志时间从：</label>
		</div>
		...
		<div class="grid_18">
			<input type="text" name="searchPeopleLogVo.title" id="title" maxlength="50" style="width: 455px" class="form-txt" />
		</div>
		<div class="grid_4 lable-right">
			<label class="form-lb1">地点：</label>
		</div>
		<div class="grid_18">
			<input type="text" name="searchPeopleLogVo.address" id="address" maxlength="50" style="width: 455px" class="form-txt" />
		</div>
		<div class="grid_4 lable-right">
			<label class="form-lb1">点评人：</label>
		</div>
		<div class="grid_18">
			<input type="text" name="searchPeopleLogVo.commentLog.commentUser" id="commentUser" maxlength="50" style="width: 455px" class="form-txt"  />
		</div>
		<div class="grid_4 lable-right">
			<label class="form-lb1">点评语：</label>
		</div>
		<div class="grid_18">
			<input type="text" name="searchPeopleLogVo.commentLog.comments" id="comments" maxlength="50" style="width: 455px" class="form-txt"  />
		</div>
	</div>
</form>
...
$(function(){
	...
	$("#sumCommentMin").blur( function () {
		...
		if(sumCommentMin >= sumCommentMax && sumCommentMax!=0){
			$.messageBox({message:"最大点评次数必须大于最小点评次数!",level:"warn"});
			$("#sumCommentMin").val("");
		}
	});
...
$('#commentStartDate').datePicker({
	...
	maxDate:'+0d',
		...
		if(dateText!=null&&dateText!=''){
			var dateUnit=dateText.split('-');
			var date=new Date(dateUnit[0],dateUnit[1]-1,dateUnit[2]);
			$("#commentEndDate").datepicker("option", "minDate",date);
		}
	}
});

$('#commentEndDate').datePicker({
	yearRange: '1900:2030',
	dateFormat:'yy-mm-dd',
	maxDate:'+0d',
    onSelect: function(dateText, inst) {
		if(dateText!=null&&dateText!=''){
			var dateUnit=dateText.split('-');
			var date=new Date(dateUnit[0],dateUnit[1]-1,dateUnit[2]);
			$("#commentStartDate").datepicker("option", "maxDate",date);
		}
	}
});
