...
<div class="container container_24">
	...
	<@pop.token />
		...
		<input type="hidden" name="id" id="objectId" value="${(id)! }" /> 
 		...
 		<input name="serviceTeamMemberVo.objectType" type="hidden" id="MemberObjectType" value="${(module)! }"/>
 		<input name="serviceTeamMemberVo.objectId" type="hidden" id="MemberObjectId" value="${(id)! }"/>
...
function setPersonData(personData){
	...
	$("#MemberObjectId").val($("#objectId").val());
	$("#MemberObjectType").val($("#module").val());
}
...
<@pop.formatterProperty name="position" domainName="@com.tianque.plugin.serviceTeam.util.Constants$ServiceTeamPositionPropertyTypes@TEAMPOSITION" />
	...
	function loadData() {
		...
		$("#supervisorMemberList").setGridParam({
			url:"${path}/baseinfo/routerManage/findServiceMemberFromTeams.action",
			datatype: "json",
			page:1,
			mtype:"post"
		});
...
var data = $("#supervisorMemberList").getRowData(memberBaseId);
	$.ajax({
		async: false,
		url:"${path}/baseinfo/routerManage/addObjectAndMemberRelation.action",
	   	data:{
	   		"serviceMemberWithObject.memberId":memberBaseId,
			"serviceMemberWithObject.objectType":$("#module").val(),
			"serviceMemberWithObject.objectName":$("#objectName").val(),
			"serviceMemberWithObject.objectId":$("#objectId").val(),
