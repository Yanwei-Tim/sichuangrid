...
<div>
	<table id="tb" border="1"
		...
		<tr>
			...
			<td name="inspectCadre" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.inspectCadre }</td>
		</tr>
		<tr>
			<td>发动群众数</td>
			<td name="inspectPeople" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.inspectPeople }</td>
		</tr>
		<tr>
			<td>组织工作组数</td>
			<td name="inspectworkingTeam" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.inspectworkingTeam }</td>
		</tr>
	
		<tr>
			<td>其他</td>
			<td name="inspectOther" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.inspectOther }</td>
		</tr>
	
		<tr>
			<td rowspan="6" align="center" style="width: 130px; height: 120px;">宣传发动数</td>
			<td colspan="2" align="center" style="width: 260px;">总数</td>
			<td name="publicitySum" class="toDataJosn" style="width: 100px; height: 24px;">${emphasisSafetyDetail.publicitySum }</td>
		</tr>
		<tr>
			<td rowspan="5" align="center" style="width: 130px; height: 98px;">其中</td>
			<td style="width: 130px; height: 24px;">发布通告数</td>
			<td name="publicityPublicNotice" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.publicityPublicNotice }</td>
		</tr>
		<tr>
			<td>召开座谈会数</td>
			<td name="publicitySymposium" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.publicitySymposium}</td>
		</tr>
	
		<tr>
			<td>群众举报数</td>
			<td name="publicityPeopleInform" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.publicityPeopleInform}</td>
		</tr>
	
		<tr>
			<td>从中破获刑事案件数</td>
			<td name="publicityCrackedCase" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.publicityCrackedCase}</td>
		</tr>
		<tr>
			<td>从中抓获犯罪嫌疑人数</td>
			<td name="publicityCatchingCriminal" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.publicityCatchingCriminal}</td>
		</tr>
		...
		<tr>
			...
			<td colspan="2" align="center" style="width: 260px;">总数</td>
			<td name="findAreaSum" class="toDataJosn" style="width: 100px; height: 24px;">${emphasisSafetyDetail.findAreaSum}</td>
		</tr>
		<tr>
			<td rowspan="4" align="center" style="width: 130px;">其中</td>
			<td style="width: 130px; height: 24px;">县市区数</td>
			<td name="findAreaCounty" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.findAreaCounty}</td>
		</tr>
		<tr>
			<td>乡镇(街道)数</td>
			<td name="findAreaStreet" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.findAreaStreet}</td>
		</tr>
	
		<tr>
			<td>村(社区)数</td>
			<td name="findAreaVillage" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.findAreaVillage}</td>
		</tr>
	
		<tr>
			<td>其他</td>
			<td name="findAreaOther" class="toDataJosn" style="height: 24px;">${emphasisSafetyDetail.findAreaOther}</td>
		</tr>
...
<script type="text/javascript">
	...
	function submitData(){
		...
		submitUrl="${path}/daily/publicSecurityRenovateManage/addEmphasisSafetyDetail.action?dailyDirectory.id="+dailyDirectoryId+"&dailyYear.id="+$("#dailyYear").val()+"&"+allJson;
		</s:if>
		<s:if test='"edit".equals(mode)'>
		submitUrl="${path}/daily/publicSecurityRenovateManage/updateEmphasisSafetyDetail.action?"+allJson;
		</s:if>
		$.ajax({
			url: submitUrl,
