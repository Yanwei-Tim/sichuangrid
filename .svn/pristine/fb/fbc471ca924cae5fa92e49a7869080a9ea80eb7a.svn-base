...
<div id="dialog-form" title="办理事件处理" class="container container_24">
	<div class="event-process">
		...
		<div class="dashedLine clearfix"></div>
			...
			<div class="grid_3 lable-right">
				<label class="form-lbl">
					<s:if test='"conceptInList".equals(mode)'>承办人：</s:if>
					<s:if test='"readInList".equals(mode)'>阅读人：</s:if>
				</label>
			</div>
			<div class="grid_8">
...
$(document).ready(function(){
	...
	<s:if test='"conceptInList".equals(mode)'>
		$("#maintainForm").attr("action","${path }/issues/issueManage/dealIssue.action");
	</s:if>
	<s:if test='"readInList".equals(mode)'>
		...
		submitHandler: function(form){
			$(form).ajaxSubmit({
				success:function(data){
					...
					if($("#DialogName").val()=='maxConcept'){
						$("#maxIssueForBenchDialog").dialog("close");
						$("#maxIssueListForNeed").addRowData(data.issueStepId,data,"after",$("#keyId").val());
						$("#maxIssueListForNeed").setSelection(data.issueStepId);
						$.confirm({
							title:"提示",
							message:"已经成功受理该事件处理，是否继续办理该事件？",
							okFunc: function() {
								dealIssue(data.issueStepId);
							},
							cancelFunc:function(){
						...
						$("#issueForBenchDialog").dialog("close");
						$("#issueListForNeed").addRowData(data.issueStepId,data,"after",$("#keyId").val());
						$("#issueListForNeed").setSelection(data.issueStepId);
						$.confirm({
							title:"提示",
							message:"已经成功受理该事件处理，是否继续办理该事件？",
							okFunc: function() {
