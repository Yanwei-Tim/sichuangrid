...
$("#statistical").click(function(){
    ...
    var glPlan = $("#isGtPlannedInvestment").val() == '大(等)于' ? 1 : 0;
    var glBen = $("#isGtbeneficiaryNumber").val() == '大(等)于' ? 1 : 0;
    var glSelf = $("#isGtSelfFund").val() == '大(等)于' ? 1 : 0;
    var glGap = $("#isGtGapFund").val() == '大(等)于' ? 1 : 0;
    var mode = "";
    var url = "";
    if(ledger == 11){
    	mode = "water";
    ...
    url = "/threeRecords/peopleaspirationComprehensive/countIssueForView.action?mode="+mode+"&orgId="+orgId+"&orgIds="+moreSelectOrgIds+"&searchVo.ledgerType="+ledger+
	"&searchVo.searchValue="+$("#seachValue").val()+"&searchVo.doneType="+doneType+"&searchVo.detailDoneType="+$("#detailDoneType").val()+
	"&searchVo.beginCreateDate="+$("#beginCreateDate").val()+"&searchVo.endCreateDate="+$("#endCreateDate").val()+"&searchVo.isGtPlannedInvestment="+glPlan+
	"&searchVo.plannedInvestment="+$("#plannedInvestment").val()+"&searchVo.isGtbeneficiaryNumber="+glBen+"&searchVo.beneficiaryNumber="+$("#beneficiaryNumber").val()+
	"&searchVo.isGtSelfFund="+glSelf+"&searchVo.selfFund="+$("#selfFund").val()+"&searchVo.isGtGapFund="+glGap+"&searchVo.gapFund="+$("#gapFund").val()+
	"&searchVo.projectCategory="+$("#projectCategory").val()+"&searchVo.buildType="+$("#buildType").val()+"&searchVo.buildAddress="+$("#buildAddress").val()+
	"&searchVo.appealContent="+$("#appealContent").val();
    if(ledger == 17){
    	mode = "labor";
    	url = "/threeRecords/peopleaspirationComprehensive/countIssueForView.action?mode="+mode+"&orgId="+orgId+"&orgIds="+moreSelectOrgIds+"&searchVo.ledgerType="+ledger+
		"&searchVo.searchValue="+$("#seachValue").val()+"&searchVo.doneType="+doneType+
		"&searchVo.beginCreateDate="+$("#beginCreateDate").val()+"&searchVo.endCreateDate="+$("#endCreateDate").val()+
		"&searchVo.projectCategory="+$("#projectCategory").val()+"&searchVo.appealContent="+$("#appealContent").val();
    }
...
$("#situation").click(function(event){
	if ($("#peopleIssueList").getGridParam("records")>0){
		$("#peopleCountDialog").createDialog({
			width: 250,
			height: 200,
			title:'导出水利详细信息',
			url:PATH+'/common/exportExcel.jsp',
			postData:{
				gridName:'peopleIssueList',
