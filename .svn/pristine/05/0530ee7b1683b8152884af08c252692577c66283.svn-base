...
public class HSSFCellUtil {
	...
	public static void setAlignment(HSSFCell cell, HSSFWorkbook workbook,
			short align) throws NestableException {
	...
	public static void setFont(HSSFCell cell, HSSFWorkbook workbook,
			HSSFFont font) throws NestableException {
	 ...
	 * then it creates a new one. If it does create a new one, then it applyies
	 * the propertyName and propertyValue to the style. This is nessasary
	...
	public static void setCellStyleProperty(HSSFCell cell,
		...
		try {
			...
			if (newStyle == null) {
				...
				newStyle.setFont(workbook.getFontAt(originalStyle
						.getFontIndex()));
				PropertyUtils.copyProperties(newStyle, originalStyle);
				PropertyUtils
						.setProperty(newStyle, propertyName, propertyValue);
			}

			cell.setCellStyle(newStyle);
		} catch (Exception e) {
			throw new SystemUtilException("Couldn't setCellStyleProperty.", e);
		}
	}
