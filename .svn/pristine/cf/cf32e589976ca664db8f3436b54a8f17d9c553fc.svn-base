...
import com.tianque.plugin.analyzing.dao.StatAnalysePlaceDao;
...
@Service("statAnalysePlaceService")
public class StatAnalysePlaceServiceImpl extends AbstractBaseService implements
		StatAnalysePlaceService {
	@Autowired
	private StatAnalysePlaceDao statAnalysePlaceDao;
	@Autowired
	...
	public List<StatAnalysePlaceVo> findStatAnalysePlace(String tableName,
		...
		for (int i = 0; i < list.size(); i++) {
			sum += statAnalysePlaceDao.getStatAnalysePlace(tableName,
					list.get(i).getOrgInternalCode(), keyType);
		}
		// 添加每条记录
		for (int i = 0; i < list.size(); i++) {
			StatAnalysePlaceVo statAnalysePlaceVo = new StatAnalysePlaceVo();
			int total = statAnalysePlaceDao.getStatAnalysePlace(tableName, list
					.get(i).getOrgInternalCode(), keyType);
			statAnalysePlaceVo.setTotal(total);
			int ishelp = statAnalysePlaceDao.getIsHelpStatAnalysePlace(
	...
	public List<StatAnalysePlaceVo> findStatAnalysePlaceFromHistory(
		...
		for (int i = 0; i < list.size(); i++) {
			...
			int total = getCountByTime(list.get(i).getOrgInternalCode(),
					BaseInfoTables.getTypeDisplayNames(keyType),
					BaseInfoTables.IMPORTANTPLACE_KEY, year, month);
			int ishelp = statisticsBaseInfoDao.getIsHelpFromHistory(list.get(i)
					.getOrgInternalCode(), BaseInfoTables
	...
	public HighchartColumnVo getStatAnalysePlaceFromHistory(Long orgId,
		...
		// 查询帮教情况，历史
		List<Map<String, Object>> list = statisticsBaseInfoDao
	...
	private HighchartColumnVo finHelpData(String keyType,
		...
		String[] propertyDictList = AnalyseUtil.groupMap.get(BaseInfoTables
				.getTypeDisplayNames(keyType));
	...
	private int getCountByTime(String orgInternalCode, String tableName,
		...
		int total = statisticsBaseInfoDao.getCountFromHistory(map);
		return total;
	}
	...
	public HighchartColumnVo getStatAnalysePlace(String tableName, Long orgId,
		...
		int[] data = new int[organizations.size()];
		for (int i = 0; i < organizations.size(); i++) {
			data[i] = statAnalysePlaceDao.getStatAnalysePlace(tableName,
		...
		// 查询帮教情况，历史
		List<Map<String, Object>> list = statAnalysePlaceDao
