...
<@s.include value="/includes/baseInclude.jsp"/>
		<div id="enterpriseInfo" class="container container_24">
			...
			<div class="grid_1"></div>
				...
				<div class="grid_7">
					<select name="location.type.id" class="form-txt dialogtip" id="location-type-id" title="规上企业：注册资金500万以上，企业职工人数超过200人的企业。规下企业：注册资金500万以下，企业职工人数少于200人的企业。"
						<@s.if test='"view".equals(mode)'>disabled='true'</@s.if>>
						<@pop.OptionTag name="@com.tianque.domain.property.PropertyTypes@SECURITY_TYPE" defaultValue="${(location.type.id)!}" />
					</select>
				</div>
			<div class='clearLine'>&nbsp;</div>
			<div class="grid_4 lable-right">
			...
			<div class="grid_20">
				<input type="text" name="location.address" value="${(location.address)! }" maxlength="50"
					<@s.if test='"view".equals(mode)'>disabled='true'</@s.if> class="form-txt" style="width: 99%" />
			</div>
			<div class="grid_1">
				<@s.if test='!"view".equals(mode)'>
			...
			<div class="grid_4">
				<select name="location.type.id" class="form-txt dialogtip"
					<@s.if test='"view".equals(mode)'>disabled='true'</@s.if>>
			...
			<div class="grid_4">
				<input type="text" name="location.manager" class="form-txt" value="${(location.manager)! }" maxlength="20"
					<@s.if test='"view".equals(mode)'>disabled='true'</@s.if> />
			</div>
			<div class="grid_1">
				<@s.if test='!"view".equals(mode)'>
		...
		<div class='clearLine'>&nbsp;</div>
			...
			<div class="grid_4">
				<input type="text" name="location.mobileNumber" value="${(location.mobileNumber)! }" maxlength="11"
					<@s.if test='"view".equals(mode)'>disabled='true'</@s.if> class="form-txt  dialogtip" title="请输入11位以1开头的手机号码!例如:13988888888" />
			</div>
			<div class='clearLine'>&nbsp;</div>
			...
			<div class="grid_21">
				<input id="isRiskEnterprise" type="checkbox" name="location.riskEnterprise" value="true" class="dialogtip"
					<@s.if test="location.riskEnterprise" >checked="checked"</@s.if>
					<@s.if test='"view".equals(mode)'>disabled='true'</@s.if> title="指依法设立且取得场所法人营业执照的，从事危险化学品生产的场所，包括最终产品或中间产品列入《危险化学品名录》的危险化学品的生产场所。" />
				<label class="form-check-text" for="isRiskEnterprise">是否为危化场所</label>
			</div>
			<div class="grid_3 lable-right">
				<label class="form-lbl">联系电话：</label>
			</div>
			<div class="grid_4">
				<input type="text" name="location.telephone" maxlength="15" value="${(location.telephone)! }" class="form-txt  dialogtip"
					<@s.if test='"view".equals(mode)'>disabled='true'</@s.if> title="请输入由数字和-组成的联系电话!例如：0577-88888888" />
			</div>
			<div class='clearLine'>&nbsp;</div>
			...
			<div class="grid_3">
				<input type="text" name="location.registeredCapital" maxlength="11" class="form-txt" style="text-align:right;" value="${(location.registeredCapital)! }"
					<@s.if test='"view".equals(mode)'>disabled='true'</@s.if> />
			</div>
			<div class="grid_2" >
			...
			<div class="grid_4">
				<select name="location.industry.id" class="form-txt"
					<@s.if test='"view".equals(mode)'>disabled='true'</@s.if>>
					<@pop.OptionTag name="@com.tianque.domain.property.PropertyTypes@CAREER" defaultValue="${(location.industry.id)!}" />
				</select>
			</div>
			<div class='clearLine'>&nbsp;</div>
			...
			<div class="grid_4">
				<input type="text" name="location.area" maxlength="11" class="form-txt" style="text-align: right; width: 85%;"
					<@s.if test='"view".equals(mode)'>disabled='true'</@s.if> value="${(location.area)!}" />M<sup>2</sup>
			</div>
			<div class="grid_4 lable-right">
				<label class="form-lbl">员工数：</label>
			</div>
			<div class="grid_5">
				<input type="text" name="location.employeeAmount" id="employeeAmount" value="${(location.employeeAmount)! }" maxlength="9" style="text-align: right;"  
			...
			<div class="grid_4">
				<input type="text" name="location.partyMemberAmount" id="partyMemberAmount" value="${(location.partyMemberAmount)! }" maxlength="9" style="text-align: right;" 
				class="form-txt" <@s.if test='"view".equals(mode)'>disabled='true'</@s.if> />
			</div>
			<div class='clearLine'>&nbsp;</div>
			 <div class="grid_3 lable-right">
				<label class="form-lbl">工商执照：</label>
			</div>
			<div class="grid_4">
				<input type="text" name="location.businessLicense" maxlength="20" class="form-txt dialogtip"
				value="${(location.businessLicense)! }" <@s.if test='"view".equals(mode)'>disabled='true'</@s.if> />
			</div>
			...
			<div class="grid_4">
				<input type="text" name="location.enterpriseTelephone" value="${(location.enterpriseTelephone)! }" maxlength="15"
			...
			<div class="grid_4">
				<input type="text" name="location.fax" value="${(location.fax)! }" maxlength="15" class="form-txt dialogtip"
			...
			<div class="grid_20">
				<input type="text" name="location.hiddenCases"
					<@s.if test='"view".equals(mode)'>disabled='true'</@s.if>
			...
			<div class="grid_20">
				<input type="text" name="location.hiddenRectification"
					<@s.if test='"view".equals(mode)'>disabled='true'</@s.if>
