...
public class ExcelImportHelper {
	...
	public static Object newProcImportDate(String[][] beanDatas,
		...
		for (int i = 0; i < length; i++) {
			...
			if (!validateDateDetail(beanDatas, cellValues, i, object, falg,
					mapMessages)) {
				continue;
			}
			...
			if (!validateStringToInteger(beanDatas, cellValues, i, object, falg,
					mapMessages)) {
				continue;
			}
	...
	private static boolean validateOrg(String[][] beanDatas,
		...
		if (DataType.DATA_TYPE_ORG.equals(beanDatas[1][i])) {
			...
			if ((null == org || null == org.getId())&&!object.getClass().equals(GridTeam.class)) {
				mapMessages.put(String.valueOf(i + 1), beanDatas[2][i] + "不正确");
				return false;
			}
			//如果导入的为网格员,根据网格名称未查到组织机构,则视为自定义的网格,待优化yangfan
			/*if (object.getClass().equals(GridTeam.class)
					&& (null != org)) {
				setProperty(object, beanDatas[0][i],
						uploadOrganization);
				return false;
			}else */
			if(object.getClass().equals(GridTeam.class)&&null == org){
				setProperty(object, "userDefinedGridName",
						cellValues[i]);
				setProperty(object, beanDatas[0][i],
						uploadOrganization);
				return false;
	...
	private static boolean validateDateDetail(String[][] beanDatas,
			String[] cellValues, int i, Object object, Boolean falg,
			Map<String, String> mapMessages) {
		if (DataType.DATA_TYPE_DETAIL_DATE.equals(beanDatas[1][i])) {
			falg = true;
			try {
				if (validateHelper.illegalDateDetail(cellValues[i])) {
					mapMessages.put(String.valueOf(i + 1), beanDatas[2][i]
							+ "输入格式不正确");
					return false;
				}
			...
			setProperty(object, beanDatas[0][i],
					convertHelper.convertToDateDetail(cellValues[i]));
			return false;
		}
		return true;
	}
	...
	private static boolean validateStringToInteger(String[][] beanDatas,
			String[] cellValues, int i, Object object, boolean falg,
			Map<String, String> mapMessages) {
		if (DataType.DATA_TYPE_STRING_TO_INTEGER.equals(beanDatas[1][i])) {
			falg = true;
			Integer yesOrNo=null;
			if(cellValues[i].equals("是")){
				yesOrNo=1;
			}else if(cellValues[i].equals("否")){
				yesOrNo=0;
			}else {
				mapMessages.put(String.valueOf(i + 1), beanDatas[2][i]
						+ "输入格式不正确");
			...
			setProperty(object, beanDatas[0][i],yesOrNo);
			return false;
		}
		return true;
