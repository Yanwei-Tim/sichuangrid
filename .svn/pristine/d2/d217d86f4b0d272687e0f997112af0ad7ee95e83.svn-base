...
/**
 * @Description:生成实口的领导视图历史月份job每个月跑一次并且生成后的数据存入缓存表里面（县级以上的数据）
 * @author zhangyouwei@hztianque.com
 * @date: 2015-4-7 下午04:30:55
 */
@Component("actualPopulationLeaderViewSummaryCountDispatch")
public class ActualPopulationLeaderViewSummaryCountDispatch implements
		Serializable {
	private static Logger logger = LoggerFactory
			.getLogger(ActualPopulationLeaderViewSummaryCountDispatch.class);
	@Autowired
	...
	public void createActualPopulationLeaderViewSummaryData() {
		...
		try {
			logger.error("生成实口的领导视图历史月份job开始执行");
			long startTime = System.currentTimeMillis();
			if (AnalyseUtilNew.IS_NEWANALYSE_JOB) {
				leaderViewNewService
						.createActualPopulationLeaderViewSummaryData();
			} else {
				leaderViewService.createActualPopulationLeaderViewSummaryData();
			}
			jobMonitorService.updateJobMonitor(id,
					"花了" + (System.currentTimeMillis() - startTime)
							+ "执行生成实口的领导视图历史月份job", true);
			logger.error("生成实口的领导视图历史月份job执行完成");
		} catch (Exception e) {
			logger.error("生成实口的领导视图历史月份job出现异常：", e);
			jobMonitorService.updateJobMonitor(id, "执行生成实口的领导视图历史月份job出现异常："
					+ e.getMessage(), false);
		}
	}
}
