...
//添加成员的监听事件
$("#supervisorList").delegate(".addMemberForObject","click",function(event){
...
var data = $("#supervisorList").getRowData(memberBaseId);
	...
	$.ajax({
		...
		url:"${path}/plugin/serviceTeam/router/routerManage/addObjectAndMemberRelation.action",
	   	data:{
			...
			"serviceMemberWithObject.objectType":$("#_populationType_").val(),
			"serviceMemberWithObject.objectName":$(personData.name).text(),
		...
		success:function(responseData){
			...
			$("#supervisorList").delRowData(memberBaseId);
