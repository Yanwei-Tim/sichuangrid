	...
	var fitColumns=true;
		var columns = [
			{name:"orgname",caption:"区域",mode:"string"},
				    ...
				    {name:"general",caption:"涉暴恐",width:60,mode:"string",children:[
...
function onOrgChanged(orgId){
	...
	$.ajax({
		dataType:"json",
		url:'${path }/plugin/taskListManage/common/getHiddenDangerVisitList.action?orgId='+orgId+'&searchType='+$("#searchType").val()+'&year='+$("#year").val()+'&month='+$("#month").val()+'&week='+$("#week").val()+'&searchYear='+$("#yearVal").val(),
...
$(document).ready(function(){
	...
	$("#export").click(function(){
		var url = '${path }/plugin/taskListManage/common/downloadHiddenDanger.action?orgId='+getCurrentOrgId()+'&searchType='+$("#searchType").val()+'&year='+$("#year").val()+'&month='+$("#month").val()+'&week='+$("#week").val()+'&searchYear='+$("#yearVal").val();
		downloadFile(url);
	});
