...
<div class="center-right">
	<div class="netConferenceContent">
		<div class="ui-corner-all" id="nav">
			...
			<div style="display:none;" >
				...
				<input id=currentUserId class=req type=text value="<s:property value="@com.tianque.core.util.ThreadVariable@getUser().getId()"/>">
		...
		<!-- <div class="center-content-right" style="width:49%;float:right">
			已选择用户：
			<div>
				<ul id="selectedUser" >
...
function operationFormatter(el, options, rowData){
	return '<a href="javascript:;" data-username="'+rowData.userName+'" data-name="'+rowData.name+'" data-userid="'+rowData.id+'" data-rowid="'+options.rowId+'" class="mettingOn"><span>发起视频</span></a>';
}
$(document).ready(function(){
	$("#sendInvite").click(metting);
	var centerHeight=$("div.ui-layout-center").outerHeight();
	$(".orgTree").height(centerHeight-30);
	...
	$("#userList").jqGridFunction({
		...
		postData: {
	   	...
	   	colModel:[
	        ...
	        {name:"userName",index:'userName',sortable:true,label:'用户名'},
      		{name:"name",index:"name",sortable:true,label:'用户姓名'},
	        //{name:'workCompany',label:'工作单位',sortable:false,width:180,hidden:true},
	        ...
	        //</s:if>
	        {name:"isOnLine",label:'在线状态',sortable:false,align:'center'}
		],
		shrinkToFit:true,
		multiselect:true,
	...
	/*
	function oneMetting(userName,name,id,rowId){
	if(userName==$('#currentUserId').val()){
		$.messageBox({"message":"无法邀请自己！","level":'warn'});
		return;
	}
	regMeetAcc($("#currentUserAcc").val(),$("#currentUserName").val());
	regMeetAcc(userName,name);
	$("#conventionersAcc").val(userName);
	$("#conventionersName").val(name);
	$("#conventionerId").val(id);
	inviteMeet();
}
	$("#userList").delegate(".mettingOn","click",function(){
		metting();
	})*/
	function show(){
...
function afterLoad(){
	getAllSessions();
	userArr = $("#userList").getDataIDs();
	for(var i=0;i<sessionArr.length;i++){
		for(var j=0;j<userArr.length;j++){
			if(userArr[j]==sessionArr[i].userId){
				$("#userList").setCell(userArr[j],'isOnLine','<a href="javascript:;" class="on-line">在线</a>');
			}else{
				$("#userList").setCell(userArr[j],'isOnLine','<a href="javascript:;" class="off-line">离线</a>');
			}
		}
	}
	//setUserOperateButton();
...
function getAllSessions(){
	$.ajax({
		url:'${path}/sessionManage/findAllSessions.action',
		success:function(data){
			sessionArr = data;
		}
	});
