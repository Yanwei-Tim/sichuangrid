...
<div class="btnbanner">
	<div class="ui-widget autosearch">
		<input type="text" value="请输入社会组织名称" class="searchtxt" id="searchByPlaceName" maxlength="18" onfocus="if(this.value == '请输入社会组织名称') this.value = '';" onblur="if(this.value == '') this.value = '请输入社会组织名称';">
...
var title = "社会组织";	
		jQuery(document).ready(function() {		
			...
			$("#searchHighA").click(function(){
				$("#statisticsDialog").createDialog({
					...
					modal:true,
					url: '${path}/baseinfo/siteinfo/newSocietyFederation/searchNewSocietyFederationDlg.jsp',
					buttons: {
						"查询" : function(event){
			...
			$("#searchByIdcardA").click(function(){
				if($("#searchByPlaceName").val()=="请输入社会组织名称"){
					$.notice({
						level: 'alert',
					    okbtnName: "确定",
					    title:'提示',
			...
			$("#refreshOrgTree1").click(function(){
				...
				$("#searchByPlaceName").attr("value","请输入社会组织名称");
			});
		});
		function getDialogParam(){
			return {
				...
				title: title+'信息',
				modal:true,
				url: '${path}/baseinfo/siteinfo/newSocietyFederation/newSocietyFederationListForStatistics.jsp?isSearch=1',
				buttons: {
			   		"关闭" : function(){
			        	$(this).dialog("close");
			   		}
		...
		function onOrgChanged(){
			...
			$("#areaDatatable").setPostData({
		    	...
		    	"tableName":"newSocietyFederations"
		   	});
