...
<%@ include file="/includes/baseInclude.jsp"%>
	...
	<input type="hidden" id="dangerousPersonKeyType" name="" value="<s:property value="#parameters.dangerousPersonKeyType"/>"/>
...
<script type="text/javascript">
	var dangerousPersonKeyType=$("#dangerousPersonKeyType").val();
	...
	function loadList(){
		...
		$("#helpPersonnelList").setPostData({
			"personnelType":dangerousPersonKeyType,
	...
	$(document).ready(function(){
		...
		if(isHelpPersonnel()){
			$("#dangerousPeopleHelpPrecord").show();
		}else{
			$("#dangerousPeopleHelpPrecord").hide();
		}
		$("#helpPersonnelList").jqGridFunction({
			...
			colModel:[
				...
				{name:"identity", sortable:false, label: '综治人员身份'},
				{name:"mobile", sortable:false, label:'联系手机',width:90},
				{name:"telephone", sortable:false, label: '联系电话',width:110},
			...
			ondblClickRow: viewdangerousPeopleHelp,
		...
		$("#addHelp").click(function(){
			...
			$("#helpPersonnelDialog").createDialog({
				...
				height: 250,
				title:'新增帮教人员信息',                                                                                                    
				url:'${path}/baseinfo/helpPersonnel/dispatch.action?personnelId='+domainId+'&personnelType=<s:property value="#parameters.dangerousPersonKeyType"/>&mode=add&personType=dangerousPeopleHelp',
				buttons: {                                                                   
					"保存" : function(event){
		...
		$("#updateHelp").click(function(){
			...
			$("#helpPersonnelDialog").createDialog({
				...
				height: 250,
				title:'修改综治负责人信息',
				url:'${path}/baseinfo/helpPersonnel/dispatch.action?helpPersonnel.id='+id[0]+'&mode=edit&personType=dangerousPeopleHelp',
				buttons: {
					"保存" : function(event){
				      $("#helpPersonnelForm").submit();
				   },
		...
		$("#logoutHelp").click(function(){
			...
			$.confirm({
				title:"确认注销",
				message:"该综治负责人信息注销后，将无法恢复,您确认注销该人员信息吗?",
				width: 300,
				okFunc: function(){
				$.ajax({
	...
	function viewdangerousPeopleHelp(){
		var selectedId = $("#helpPersonnelList").getGridParam("selrow");
		if(selectedId==null){
		  return;
		}
		...
		$("#helpPersonnelDialog").createDialog({
			...
			height: 250,
			title:'查看综治负责人信息',
			url:'${path}/baseinfo/helpPersonnel/dispatch.action?helpPersonnel.id='+id[0]+'&mode=view&personType=dangerousPeopleHelp',
			buttons: {
			   "关闭" : function(){
			        $(this).dialog("close");
			   }
	...
	function isHelpPersonnel(){
		...
		$.ajax({
			...
			data:{
				...
				"personnelType":$("#dangerousPersonKeyType").val()
			},				
			success:function(responseData){
