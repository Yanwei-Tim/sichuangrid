...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#helpPrecordList").jqGridFunction({
			...
			colModel:[
				...
				{name:"helpTime",sortable:false ,index:"floorpersonsTime", label: '管理时间',width:100},
				{name:"address", sortable:false, label:'管理地点', width:100},
				{name:"helpPersonnelName", sortable:false, label: '管理人员',width:200},
				{name:"events", sortable:false, label: '管理情况'}
			],
		...
		$("#addHelpPrecord").click(function(){
			...
			$("#helpPrecordDialog").createDialog({
				...
				height: 400,
				title:'新增管理情况',
				url:'${path}/baseinfo/helpPrecord/dispatch.action?personnelId='+domainId+'&personnelType=<s:property value="#parameters.keyType"/>&mode=add&precordType=management',
				buttons: {
					"保存" : function(event){
		...
		$("#updateHelpPrecord").click(function(){
			...
			$("#helpPrecordDialog").createDialog({
				...
				height: 400,
				title:'修改管理情况',
				url:'${path}/baseinfo/helpPrecord/dispatch.action?helpPrecord.id='+selectedId+'&mode=edit&precordType=management',
				buttons: {
					"保存" : function(event){
				      $("#helpPrecordForm").submit();
				   },
		...
		$("#deleteHelpPrecord").click(function(){
			...
			$.confirm({
				title:"确认删除",
				message:"该管理情况信息删除后，将无法恢复,您确认删除该管理情况吗?",
				width: 300,
	...
	function viewHelpPrecord(){
		...
		$("#helpPrecordDialog").createDialog({
			...
			height: 400,
			title:'查看管理情况',
			url:'${path}/baseinfo/helpPrecord/dispatch.action?helpPrecord.id='+selectedId+'&mode=view&precordType=management',
			buttons: {
			   "关闭" : function(){
			        $(this).dialog("close");
			   }
			...
			success:function(responseData){
				if(responseData){
					if(responseData == true){
						...
						$.messageBox({message:"管理情况删除成功！"});
						disableHelpPrecordButton();
					}else{
							$.messageBox({message:"管理情况删除失败！"});
						}
				}
			}
		});
