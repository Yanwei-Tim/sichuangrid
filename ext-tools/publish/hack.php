...
function stopMobileServer($serviceName,$service){
	global $mode,$antbin,$logDir,$afterStopSleepSeconds ;
	$command ="ssh admin@{$service['serverName']}  ".$antbin. 
		" -buildfile {$service['baseDir']}build.xml  stopJetty -Dlistenerport={$service['listenerPort']} ";
	echo "准备停止服务:{$serviceName}\n";
	echo $command."\n";
	system($command);
	sleep($afterStopSleepSeconds);
...
function stopAnaServer($serviceName,$service){
	global $mode,$antbin,$logDir,$afterStopSleepSeconds ;
	$command ="ssh admin@{$service['serverName']}  ".$antbin. 
		" -buildfile {$service['baseDir']}production.build.xml  stopJetty -Dlistenerport={$service['listenerPort']} ";
	echo "准备停止服务:{$serviceName}\n";
...
function stopCoreServer($serviceName,$service){
	global $mode,$antbin,$logDir,$afterStopSleepSeconds ;
	$command ="ssh admin@{$service['serverName']}  ".$antbin. 
		" -buildfile {$service['baseDir']}production.build.xml  stopJetty -Dlistenerport={$service['listenerPort']} ";
	echo "准备停止服务:{$serviceName}\n";
...
function startMobileServer($serviceName,$service){
	global $mode,$antbin,$logDir;
	$command = "ssh admin@{$service['serverName']}   ".$antbin. 
		" -buildfile {$service['baseDir']}build.xml  startJetty -Dport={$service['port']} -Dlistenerport={$service['listenerPort']} -Dpath=/ -Drootdir=./webroot ";
	echo "准备启动服务:{$serviceName}\n";
	echo $command."\n";
	system($command);
}  

function startAnaServer($serviceName,$service){
	global $mode,$antbin,$logDir;
	$command = "ssh admin@{$service['serverName']}   ".$antbin. 
		" -buildfile {$service['baseDir']}production.build.xml  startJetty -Dport={$service['port']} -Dlistenerport={$service['listenerPort']} -Dpath=/ -Drootdir=./webroot ";
	echo "准备启动服务:{$serviceName}\n";
...
function startCoreServer($serviceName,$service){
	global $mode,$antbin,$logDir;
	$command = "ssh admin@{$service['serverName']}   ".$antbin. 
		" -buildfile {$service['baseDir']}production.build.xml  startJetty -Dport={$service['port']} -Dlistenerport={$service['listenerPort']} -Dpath=/ -Drootdir=./webroot ";
	echo "准备启动服务:{$serviceName}\n";
...
function stopCache($serviceName,$service){
	$command ="ssh admin@{$service['serverName']} /home/admin/wzmem/wzmem.sh stop";
	echo $command."\n";
	system($command);
}
...
function startCache($serviceName,$service){
	$command ="ssh admin@{$service['serverName']} /home/admin/wzmem/wzmem.sh start";
	echo $command."\n";
	system($command);
}
...
function restartCache($serviceName,$service){
	$command ="ssh admin@{$service['serverName']} /home/admin/wzmem/wzmem.sh restart";
	echo $command."\n";
	system($command);
}
