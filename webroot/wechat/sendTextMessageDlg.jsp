	...
	<form id="maintainForm" method="post" action="">
     ...
     <pop:token></pop:token>
		...
		<div id="content">
			...
			<div class="grid_20 heightAuto">
			    <s:if test="#request.sendOrReply=='retransmission'">
			       <textarea  id="textareaContent" name="textMessage.content"  readonly style='height:70px;width: 97%' class='form-txt'>${ inbox.content}</textarea>
		    	</s:if>
		        <s:else>
...
$(document).ready(function(){
	    <s:if test='"replyTextMessage".equals(mode)'>
	       $("#maintainForm").attr("action","/weChat/inbox/replyTextMessage.action");
		</s:if>
		<s:else>
		 $("#maintainForm").attr("action","/weChat/inbox/retransmissionTextMessage.action");
