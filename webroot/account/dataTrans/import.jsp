...
<form id="mForm" method="post" enctype="multipart/form-data" action="${path}/threeRecords/dataTransManage/importToDomain.action">
	<pop:token />
	...
	<div id="upload-div" class="container container_24">
		<div class="grid_24 heightAuto">
		    <label class="form-lbl heightAuto" id="templateVersion">
		            ...
		            <font color="red"><s:property value="@com.tianque.threeRecordsIssue.dataTrans.ImportTemplatesSetting@getImportTemplatesVo(#parameters.templates).version"/></font>
		                               版本，如数据模板版本不正确，您可以下载
                <a href='${path}/threeRecords/dataTransManage/downloadDataTemplate.action?dataTemplatesName=
                <s:property value="@com.tianque.threeRecordsIssue.dataTrans.ImportTemplatesSetting@getImportTemplatesVo(#parameters.templates).url"/>&sid=
                <s:property value="@com.tianque.core.util.ThreadVariable@getSession().getSessionId()"/>'><b><font color="dimgray"><i>最新数据模板</i></font></b></a>
            </label>
		</div>
		<div id="importBox">
		...
		submitHandler: function(form){
			$(form).ajaxSubmit({
				...
				success: function(data){
					...
					var str = "<div id='load' class='ui-widget-border'>" +
                            "<div style='text-align:left;margin-left:30px;margin-top:20px;margin-bottom:-70px;margin-right:30px;' " +
                            "id='templateVersionLabel'>"+$("#templateVersion").html()+"</div>" +
                            "<p class='filename'>"+dataJson.uploadFileName+"</p><p id='userTicketText'>您前面有<b id='userTicketNumber' style='color:red;'>"+userTicketNumber+"</b>位用户正在导入，请稍候...</p>" +
                            "<p id='progressbar-msg'>文件正在上传...</p><p id='currentText'>您选择导入<b id='totalCount' style='color:red;'></b>条数据，正在处理第<b id='currentNumber' style='color:red;'>1</b>条数据</p>" +
                            "<div id='progressbar'></div><span id='progressbar-value'>0%</span><div class='clear'>&nbsp;</div>";
					...
					str += "<div class='ui-dialog-buttonset'><div id='errorMsgButton' style='float:left'><button id='reloadExcel' type='button' class='ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only'>重新上传</button>" +
                    "<button id='cancel' type='button' class='ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only'>关闭</button></div>" +
                    "<div style='float:left' id='closeImportDiv'><button id='closeImportDialog' type='button' class='ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only'>后台处理</button></div>";
					str +="</div>";
					$("body").mask(str);
...
function getTicketByIds(){
	 $.ajax({
		...
		data:{
			...
			var serverMsg = '';
			if(data.description){
				serverMsg = "<div><textarea rows='10' cols='70' readonly='readonly'>"+data.description+"</textarea></div>";
			}
			if(state=='<s:property value="@com.tianque.state.TicketState@DONE"/>' ){
				...
				var succHtml = "<div id='load' class='ui-widget-border'>"+
								"<div class='ui-widget' style='margin-top:30px;'>"+
									"<div class='ui-corner-all ui-widget-border' style='width:400px;margin-top:5px;text-align:left;margin-left:5px;'><em class='ui-icon ui-icon-circle-check' style='float: left; margin-right: .3em;'></em>导入成功.</div>"+serverMsg+
								"</div>"+
								"<div class='clear'></div>"+
...
function showErrorMsgButton(){
    ...
    $('#reloadExcel').bind('click',function(){
        ...
        $("#<s:property value='#parameters.dialog'/>").createDialog({
            ...
            height: 220,
            title:'数据导入',
            url:"${path}/account/dataTrans/import.jsp?isNew=<s:property value='#parameters.isNew'/>&dataType=<s:property value='#parameters.dataType'/>&dialog=<s:property value='#parameters.dialog'/>&startRow=<s:property value='#parameters.startRow'/>&templates=<s:property value='#parameters.templates'/>&isTownLeaderImport=<s:property value='#parameters.isTownLeaderImport'/>",
            buttons:{
                "重新导入" : function(event){
                      clearInterval(intarcalId);
                      threadStop();
...
function threadStop(){
	$.ajax({
		url: '${path}/threeRecords/dataTransManage/threadStop.action',
