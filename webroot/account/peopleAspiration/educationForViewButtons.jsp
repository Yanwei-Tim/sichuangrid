...
<pop:JugePermissionTag ename="addEducation">
	<a id="add" href="javascript:;"><span>新增</span></a>
</pop:JugePermissionTag> <pop:JugePermissionTag ename="updateEducation">
	<a id="update" href="javascript:void(0)"><span>修改</span></a>
</pop:JugePermissionTag> <pop:JugePermissionTag ename="deleteEducation">
...
$(function(){
	...
	$("#add").click(function(event){
		$("#issueDialog").createDialog({
			...
			height:570,
			title:'新增教育信息',
			url:'${path}/threeRecords/education/dispatch.action?mode=add&keyId='+$("#currentOrgId").val(),
			buttons: {
	        	"上一步": function (event) {
	...
	$("#delete").click(function(event){
		if (hasRowSelected()){
			$.confirm({
				title:"确认删除",
				message:"该教育处理信息删除后,将无法恢复,您确认删除该教育处理信息吗?",
				width:400,
				okFunc:function(){
					//deleteEducation(getSelectedData().encryptIdByIssueId);
					var strs = getSelectedIds();
                    	for (i=0;i<strs.length ;i++) {
                    		deleteEducation(getData(strs[i]).encryptIdByIssueId);
                    	}
				}
			});
		}else{
	...
	$("#search").click(function(event){
		$("#issueDialog").createDialog({
			...
			height: 400,
			title: "教育查询-请输入查询条件",
			url: "${path}/threeRecords/education/dispatch.action?mode=search&keyId="+$("#currentOrgId").val()+'&tag='+viewType.value+"&selectedIssueType=",
			buttons: {
		   		"查询" : function(event){
	...
	$("#import").click(function(event){
        $("#educationDialog").createDialog({
            width: 400,
            height: 230,
            title:'数据导入',
            url:PATH+'/account/dataTrans/import.jsp?isNew=1&dataType=EducationData&dialog=educationDialog&startRow=1&templates=EDUCATIONDATA&listName=educationForViewList',
            buttons:{
                "导入" : function(event){
                    $("#mForm").submit();
                },
	...
	$("#printIssue").click(function(event){
		...
		$("#issueDialog").createDialog({
			...
			height: 600,
			title: '打印教育信息',
			url:'${path}/threeRecords/education/dispatch.action?mode=print&keyId=' + selectedIssue.encryptIdByIssueId,
			buttons: {
		   		"打印" : function(event){
					$("#educationPrint").printArea();
	   			},
		   		"关闭" : function(){
		        	$(this).dialog("close");
		   		}
    ...
    $("#exportIssue").click(function (event) {
        ...
        var selectedIssue = getSelectedData();
        var url='${path}/threeRecords/education/exportExcel.action?keyId=' + selectedIssue.encryptIdByIssueId;
	...
	$("#conceptTop,#conceptBottom").click(function(event){
		...
		var currentOrgId='${loginAction.user.organization.id}';
		 if(currentOrgId !=selectedIssue['targetOrg.id']){
       	 $.messageBox({level: 'warn', message: "该教育不可受理，请进行其它操作！"}); 
       	 return;
       }
	...
	//由于该页面是动态的，使用click会重复绑定教育,所有使用以下方法
	$("#dealTop,#dealBottom").die().live("click",function(event){
		...
		if(selectedIssue.dealState!=null && '办理中' == selectedIssue.dealState){
			$.ajax({
	            url: "/threeRecords/education/existEducation.action",
	            data: {
	                "keyId": selectedIssue.issueId
	            },
	            success: function (responseData) {
	                if (responseData==null||responseData.id==null) {
	                    $.messageBox({level: 'warn',message: "该条教育类信息不可办理，请完善研究管理信息内容！"});
	                } else {
			...
			$.messageBox({level:'warn',message:"该条教育类信息不可办理，请先受理后再作办理操作！"});
		}
	});
	$("#reportToTop,#reportToBottom").click(function(event){
		...
		if(selectedIssue.dealState!=null){
			...
			$.messageBox({level:'warn',message:"该教育不可 上报指挥中心，请进行其它操作！"});
		}
	});
...
            searchVo[data[i].name] = data[i].value;
	...
	var pageData = $.extend(
			{
			"searchVo.issueType":<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_EDUCATION"/>,	
...
function printAcceptForm(id){
	$("#issueDialog").createDialog({
	    ...
	    height: 400,
	    title: "受理单",
	    url: '${path}/threeRecords/acceptForm/dispatch.action?mode=add&keyId='+id+'&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_EDUCATION"/>',
	    ajaxType:'post',
	    buttons: {
	        "保存打印": function (event) {
	            $("#accept_form").submit();
...
function editThreeRecords(id){
	$("#issueDialog").createDialog({
		...
		height:570,
		title: '修改教育信息',
		url:'/threeRecords/education/dispatch.action?mode=edit&keyId='+id,
		buttons: {
	    	"上一步": function (event) {
	        	$(".ui-dialog-buttonset  button").eq(0).hide();
	        	$('#tabs-option').tabs('select',0);
...
function deleteEducation(keyId){
	$.ajax({
		url:"/threeRecords/education/deleteEducation.action",
		data:{
			"keyId":keyId
		},
		success:function(responseData){
			if (responseData==true){
				$.messageBox({message:"已经成功删除该教育处理信息!"});
				//reloadIssue();
				searchIsssueById();
				getMessageByUser();
			}else{
				$.messageBox({level:'error',message:"删除教育失败!"});
			}
		}
	});
}
...
$("#feedBack").click(function(event){
	...
	$.post('${path}/threeRecords/feedBack/isCanFeedBack.action?&ledgerId='+ selectedIssue.issueId+'&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_EDUCATION"/>', function (data) {
		if(!data){
			$.messageBox({level:'warn',message: "该教育不可反馈，请进行其它操作！"});
			return;
		}
		$("#feedBackDialog").createDialog({
			...
			height: 400,
			title: "反馈",
			url: '${path}/threeRecords/feedBack/dispatch.action?keyId=' + selectedIssue.issueStepId+'&ledgerId='+ selectedIssue.issueId+'&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_EDUCATION"/>',
			buttons: {
		   		"反馈" : function(event){
...
$("#replyFormTop").click(function (event) {
    ...
    var selectedIssue = getSelectedData();
     $("#feedBackDialog").createDialog({
         ...
         height: 400,
         title: "回复单",
         url: '${path}/threeRecords/replyForm/dispatch.action?keyId=' + selectedIssue.issueStepId+'&serialNumber='+ selectedIssue.serialNumber + '&ledgerId=' + selectedIssue.issueId + '&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_EDUCATION"/>',
         buttons: {
             "回复": function (event) {
