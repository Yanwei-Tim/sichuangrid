...
<div class="content">
	<jsp:include page="${path}/account/peopleAspiration/scienceForViewButtons.jsp"/>
...
function initIssueCount(){
	$.ajax({
        url: "/threeRecords/science/getIssueCount.action",
        data: {
...
function viewIssue(rowid){
	...
	//if (hasRowSelected() && $("#view").isButtonEnabled()){
		...
		$("#issueDialog").createDialog({
			...
			height: 500,
			title: "查看科技文体信息详情",
			url: "${path}/threeRecords/science/dispatch.action?mode=viewNew&keyId="+selectedIssue.encryptIdByIssueId,
			buttons: {
		   		"关闭" : function(){
		        	$(this).dialog("close");
		   		}
...
function onOrgChanged(isEvaluate){
	...
	$("#issueList").setPostData({
		...
		"fourTeamsType":"<s:property value='@com.tianque.domain.property.FourTeamsType@GRIDSERVICETEAM'/>",
