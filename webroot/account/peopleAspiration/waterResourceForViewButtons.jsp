...
<c:if test="${!isFun}">
    <div class="btnbanner btnbannerData" style="float: left">
        ...
        <!-- <div class="userState" id="fastSearchSelect"> -->
	        <select onchange="onOrgChanged()" id="seachValue" class="form-txt"
	                style="width: 100px;">
...
<div class="ui-corner-all nav-buttons" id="nav" style="float: left">
    <pop:JugePermissionTag ename="addWaterResource">
        <a id="add" href="javascript:;"><span>新增</span></a>
    </pop:JugePermissionTag>
    <pop:JugePermissionTag ename="updateWaterResource">
        <a id="update" href="javascript:void(0)"><span>修改</span></a>
    </pop:JugePermissionTag>
    <pop:JugePermissionTag ename="deleteWaterResource">
...
<input type="hidden" name="selectnodeType" id="selectnodeType"/></div>
...
<script type="text/javascript">
    ...
    var issueButtons = {
        ...
        myneed: function () {//我的待办
            ...
            this.showButtons(["修改",<c:if test='${isShowDeleted}'> "删除",</c:if> "查询","导入", "查看", "刷新", "打印","导出", "受理", "办理","回复"]);
            </s:if>
            <s:else>
            this.showButtons(["新增", "修改",<c:if test='${isShowDeleted}'> "删除",</c:if> "查询","导入", "查看", "刷新", "打印", "导出","受理", "办理","回复"]);
            </s:else>
        },
    ...
    $(function () {
        ...
        $("#add").click(function (event) {
            $("#issueDialog").createDialog({
                ...
                height: 570,
                title: '新增水利信息',
                url: '${path}/threeRecords/water/dispatch.action?mode=add&keyId=' + $("#currentOrgId").val(),
                buttons: {
                	"上一步": function (event) {
        ...
        $("#delete").click(function (event) {
            if (hasRowSelected()) {
                $.confirm({
                    title: "确认删除",
                    message: "该水利台账处理信息删除后,将无法恢复,您确认删除该水利台账处理信息吗?",
                    width: 400,
                    okFunc: function () {
                    	var strs = getSelectedIds();
                    	for (i=0;i<strs.length ;i++) {
                    		removeWaterAndStep(getData(strs[i]).encryptIdByIssueId);
                    	}
                        //removeWaterAndStep(getSelectedData().encryptIdByIssueId);
                    }
                });
            } else {
        ...
        $("#search").click(function (event) {
            ...
            $("#issueDialog").createDialog({
                ...
                height: 400,
                title: "水利查询-请输入查询条件",
                url: "${path}/threeRecords/water/dispatch.action?mode=search&keyId=" + $("#currentOrgId").val() + '&tag=' + viewType.value + "&selectedIssueType=",
                buttons: {
                    "查询": function (event) {
        ...
        $("#import").click(function(event){
            $("#waterResourceDialog").createDialog({
                width: 400,
                height: 230,
                title:'数据导入',
                url:PATH+'/account/dataTrans/import.jsp?isNew=1&dataType=WaterResourceData&dialog=waterResourceDialog&startRow=1&templates=WATERRESOURCEDATA&listName=waterResourceForViewList',
                buttons:{
                    "导入" : function(event){
                        $("#mForm").submit();
                    },
        ...
        $("#printIssue").click(function (event) {
            ...
            $("#issueDialog").createDialog({
                ...
                height: 600,
                title: '水利信息打印',
                url: '${path}/threeRecords/water/dispatch.action?mode=print&keyId=' + selectedIssue.encryptIdByIssueId,
                buttons: {
                    "打印": function (event) {
                        $("#waterPrint").printArea();
                    },
                    "关闭": function () {
                        $(this).dialog("close");
                    }
        ...
        $("#exportIssue").click(function (event) {
            ...
            var selectedIssue = getSelectedData();
            var url='${path}/threeRecords/water/exportExcel.action?keyId=' + selectedIssue.encryptIdByIssueId;
        ...
        $("#readTop,#readBottom").click(function (event) {
            ...
            if (selectedIssue.dealState != null && "待阅读" == selectedIssue.dealState) {
                simpleDealIssue(selectedIssue.encryptIdByIssueStepId, <s:property value="@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@READ.code"/>);
            } else {
                $.messageBox({level: 'warn', message: "该水利台账不可阅读，请进行其它操作！"});
            }
        });
        $("#conceptTop,#conceptBottom").click(function (event) {
            ...
            if(currentOrgId !=selectedIssue['targetOrg.id']){
            	 $.messageBox({level: 'warn', message: "该水利台账不可受理，请进行其它操作！"}); 
            	 return;
            }
        ...
        //由于该页面是动态的，使用click会重复绑定水利台账,所有使用以下方法
        $("#dealTop,#dealBottom").die().live("click", function (event) {
            ...
            if (selectedIssue.dealState != null && '办理中' == selectedIssue.dealState) {
			    $.ajax({
		            url: "/threeRecords/water/existWaterResource.action",
		            data: {
		                "keyId": selectedIssue.issueId
		            },
		            success: function (responseData) {
		                if (responseData==null||responseData.id==null) {
		                    $.messageBox({level: 'warn',message: "该条水利类信息不可办理，请完善研究管理信息内容！"});
		                } else {
                ...
                $.messageBox({level: 'warn', message: "该条水利类信息不可办理，请先受理后再作办理操作！"});
            }
        });
    ...
    function searchIssue() {
        ...
        var pageData = $.extend(
                {
                    "searchVo.issueType":<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_WATER"/>,
    ...
    function printAcceptForm(id){
    	$("#issueDialog").createDialog({
    	    ...
    	    height: 400,
    	    title: "受理单",
    	    url: '${path}/threeRecords/acceptForm/dispatch.action?mode=add&keyId='+id+'&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_WATER"/>',
    	    ajaxType:'post',
    	    buttons: {
    	        "保存打印": function (event) {
    	            $("#accept_form").submit();
    ...
    function editThreeRecords(id) {
        $("#issueDialog").createDialog({
            ...
            height: 570,
            title: '修改水利信息',
            url: '/threeRecords/water/dispatch.action?mode=edit&keyId=' + id,
            buttons: {
	        	"上一步": function (event) {
		        	$(".ui-dialog-buttonset  button").eq(0).hide();
		        	$('#tabs-option').tabs('select',0);
    ...
    function removeWaterAndStep(keyId) {
        $.ajax({
            url: "/threeRecords/water/deleteWaterSource.action",
            data: {
                "keyId": keyId
            },
            success: function (responseData) {
                if (responseData == true) {
                    $.messageBox({message: "已经成功删除该水利台账处理信息!"});
                   // reloadIssue();
                   searchIsssueById();
                    getMessageByUser();
                } else {
                    $.messageBox({level: 'error', message: "删除水利台账失败!"});
                }
            }
        });
    }
    ...
    $("#feedBack").click(function (event) {
        ...
        $.post('${path}/threeRecords/feedBack/isCanFeedBack.action?ledgerId=' + selectedIssue.issueId + '&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_WATER"/>', function (data) {
            if (!data) {
                $.messageBox({level: 'warn', message: "该水利不可反馈，请进行其它操作！"});
                return;
            }
            $("#feedBackDialog").createDialog({
                ...
                height: 400,
                title: "反馈",
                url: '${path}/threeRecords/feedBack/dispatch.action?keyId=' + selectedIssue.issueStepId + '&ledgerId=' + selectedIssue.issueId + '&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_WATER"/>',
                buttons: {
                    "反馈": function (event) {
    ...
    $("#replyFormTop").click(function (event) {
        ...
        var selectedIssue = getSelectedData();
         $("#feedBackDialog").createDialog({
             ...
             height: 400,
             title: "回复单",
             url: '${path}/threeRecords/replyForm/dispatch.action?keyId=' + selectedIssue.issueStepId+'&serialNumber='+ selectedIssue.serialNumber + '&ledgerId=' + selectedIssue.issueId + '&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_WATER"/>',
             buttons: {
                 "回复": function (event) {
