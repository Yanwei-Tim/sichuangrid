...
<style type="text/css">
    ...
    #blPage .innerTxt strong{float: left;margin-left: -80px;width:100px;text-align:right; /* position:absolute;top:0; left:0; */}
...
<s:if test="lastStep.dealType == null">
	<input type="hidden" id="lastDealDate" value="<fmt:formatDate value="${peopleAspirations.occurDate}" type="date" pattern="yyyy-MM-dd" />" />
</s:if>
<s:else>
...
<div id="blPage" class="cf">
		...
		<div class="left">
			...
			<tbody>
		        <tr>
					<td ><div class='innerTxt cf'><strong>编号:</strong>${peopleAspirations.serialNumber}</div></td>
				</tr>
				 <tr>
					<td ><div class='innerTxt cf'><strong>登记时间:</strong><s:date format="yyyy-MM-dd" name="peopleAspirations.occurDate" /></div></td>
				</tr>
				 <tr>
					<td ><div class='innerTxt cf'><strong>主要当事人:</strong>${peopleAspirations.name}</div></td>
				</tr>
				 <tr>
					<td ><div class='innerTxt cf'><strong>服务联系人:</strong>${peopleAspirations.serverContractor}${peopleAspirations.serverTelephone}</div></td>
				</tr>
				<tr>
					<td ><div class='innerTxt cf'><strong>单位及职务:</strong>${peopleAspirations.serverJob}</div></td>
				</tr>
				<tr>
					<td ><div class='innerTxt cf'><strong>主要愿望或诉求:</strong>${peopleAspirations.appealContent}</div></td>
				</tr>
				
				<s:if test="peopleAspirations.ledgerType == @com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_WATER">
					<s:action name="getWaterResource" namespace="/threeRecords/water" var="waterResource" executeResult="true">
						<s:param name="keyId" value="peopleAspirations.id"></s:param>
					</s:action>
				</s:if>
				<s:if test="peopleAspirations.ledgerType == @com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_TRAFFIC">
					<s:action name="getTraffic" namespace="/threeRecords/traffic" var="traffic" executeResult="true">
						<s:param name="keyId" value="peopleAspirations.id"></s:param>
					</s:action>
				</s:if>
				<s:if test="peopleAspirations.ledgerType == @com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_TOWN">
					<s:action name="getTown" namespace="/threeRecords/town" var="town" executeResult="true">
						<s:param name="keyId" value="peopleAspirations.id"></s:param>
					</s:action>
				</s:if>
				<s:if test="peopleAspirations.ledgerType == @com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_SCIENCE">
					<s:action name="getScience" namespace="/threeRecords/science" var="science" executeResult="true">
						<s:param name="keyId" value="peopleAspirations.id"></s:param>
					</s:action>
				</s:if>
				<s:if test="peopleAspirations.ledgerType == @com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_OTHER">
					<s:action name="getOther" namespace="/threeRecords/other" var="other" executeResult="true">
						<s:param name="keyId" value="peopleAspirations.id"></s:param>
					</s:action>
				</s:if>
				<s:if test="peopleAspirations.ledgerType == @com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_MEDICAL">
					<s:action name="getMedical" namespace="/threeRecords/medical" var="medical" executeResult="true">
						<s:param name="keyId" value="peopleAspirations.id"></s:param>
					</s:action>
				</s:if>
				<s:if test="peopleAspirations.ledgerType == @com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_LABOR">
					<s:action name="getLabor" namespace="/threeRecords/labor" var="labor" executeResult="true">
						<s:param name="keyId" value="peopleAspirations.id"></s:param>
					</s:action>
				</s:if>
				<s:if test="peopleAspirations.ledgerType == @com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_ENVIRONMENT">
					<s:action name="getEnvironment" namespace="/threeRecords/environment" var="environment" executeResult="true">
						<s:param name="keyId" value="peopleAspirations.id"></s:param>
					</s:action>
				</s:if>
				<s:if test="peopleAspirations.ledgerType == @com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_ENERGY">
					<s:action name="getEnergy" namespace="/threeRecords/energy" var="energy" executeResult="true">
						<s:param name="keyId" value="peopleAspirations.id"></s:param>
					</s:action>
				</s:if>
				<s:if test="peopleAspirations.ledgerType == @com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_EDUCATION">
					<s:action name="getEducation" namespace="/threeRecords/education" var="education" executeResult="true">
						<s:param name="keyId" value="peopleAspirations.id"></s:param>
					</s:action>
				</s:if>
				<s:if test="peopleAspirations.ledgerType == @com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_AGRICULTURE">
					<s:action name="getAgriculture" namespace="/threeRecords/agriculture" var="agriculture" executeResult="true">
						<s:param name="keyId" value="peopleAspirations.id"></s:param>
					</s:action>
				</s:if>
		...
		<div class="right">
			<div id="dialog-form" title="办理台账处理">
				<form id="issueDealForm" method="post" action="${path }/threeRecordsIssues/issueManage/dealIssue.action">
					<pop:token />
					...
					<input type="hidden" name="operation.ledgerId" id="issueId" value="${peopleAspirations.id }" />
					<input type="hidden" name="operation.ledgerType" id="ledgerType" value="${peopleAspirations.ledgerType}" />
					...
					<div class='clearLine'></div>
					<div class="grid_4 lable-right poepleAspiration" id= "typeComplete" >
							<em class="form-req">*</em><label>办结类型：</label>
					</div>
					<div class="grid_8 form-left poepleAspiration" id="poepleAspiration<s:property value='@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@COMPLETE.code'/>" >
			    		<input class="completeTypeText" name="operation.completeType" type="radio" value="<s:property value='@com.tianque.plugin.account.constants.CompleteType@REALCOMPLETE'/>">实质性办结
					...
					<div class="grid_12 form-left poepleAspiration" id="poepleAspiration<s:property value='@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@PROGRAM_COMPLETE.code'/>" >
					...
					<div class="grid_8 form-left poepleAspiration" id="poepleAspiration<s:property value='@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@PERIOD_COMPLETE.code'/>"  >
			    		<input class="completeTypeText" name="operation.completeType" type="radio" value="<s:property value='@com.tianque.plugin.account.constants.CompleteType@IMPLEMENT_PROJECT'/>">落实项目
			    		<c:if test="${interid != 10}">
			    			<input class="completeTypeText" name="operation.completeType" type="radio" value="<s:property value='@com.tianque.plugin.account.constants.CompleteType@INTEGRATE_PROJECT'/>">整合项目
			    		</c:if>
					</div>
					<div class='clearLine'></div>
					...
					<div id="transferToDiv_future" style="display: none;">
...
$(function(){
    ...
    $(".poepleAspiration").each(function(){
		$(this).hide();
	})
    dealTypeChanged();
    ...
    if(end.exec($("#currentDepartNo").val()) && viewType.value=='need'){
    	$("#toCommittee").show();
    }
	...
	$("#issueDealForm").formValidate({
		...
		submitHandler: function(form){
			...
			if( $("#dealCode").val() == "<s:property value='@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@COMPLETE.code'/>"
			    ...
			    || $("#dealCode").val() == <s:property value='@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@PROGRAM_COMPLETE.code'/>){
			   	var checked=false;
				$("#poepleAspiration"+$("#dealCode").val()).find("input:radio[name='operation.completeType']").each(function(){
...
function dealTypeChanged(){
	...
	|| operateCode == <s:property value='@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@PROGRAM_COMPLETE.code'/>){
		...
		$("#poepleAspiration"+operateCode).show();
		$("#typeComplete").show();
