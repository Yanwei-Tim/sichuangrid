...
<div class="content" >
	<div id="search-condition-form" title="查询" class="container container_24">
		...
		<input type="hidden" id="twice" value="${twice}"/>
		<input type="hidden" id="seletedMore" value="${seletedMore}"/>
...
<script>
var multiSelected = '${param.seletedMore}';
$(document).ready(function(){
	...
	function getSearchAction(){
		if ($("#mode").val()=="searchTarget"){
			return "${path}/threeRecordsIssues/issueManage/searchTransferTarget.action?mode=list";
		}else if ($("#mode").val()=="searchTells"){
			return "${path}/threeRecordsIssues/issueManage/searchTellTarget.action?mode=list";
		}else if($("#mode").val()=="searchNotices"){
			return 	"${path}/threeRecordsIssues/issueManage/searchNoticeTarget.action?mode=list";
		}else {
			return "";
		}
	}
	...
	function canMultiSelected(){
		return 	$("#mode").val()=="searchTells"||$("#mode").val()=="searchNotices";
	}
	...
	function selectCorrectOrg(rowId,e){
		if (!canMultiSelected()&&multiSelected!=1){
			$("#orgList").resetSelection();
		}
		return true;
	}
	if(!canMultiSelected()&&multiSelected!=1)
		$("#orgList_cb :input").eq(0).attr("disabled","disabled");
