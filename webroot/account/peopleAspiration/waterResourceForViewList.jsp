...
<div class="content">
	<jsp:include page="${path}/account/peopleAspiration/waterResourceForViewButtons.jsp"/>
    ...
    <div id="waterResourceDialog"></div>
...
$(document).ready(function(){
	...
	function dealStateFormatter(el, options, rowData){
		/*if(viewType.value=='done'||viewType.value=='follow'){//对于已办台账和待跟进台账状态的处理 
			if(rowData.status==1){
				return "办理中";
	...
	$("#issueList").jqGridFunction({
		...
		colModel:[
			{name:'issueId',frozen:true,hidden:true,hidedlg:true,frozen:true},
			{name:'issueStepId',frozen:true,hidden:true,hidedlg:true,frozen:true,key:true},
			...
			{name:'publicltycass',frozen:true,hidden:true,hidedlg:true,frozen:true},
			{name:'earingWarn',frozen:true,hidden:true,hidedlg:true,frozen:true},
			{name:'supervisionState',index:'superviselevel',frozen:true,hidden:true,hidedlg:true},
			{name:'urgent',index:'lpa.urgent',frozen:true,hidden:true,hidedlg:true,frozen:true},
			...
			{name:'status',index:'lpa.status',frozen:true,hidden:true,hidedlg:true,frozen:true},
...
function initIssueCount(){
	$.ajax({
        url: "/threeRecords/water/getIssueCount.action",
        data: {
...
function viewIssue(rowid){
	...
	//if (hasRowSelected() && $("#view").isButtonEnabled()){
		...
		$("#issueDialog").createDialog({
			...
			height: 500,
			title: "查看水利信息详情",
			url: "${path}/threeRecords/water/dispatch.action?mode=viewNew&keyId="+selectedIssue.encryptIdByIssueId,
			buttons: {
		   		"关闭" : function(){
		        	$(this).dialog("close");
		   		}
