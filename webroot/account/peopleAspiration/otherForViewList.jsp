...
<div class="content">
	<jsp:include page="${path}/account/peopleAspiration/otherForViewButtons.jsp"/>
...
function initIssueCount(){
	$.ajax({
        url: "/threeRecords/other/getIssueCount.action",
        data: {
...
function viewIssue(rowid){
	...
	//if (hasRowSelected() && $("#view").isButtonEnabled()){
		...
		$("#issueDialog").createDialog({
			...
			height: 500,
			title: "查看其他信息详情",
			url: "${path}/threeRecords/other/dispatch.action?mode=viewNew&keyId="+selectedIssue.encryptIdByIssueId,
			buttons: {
		   		"关闭" : function(){
		        	$(this).dialog("close");
		   		}
