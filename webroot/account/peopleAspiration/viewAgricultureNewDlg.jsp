			...
			<table class="newTableList">
				<tbody>
			      ...
			      <!--  	<tr class="contradiction resolveTheContradictions">
			       ...
			       <tr>
						<td class="title">工程名称</td><td class="contable">${agriculture.projectName}</td>
					</tr>
			       	<tr>
						<td class="title">受益人口</td><td class="contable">${agriculture.beneficiaryNumber}</td>
					</tr>
					<tr>
						<td class="title">建设起点</td><td class="contable">${agriculture.fromAddress}</td>
					</tr>
					<tr>
						<td class="title">建设讫点</td><td class="contable">${agriculture.toAddress}</td>
					</tr>
					<c:if test="${agriculture.buildType.id != null}">
			       	<tr>
			        	<td class="title">建设性质</td><td class="contable">
			        	<pop:PropertyDictViewTag name="@com.tianque.plugin.account.property.PropertyTypes@AGRICULTURE_PROJECT_BUILD_TYPE" defaultValue="${agriculture.buildType.id}" /></td>
			       	</tr>
			       	</c:if>
			       	<c:if test="${agriculture.projectCategory.id != null}">
			       	<tr>
			        	<td class="title">项目类别</td><td class="contable">
			        	<pop:PropertyDictViewTag name="@com.tianque.plugin.account.property.PropertyTypes@AGRICULTURE_PROJECT" defaultValue="${agriculture.projectCategory.id}" /></td>
			       	</tr>
			       	</c:if>
			       	<c:if test="${agriculture.projectSubCategory.id != null}">
			       	<tr>
			        	<td class="title">项目类型</td><td class="contable">
			        	<pop:PropertyDictViewTag name="@com.tianque.plugin.account.property.PropertyTypes@AGRICULTURE_PROJECT_SUB" defaultValue="${agriculture.projectSubCategory.id}" /></td>
			       	</tr>
			       	</c:if>
	
					<tr>
						<td class="title">规模</td><td class="contable">${agriculture.scopeNumber}</td>
					</tr>
					
			       	<tr>
						<td class="title">计划资金(万元)</td><td class="contable">${agriculture.plannedInvestment}</td>
					</tr>
					<tr>
						<td class="title">自筹资金(万元)</td><td class="contable">${agriculture.selfFund}</td>
					</tr>
						<tr>
						<td class="title">缺口资金(万元)</td><td class="contable">${agriculture.gapFund}</td>
					</tr>
