...
<pop:JugePermissionTag ename="addAgriculture">
	<a id="add" href="javascript:;"><span>新增</span></a>
</pop:JugePermissionTag> <pop:JugePermissionTag ename="updateAgriculture">
	<a id="update" href="javascript:void(0)"><span>修改</span></a>
</pop:JugePermissionTag> <pop:JugePermissionTag ename="deleteAgriculture">
...
var issueButtons={
	...
	myneed:function(){//我的待办
		...
		<s:else>
			this.showButtons(["新增","修改",
			      			<c:if test='${isShowDeleted}'>"删除",</c:if>
			      			"导入","查询","查看","刷新","打印","导出","受理","办理","回复"]);
		</s:else>
	},
...
$(function(){
	...
	$("#add").click(function(event){
		$("#issueDialog").createDialog({
			...
			height:570,
			title:'新增农业资源信息',
			url:'${path}/threeRecords/agriculture/dispatch.action?mode=add&keyId='+$("#currentOrgId").val(),
			buttons: {
				"上一步": function (event) {
	...
	$("#delete").click(function(event){
		if (hasRowSelected()){
			$.confirm({
				title:"确认删除",
				message:"该农业资源处理信息删除后,将无法恢复,您确认删除该农业资源处理信息吗?",
				width:400,
				okFunc:function(){
					//deleteAgriculture(getSelectedData().encryptIdByIssueId);
					var strs = getSelectedIds();
                    	for (i=0;i<strs.length ;i++) {
                    		deleteAgriculture(getData(strs[i]).encryptIdByIssueId);
                    	}
				}
			});
		}else{
	...
	$("#search").click(function(event){
		$("#issueDialog").createDialog({
			width: 750,
			height: 400,
			title: "农业资源查询-请输入查询条件",
			url: "${path}/threeRecords/agriculture/dispatch.action?mode=search&keyId="+$("#currentOrgId").val()+'&tag='+viewType.value+"&selectedIssueType=",
			buttons: {
		   		"查询" : function(event){
	...
	$("#import").click(function(event){
        $("#agricultureDialog").createDialog({
            width: 400,
            height: 230,
            title:'数据导入',
            url:PATH+'/account/dataTrans/import.jsp?isNew=1&dataType=AgricultureData&dialog=agricultureDialog&startRow=1&templates=AGRICULTUREDATA&listName=agricultureForViewList',
            buttons:{
                "导入" : function(event){
                    $("#mForm").submit();
                },
	...
	$("#printIssue").click(function(event){
		...
		$("#issueDialog").createDialog({
			...
			height: 600,
			title: '打印农业信息',
			url:'${path}/threeRecords/agriculture/dispatch.action?mode=print&keyId=' + selectedIssue.encryptIdByIssueId,
			buttons: {
		   		"打印" : function(event){
					$("#agriculturePrint").printArea();
	   			},
		   		"关闭" : function(){
		        	$(this).dialog("close");
		   		}
    ...
    $("#exportIssue").click(function (event) {
        ...
        var selectedIssue = getSelectedData();
        var url='${path}/threeRecords/agriculture/exportExcel.action?keyId=' + selectedIssue.encryptIdByIssueId;
	...
	$("#conceptTop,#conceptBottom").click(function(event){
		...
		var currentOrgId='${loginAction.user.organization.id}';
		 if(currentOrgId !=selectedIssue['targetOrg.id']){
        	 $.messageBox({level: 'warn', message: "该农业资源不可受理，请进行其它操作！"}); 
        	 return;
        }
	...
	//由于该页面是动态的，使用click会重复绑定农业资源,所有使用以下方法
	$("#dealTop,#dealBottom").die().live("click",function(event){
		...
		if(selectedIssue.dealState!=null && '办理中' == selectedIssue.dealState){
			$.ajax({
	            url: "/threeRecords/agriculture/existAgriculture.action",
	            data: {
	                "keyId": selectedIssue.issueId
	            },
	            success: function (responseData) {
	                if (responseData==null||responseData.id==null) {
	                    $.messageBox({level: 'warn',message: "该条农业资源类信息不可办理，请完善研究管理信息内容！"});
	                } else {
			...
			$.messageBox({level:'warn',message:"该条农业资源类信息不可办理，请先受理后再作办理操作！"});
		}
	});
	$("#reportToTop,#reportToBottom").click(function(event){
		...
		if(selectedIssue.dealState!=null){
			...
			$.messageBox({level:'warn',message:"该农业资源不可 上报指挥中心，请进行其它操作！"});
		}
	});
	<c:if test="${isShowTreeSelect}">
...
function printAcceptForm(id){
	$("#issueDialog").createDialog({
	    ...
	    height: 400,
	    title: "受理单",
	    url: '${path}/threeRecords/acceptForm/dispatch.action?mode=add&keyId='+id+'&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_AGRICULTURE"/>',
	    ajaxType:'post',
	    buttons: {
	        "保存打印": function (event) {
	            $("#accept_form").submit();
...
function editThreeRecords(id){
	$("#issueDialog").createDialog({
		...
		height:570,
		title: '修改农业资源信息',
		url:'/threeRecords/agriculture/dispatch.action?mode=edit&keyId='+id,
		buttons: {
			"上一步": function (event) {
		    	$(".ui-dialog-buttonset  button").eq(0).hide();
		    	$('#tabs-option').tabs('select',0);
...
function deleteAgriculture(keyId){
	$.ajax({
		url:"/threeRecords/agriculture/deleteAgriculture.action",
		data:{
			"keyId":keyId
		},
		success:function(responseData){
			if (responseData==true){
				$.messageBox({message:"已经成功删除该农业资源处理信息!"});
				//reloadIssue();
				searchIsssueById();
				getMessageByUser();
			}else{
				$.messageBox({level:'error',message:"删除农业资源失败!"});
			}
		}
	});
}
$("#feedBack").click(function(event){
	...
	$.post('${path}/threeRecords/feedBack/isCanFeedBack.action?&ledgerId='+ selectedIssue.issueId+'&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_AGRICULTURE"/>', function (data) {
		if(!data){
			$.messageBox({level:'warn',message: "该农业资源不可反馈，请进行其它操作！"});
			return;
		}
		$("#feedBackDialog").createDialog({
			...
			height: 400,
			title: "反馈",
			url: '${path}/threeRecords/feedBack/dispatch.action?keyId=' + selectedIssue.issueStepId+'&ledgerId='+ selectedIssue.issueId+'&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_AGRICULTURE"/>',
			buttons: {
		   		"反馈" : function(event){
...
$("#replyFormTop").click(function (event) {
    ...
    var selectedIssue = getSelectedData();
     $("#feedBackDialog").createDialog({
         ...
         height: 400,
         title: "回复单",
         url: '${path}/threeRecords/replyForm/dispatch.action?keyId=' + selectedIssue.issueStepId+'&serialNumber='+ selectedIssue.serialNumber + '&ledgerId=' + selectedIssue.issueId + '&ledgerType=<s:property value="@com.tianque.plugin.account.constants.LedgerConstants@PEOPLEASPIRATION_AGRICULTURE"/>',
         buttons: {
             "回复": function (event) {
