...
<div class="content">
	<jsp:include page="${path}/account/peopleAspiration/medicalForViewButtons.jsp"/>
...
$(document).ready(function(){
	...
	$("#issueList").jqGridFunction({
		...
		colModel:[
			...
			{name:'occurOrg.orgName',index:'occurOrg',sortable:false,width:200},
...
function initIssueCount(){
	$.ajax({
        url: "/threeRecords/medical/getIssueCount.action",
        data: {
...
function viewIssue(rowid){
	...
	//if (hasRowSelected() && $("#view").isButtonEnabled()){
		...
		$("#issueDialog").createDialog({
			...
			height: 500,
			title: "查看医疗卫生信息详情",
			url: "${path}/threeRecords/medical/dispatch.action?mode=viewNew&keyId="+selectedIssue.encryptIdByIssueId,
			buttons: {
		   		"关闭" : function(){
		        	$(this).dialog("close");
		   		}
