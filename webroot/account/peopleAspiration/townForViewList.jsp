...
<div class="content">
	<jsp:include page="${path}/account/peopleAspiration/townForViewButtons.jsp"/>
...
function initIssueCount(){
	$.ajax({
        url: "/threeRecords/town/getIssueCount.action",
        data: {
...
function viewIssue(rowid){
	...
	//if (hasRowSelected() && $("#view").isButtonEnabled()){
		...
		$("#issueDialog").createDialog({
			width: 880,
			height: 500,
			title: "查看城乡规划建设管理信息详情",
			url: "${path}/threeRecords/town/dispatch.action?mode=viewNew&keyId="+selectedIssue.encryptIdByIssueId,
			buttons: {
		   		"关闭" : function(){
		        	$(this).dialog("close");
		   		}
