...
<div class="content">
	<jsp:include page="${path}/account/peopleAspiration/environmentForViewButtons.jsp"/>
...
function initIssueCount(){
	$.ajax({
        url: "/threeRecords/environment/getIssueCount.action",
        data: {
...
function viewIssue(rowid){
	...
	//if (hasRowSelected() && $("#view").isButtonEnabled()){
		...
		$("#issueDialog").createDialog({
			...
			height: 500,
			title: "查看环境保护信息详情",
			url: "${path}/threeRecords/environment/dispatch.action?mode=viewNew&keyId="+selectedIssue.encryptIdByIssueId,
			buttons: {
		   		"关闭" : function(){
		        	$(this).dialog("close");
		   		}
