...
<s:if test="lastStep.dealType == null">
	<input type="hidden" id="lastDealDate" value="<fmt:formatDate value="${ledgerSteadyWork.registrationTime}" type="date" pattern="yyyy-MM-dd" />" />
</s:if>
<s:else>
...
<div id="blPage" class="cf">
		...
		<div class="left">
			...
			<tbody>
		        <tr>
					<td ><div class='innerTxt cf'><strong>台账名称:</strong>${ledgerSteadyWork.subject}</div></td>
				</tr>
				 <tr>
					<td ><div class='innerTxt cf'><strong>登记时间:</strong><s:date format="yyyy-MM-dd" name="ledgerSteadyWork.occurDate" /></div></td>
				</tr>
				 <tr>
					<td ><div class='innerTxt cf'><strong>主要当事人:</strong>${ledgerSteadyWork.name}</div></td>
				</tr>
				 <tr>
					<td ><div class='innerTxt cf'><strong>服务联系人:</strong>${ledgerSteadyWork.serverContractor}${ledgerSteadyWork.serverTelephone}</div></td>
				</tr>
				<tr>
					<td ><div class='innerTxt cf'><strong>服务职务:</strong>${ledgerSteadyWork.serverJob}</div></td>
				</tr>
				<tr>
					<td ><div class='innerTxt cf'><strong>服务联系人单位:</strong>${ledgerSteadyWork.serverUnit}</div></td>
				</tr>
				<tr>
					<td ><div class='innerTxt cf'><strong>涉稳事项:</strong>${ledgerSteadyWork.involvingSteadyInfo}</div></td>
				</tr>
									...
									<li class="record" id="2012-12-11" <c:if test ="${submitToCommittee}">canSubmit='1'</c:if>>
                                        ...
                                        <div class="dispose">
                                            <div class="small_Text">
                                              ...
                                              <strong>工作措施：</strong>${content }<br>
                                              <strong>交流地点：</strong>${address }<br>
                                              <strong>参与人：</strong>${joinMen }
                                            </div>
		...
		<div class="right">
			<div id="dialog-form" title="办理台账处理">
				<form id="issueDealForm" method="post" action="${path }/threeRecordsIssue/ledgerSteadyWorkIssueManage/dealIssue.action">
					...
					<input type="hidden" name="operation.ledgerId" id="issueId" value="${operation.ledgerId }" />
					<input type="hidden" name="keyId" id="keyId" value="${keyId }" />
					<input type="hidden" name="operation.ledgerType" id="ledgerType" value="${ledgerSteadyWork.ledgerType}" />
					...
					<div id="exchange">
						<div class="grid_4 lable-right">
							<label class="form-lbl">交流日期：</label>
						</div>
						<div class="grid_10 form-left">
						    <input type="text" id="exchangeDate" name="operation.exchangeDate" readonly="readonly" class="form-txt" />
						</div>
					<div class='clearLine'></div>
					<div>
						<div class="grid_4 lable-right">
							<em class="form-req">*</em><label class="form-lbl">参与人：</label>
						</div>
						<div class="grid_10 form-left">
						    <input type="text" id="joinMen" name="operation.joinMen" class="form-txt" />
						</div>
						<div class='clearLine'></div>
					<div class="grid_4 lable-right">
						<label class="form-lbl">交流地点：</label>
					</div>
					<div class="grid_10 heightAuto">
						<textarea name="operation.address" style="width:180px; MARGIN-TOP: 10PX;MARGIN-bottom:10px" rows="3" cols="26"  id="address" ></textarea>
					</div>
					</div>
					<div class='clearLine'></div>
					</div>
					...
					<div class="grid_4 lable-right steady" id= "typeComplete" >
						<em class="form-req">*</em><label>办结类型：</label>
					</div>
					<div class="grid_8 form-left steady heightAuto" id="steady<s:property value='@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@COMPLETE.code'/>" >
			    		<input class="completeTypeText" name="operation.completeType" type="radio" value="<s:property value='@com.tianque.plugin.account.constants.CompleteType@REALCOMPLETE_SOLVE'/>">实质性化解
			    		<input class="completeTypeText" name="operation.completeType" type="radio" value="<s:property value='@com.tianque.plugin.account.constants.CompleteType@PROGRAM_COMPLETE'/>">程序性终结
			    		<input class="completeTypeText" name="operation.completeType" type="radio" value="<s:property value='@com.tianque.plugin.account.constants.CompleteType@COMPLETE'/>">实质性终结
					</div>
					
					
					<div class="grid_8 form-left steady" id="steady<s:property value='@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@PERIOD_COMPLETE.code'/>"  >
			    		<input class="completeTypeText" name="operation.completeType" type="radio" value="<s:property value='@com.tianque.plugin.account.constants.CompleteType@STEADY_CONTROL'/>">稳控中
			    	</div>
			    	
			    	<div class="grid_8 form-left steady" id="steady<s:property value='@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@PROGRAM_COMPLETE.code'/>"  >
			    		<input class="completeTypeText" name="operation.completeType" type="radio" value="<s:property value='@com.tianque.plugin.account.constants.CompleteType@OTHER'/>">其它
			    	</div>
...
$(function(){
    ...
    $("#exchangeDate").attr("value",formatdate);
    $(".steady").each(function(){
		$(this).hide();
	})
    ...
    if(new RegExp(/(1lx|1xw|1jg)$/).exec($("#currentDepartNo").val())){
    	$("#exchange").hide();
    	$("#_opinion").hide();
	...
	$("#issueDealForm").formValidate({
		...
		submitHandler: function(form){
			...
			if(!new RegExp(/(1lx|1xw|1jg)$/).exec($("#currentDepartNo").val())){
				if($("#joinMen").val()==""){
					$.messageBox({message:"请输入参与人！",level:"error"});
					return;
				}
			}
			if( $("#dealCode").val() == "<s:property value='@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@COMPLETE.code'/>"
			    ...
			    || $("#dealCode").val() == <s:property value='@com.tianque.plugin.account.state.ThreeRecordsIssueOperate@PROGRAM_COMPLETE.code'/>
			   	...
			   	var checked=false;
				$("#steady"+$("#dealCode").val()).find("input:radio[name='operation.completeType']").each(function(){
