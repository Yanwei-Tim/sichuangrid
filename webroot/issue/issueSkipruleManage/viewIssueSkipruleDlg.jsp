...
<div id="dialog-form" title="查看事件类型越级规则设置" class="container container_24">
	...
	<form id="maintainForm" method="post" action="">
		...
		<s:if test="@com.tianque.core.util.DialogMode@VIEW_MODE.equalsIgnoreCase(mode)">
			<pop:token />
		</s:if>
		...
		<s:if test="@com.tianque.core.util.ThreadVariable@getUser().organization.orgLevel.displayName=='县（区）'">
			<select name="issueSkiprule.issueUrgentLevel.id" class='form-select {required:true,messages:{required:"请选择紧急等级"}}' id="org" >
				<option value='${issueSkiprule.issueUrgentLevel.id}' <s:if test="issueSkiprule.issueUrgentLevel.displayName==高">selected</s:if> internalId=''>高</option>
			</select>
		</s:if>
		
		<s:if test="@com.tianque.core.util.ThreadVariable@getUser().organization.orgLevel.displayName=='乡镇（街道）'">
			<select name="issueSkiprule.issueUrgentLevel.id" class='form-select {required:true,messages:{required:"请选择紧急等级"}}' id="org" >
				<option value='${issueSkiprule.issueUrgentLevel.id}' <s:if test="issueSkiprule.issueUrgentLevel.displayName==中">selected</s:if> internalId=''>中</option>
			</select>
		</s:if>
		</div>
		<div class="grid_4 lable-right">
		...
		<s:if test="@com.tianque.core.util.ThreadVariable@getUser().organization.orgLevel.displayName=='县（区）'">
			<select name="issueSkiprule.submitLevel.id" class='form-select {required:true,messages:{required:"请选择上报层级"}}' id="org" >
				<option value='${issueSkiprule.submitLevel.id}' <s:if test="issueSkiprule.submitLevel.displayName==县（区）">selected</s:if> internalId=''>县（区）</option>
			</select>
		</s:if>
		<s:if test="@com.tianque.core.util.ThreadVariable@getUser().organization.orgLevel.displayName=='乡镇（街道）'">
			<select name="issueSkiprule.submitLevel.id" class='form-select {required:true,messages:{required:"请选择上报层级"}}' id="org" >
				<option value='${issueSkiprule.submitLevel.id}' <s:if test="issueSkiprule.submitLevel.displayName==乡镇（街道）">selected</s:if> internalId=''>乡镇（街道）</option>
			</select>
		</s:if>
		</div>
		<div class='clearLine'>&nbsp;</div>
