...
<style type="text/css">
.dialog-formArea{background: #f4f5f5;}
...
<div class="dialog-formArea">
<div id="dialog-form" title="" class="container container_24">
	<input type="hidden"" id="isShowSubmit" value="true">
	<form id="issueMaintainForm" method="post" action="">
			...
			<div class="grid_10 form-left">
		    	<input type="text" id="issueSubject" disabled="disabled" name="issue.subject" maxlength="50" value="${issue.subject}" class="form-txt" />
			</div>
			<div class="grid_3 lable-right">
				<em class="form-req">*</em><label class="form-lbl">发生网格：</label>
	  		</div>
    		<div class="grid_7 form-left">
				<input type="text" id="issueOccurOrgSelector" disabled="disabled" name="selectOrgName"  class="form-txt" />
    		</div>
			...
			<div class="dlg_innerBox">
				...
				<div class="grid_4 form-left">
			    	<input type="text" id="occurDate" disabled="disabled" name="issue.occurDate" class="form-txt"
					value="<s:date name="issue.occurDate" format="yyyy-MM-dd "/>" readonly />
				</div>
				<div class="grid_1"></div>
	    		<div class="grid_2 form-left">
					<select id="hours" name="hours" disabled="disabled" class="form-txt">
						<option value="">时</option>
					</select>
	    		</div>
	    		<div class="grid_1"></div>
	    		<div class="grid_2 form-left">
	    			<select id="minute" name="minute" disabled="disabled" class="form-txt">
				    ...
				    <c:when test="${issue.emergencyLevel!=null}">
				     	<div class="grid_4 lable-right" id="emergencyLevelDiv">
				  		...
				  		<div class="grid_6" id="emergencyLevel">	
							<label><input type="radio" disabled="disabled" value="<s:property value='issue.emergencyLevel.id'/>" checked="checked" name="issue.emergencyLevel.id"  /><s:property value='issue.emergencyLevel.displayName'/></label>
				  		</div>
				    </c:when>
				    <c:otherwise>
				...
				<div id="occurLocationDiv">
					...
					<div class="grid_14 form-left" id="div1">
				    	<input type="text" disabled="disabled" id="issueLocation" readonly="readonly" name="issue.occurLocation" maxlength="50" value="${issue.occurLocation}" class="form-txt" />
					</div>
				</div>
		    		...
		    		<div class="grid_4 lable-right">
		    			<c:choose>
						    <c:when test="${issue.importantPlace==1}">
						     	<label><input type="checkbox" checked="checked" disabled="disabled" value="1" name="issue.importantPlace"/>是否重点场所</label>
						    </c:when>
						    <c:otherwise>
						    	<label><input type="checkbox" value="1" disabled="disabled"  name="issue.importantPlace"/>是否重点场所</label>
						    </c:otherwise>
						</c:choose>
					</div>
				</div>
				    			...
				    			<s:if test="@com.tianque.core.util.DialogMode@EDIT_MODE.equalsIgnoreCase(mode)">
				    				<s:iterator value="issue.issueRelatedPeoples" id="issueRelatedPeople">
					    				<li>
					    					...
					    					<div class="grid_4"><input type="text" disabled="disabled" name="issueRelatedPeopleNames" value='<s:property value="name"/>' maxlength="20" class='form-txt 
					    							{required:true,maxlength:20,minlength:2,exculdeParticalChar:true,messages:{required:"请输入姓名",maxlength:$.format("姓名不能多于{0}个字符"),minlength:$.format("姓名不能少于{0}个字符"),exculdeParticalChar:"不能输入非法字符"}}' /></div>
					    					<div class="grid_3 lable-right">联系手机：</div>
					    					<div class="grid_4"><input type="text" disabled="disabled" name="issueRelatedPeopleTelephones" value='<s:property value="telephone"/>' maxlength="11" class='form-txt {maxlength:11,mobile:true,messages:{maxlength:$.format("联系手机不能多于{0}个字符"),mobile:"手机号码必须由1开头的11位数字组成"}}' /></div>
					    					<div class="grid_3 lable-right">固话：</div>
					    					<div class="grid_4"><input type="text" disabled="disabled" name="issueRelatedPeopleFixPhones" value='<s:property value="fixPhone"/>' maxlength="15" class='form-txt {maxlength:15,telephone:true,messages:{maxlength:$.format("固话不能多于{0}个字符"),telephone:"固话不合法，只能输数字和横杠(-)"}}' /></div>
					    				    <div class="grid_3 delItemBox" ><a href="javascript:;" class="delPeopleItem" style="display: none;">删除</a></div>
				    					</li>
			    					</s:iterator>
								</s:if>
		    			</ul>
	    		...
	    		<div id="involvedPersonnelDiv">
		    		...
		    		<div class="grid_20 form-left heightAuto" style="line-height:20px;">
						<input type="text" id="involvedPersonnel" value="" name="involvedPersonnel" disabled="disabled" class="form-txt" />
		    		</div>
	    		</div>
				...
				<div id="issueKindAndCount">
					...
					<select id="issueKind" disabled="disabled" name="issue.issueKind.id" class='form-txt'>
					...
					<div class="grid_7 form-left">
						<input type="text" disabled="disabled"  name="issue.relatePeopleCount" maxlength="6" value="${issue.relatePeopleCount}" class="form-txt" style="text-align:right;" />
					</div>
				...
				<div class="grid_20 heightAuto">
	    			<textarea id="issueContent" disabled="disabled" name="issue.issueContent" class="form-txt" style="height:50px;">${issue.issueContent}</textarea>
				</div>
			</div>
			...
			<div class="clearfix">
				...
				<div class="grid_8 heightAuto" >
					<div>&nbsp;</div>
					<div id="issueViewMap" style="width: 250px; height: 165px;margin-top:-32px;overflow-y: hidden;overflow-x: hidden;border:1px solid #7F9DB9">
					</div>
				</div>
			</div>
	</form>
	...
	<div id="appendImportantPlaceDiv" style="display: none;">
   		<div class="grid_4 lable-right">
   			<c:choose>
			    <c:when test="${issue.importantPlace==1}">
			     	<label><input type="checkbox" disabled="disabled" checked="checked" value="1" name="issue.importantPlace"/>是否重点场所</label>
			    </c:when>
			    <c:otherwise>
			    	<label><input type="checkbox" disabled="disabled" value="1"  name="issue.importantPlace"/>是否重点场所</label>
			    </c:otherwise>
			</c:choose>
		</div>
	</div>
	<div id="appendOccurLocationDiv" style="display: none;">
		...
		<div class="grid_14 form-left" id="div1">
	    	<input type="text" id="issueLocation" disabled="disabled" name="issue.occurLocation" maxlength="50" value="${issue.occurLocation}" class="form-txt" />
		</div>
	</div>
...
function setDivShow(flag){
	switch(flag){
		...
		case '<s:property value="@com.tianque.issue.constants.IssueTypes@SECURITYPRECAUTIONS" escape="false"/>':
			$("#issueRelatedPeoplesLabel").html('<em class="form-req">*</em>联系人：');
			$("#occurDateLabel").html("发生时间：");
			$("#issueKindAndCount").empty();
			$("#occurLocationDiv").empty().append($("#appendOccurLocationDiv").html());	
			$("#importantPlaceDiv").empty().append($("#appendImportantPlaceDiv").html());
...
$(document).ready(function(){
	...
	function initAttachFileUploader(){
		...
		$('#custom_file_uploadUploader').hide();
	}
	...
	function initIssueTypeSelector(){
		<s:iterator value="issueTypeNames" var="issueTypeName" status="st">
			<c:if test="${fn:length(issueTypes[#issueTypeName.typeName])>0}">
	...
	function bindFormValidation(){
		$("#issueMaintainForm").formValidate({
			...
			$.messageBox({level:'warn',message:"该事件已进入流转过程，不能修改！"});
			return  ;
			$(form).ajaxSubmit({
				success:function(data){
