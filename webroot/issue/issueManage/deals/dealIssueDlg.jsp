			...
			<tbody>
				...
				<tr>
					<td ><div class='innerTxt cf'><strong>事件类型:</strong>
						${issue.issueTypeDomainName},${issue.issueTypeName}
                    </div></td>
			    </tr>
				<tr>
				...
				<form id="issueDealForm" method="post" action="${path }/issues/issueManage/dealIssue.action">
						...
						<c:if test="${issueSkiprule==null||issueSkiprule.id==null}">
							...
							<div class="grid_10">
								...
								<s:if test="@com.tianque.core.util.GridProperties@CMS_CALLCENTER_IS_OPEN">
		                            <label for="transferToCallCenter">
		                                <input type="radio" name="transferToType" id="transferToCallCenter" value="2" onclick="transferToTypeChange(this)"/>12345指挥中心
									</label>
									<input type="hidden" name="operation.report12345" id="isReport12345">
								</s:if>
							</div>
						</c:if>
					...
					<c:choose>
						...
						<div id="tellToSelectorDiv" class="cf heightAuto" style="margin:10px 0;">
					    	...
					    	<!-- 
								<div class="grid_4 lable-right">
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label class="form-lbl">抄告2：</label>
								</div>
								<div class="grid_5">
...
$(function(){
	...
	$("#issueDealForm").formValidate({
		...
		submitHandler: function(form){
			...
			$("#isReport12345").val($("#transferToCallCenter").attr("checked")=="checked");
			$(form).ajaxSubmit({
	...
	function createOrgSearchDialog(searchUrl,inputId,isMultiselect){
		$("#orgSelectDialog").createDialog({
			...
			postData:{	adminTarget:function(){return targetIsAdmin();},
					    ...
					    report12345: $("#transferToCallCenter").attr("checked")=="checked"
					},
			buttons: {
				"确定" : function(event){
...
function renderIssueType(){
	...
	var typeDesc;
	var actualIndex;
    <s:iterator value="issue.issueType" var="type" >
