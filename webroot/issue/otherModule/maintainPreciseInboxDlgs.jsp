...
<div id="dialog-form" title="" class="container container_24" style="background: #f4f5f5;">
	<form id="issueMaintainForm" method="post" action="${path}/issues/issueManage/addIssueByTencentPreciseInbox.action">
			...
			<input type="hidden" name="textSendMessage.touser" value="${preciseInbox.fromUserName}">
			<input id="inboxTypeDlg" name="preciseInbox.inboxType" type="hidden" value="${preciseInbox.inboxType}" />
			...
			<div class="dlg_innerBox">
				...
				<div id="occurLocationDiv">
					...
					<div class="grid_14 form-left" id="div1">
				    	<input type="text" id="issueLocation" name="issue.occurLocation" maxlength="50" value="${issue.occurLocation}" class="form-txt {required:true,messages:{required:'发生地点不能为空'}}" />
					</div>
				</div>
				    			...
				    			<s:if test="@com.tianque.core.util.DialogMode@EDIT_MODE.equalsIgnoreCase(mode)">
				    				<s:iterator value="issue.issueRelatedPeoples" id="issueRelatedPeople">
					    				<li>
					    					...
					    					<div class="grid_4"><input type="text" name="issueRelatedPeopleNames" value='${issue.sourcePerson}' maxlength="20" class='form-txt 
					    					...
					    					<div class="grid_4"><input type="text" name="issueRelatedPeopleTelephones" value='${issue.sourceMobile}' maxlength="11" class='form-txt {maxlength:11,mobile:true,messages:{maxlength:$.format("联系手机不能多于{0}个字符"),mobile:"手机号码必须由1开头的11位数字组成"}}' /></div>
				...
				<div class="grid_20 heightAuto" style='line-height:18px;'>
	    			<textarea id="issueContent" name="issue.issueContent" class="form-txt"  style="height:80px;width:620px;"><s:iterator value="map" status="st" begin="0"></s:iterator>${issue.issueContent}</textarea>
				</div>
			</div>
			...
			<div class="grid_24">
				...
				<div class="grid_21 heightAuto">
					<textarea name="textSendMessage.text.content" id="content" style="height:70px;width:625px;" class='form-txt {maxlength:1000,messages:{maxlength:"查询语句最多需要输入1000个字符"}}'>您的事件已受理！</textarea>
				</div>
			</div>
	</form>
	
	<div class="clearfix" style="position: relative;top: 60px;">
		<div class="grid_3 lable-right">
				...
				success:function(data){
					if(data==null || !data.issueId){
						...
						getInboxList({ 
							...
							"preciseInbox.inboxType" :$("#inboxTypeDlg").val()
							});
						getMessageByUser();
