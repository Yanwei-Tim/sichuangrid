...
<div class="container container_24">
	...
	<c:if test='${"edit"==mode }'>
   	<form id="routes-form" method="post" action="${path }/evaluate/evaluateManage/updateDetailedRule.action" >
		<c:if test="${detailedRule.parentRule.id }">
		...
		<div class="grid_20" style="height: 100px">
			<textarea name="detailedRule.content" rows="4" class="form-txt" style="overflow-y:scroll;height:100px"
		...
		<br/>
		<br/>
		<c:if test='${parameters.typeOfDetailedRule[0] != "HAVENOSCORE" }'>
		<c:if test='${"view"!=mode && parameters.level[0]!=2 }'>
		
		<div class="grid_4 lable-right">
		<c:if test='${"view"!=mode && parameters.level[0]==4 }'>
				<em class="form-req">*</em>
		</c:if>
		<c:if test='${parameters.typeOfDetailedRule[0] == "HAVESCORE" }'>
				<em class="form-req">*</em>
		</c:if>
			<label>标准分：</label>
		</div>
...
$(document).ready(function(){
	...
	if(rowData.treelevel == 2 && mode == "edit"){
		$("#score").attr("readonly","readonly")
	}else{
		if($(rowData.treelevel).text() == 2 && mode =="edit"){
			$("#score").attr("readonly","readonly")
		}
	}
	$("#routes-form").formValidate({
		...
		<c:if test='${"add"==mode}'>
			...
			$(form).ajaxSubmit({
				success:function(data){
					...
					$.ajax({
						url:'${path}/evaluate/evaluateManage/getEvaluateById.action?evaluate.id=${detailedRule.evaluate.id }',
						success:function(data){
							$("#evaluateList").setRowData('${detailedRule.evaluate.id }',{templateTotalScore:data.templateTotalScore});
							$("#itemsList").clearGridData();
		...
		rules:{
			...
			"detailedRule.standardScore":{
				<c:if test='${"view"!=mode && parameters.level[0]==4 || parameters.typeOfDetailedRule[0] == "HAVESCORE"}'>
				required:true,
				</c:if>
