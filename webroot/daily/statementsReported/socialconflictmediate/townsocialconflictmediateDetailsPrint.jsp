...
<table id='tb' class="tb" border=0 cellpadding=0 cellspacing=0 style='margin:10px 10px;border-collapse:collapse;table-layout:fixed;width:703pt'>
	...
	<tr>
		<td colspan=10 height=44 id="reportName" class=head style='text-align:center;border:0px;height:33.0pt;font-size:25px; font-weight:bold;'>${dailyYear.name}年<span id="selectedMonthForTown"></span>${organization.orgName}矛盾纠纷排查情况表</td>
	</tr>
	</s:if>
	<s:else>
		<tr>
	...
	<tr>
		...
		<s:if test="reportTypeInternalId==@com.tianque.domain.property.DirectoryReportType@MONTH_REPORT">
			<td colspan=3 class=head style="border:0px; border-bottom:1px solid gray; text-align:right;margin-right:20px;">填报月份：
	  	...
	  	<s:if test="reportTypeInternalId==@com.tianque.domain.property.DirectoryReportType@QUARTER_REPORT">
		  	<td colspan=3 class=head style="border:0px; border-bottom:1px solid gray; text-align:right;margin-right:20px;">填报季度：
			  	<select id="reportTime" <s:if test='!"add".equals(mode)'>disabled="disabled"</s:if>>
					<s:iterator value="dateMap" id="date" >
					    <s:property value="#date"/>
						<option value='<s:property value="key"/>'><s:property value="value"/></option>
	  	...
	  	<s:if test="reportTypeInternalId==@com.tianque.domain.property.DirectoryReportType@SEMIYEARLY_REPORT">
		  	<td colspan=3 class=head style="border:0px; border-bottom:1px solid gray; text-align:right;margin-right:20px;">填报半年度：
			  	<select id="reportTime" <s:if test='!"add".equals(mode)'>disabled="disabled"</s:if>>
	  	...
	  	<s:if test="reportTypeInternalId==@com.tianque.domain.property.DirectoryReportType@YEAR_REPORT">
		  	<td colspan=3 class=head style="border:0px; border-bottom:1px solid gray; text-align:right;margin-right:20px;">填报年度：
	...
	<tr height=37 >
		<td colspan=2 height=37 class=bottom>签发人：<input  id="checkUser" type= "text"   maxLength= "12 " value="${disputEsexamine.dealPerson}"   name="disputEsexamine.dealPerson"  size= "12 "   style= "BORDER-BOTTOM:   #000000   1px solid; BORDER-LEFT:   #ffffff   0px   solid;   BORDER-RIGHT:   #ffffff   0px   solid;   BORDER-TOP:   #ffffff   0px   solid "/>  </td>
		<td colspan=2 class=bottom>制表人：<input id="createTabUser" type= "text"   maxLength= "12 " value="${disputEsexamine.lister}"   name="disputEsexamine.lister" size= "12 "   style= "BORDER-BOTTOM:   #000000   1px solid; BORDER-LEFT:   #ffffff   0px   solid;   BORDER-RIGHT:   #ffffff   0px   solid;   BORDER-TOP:   #ffffff   0px   solid "/> </td>
...
$("#reportTime option[value = '"+i+"']").remove();
	...
	function inputKeyOut(selfDoc){
		...
		//var TwonDealcount = ["totalTwonDealcount","totalVilDealcount","totalUnDealcount","totalYearDealcount"];
