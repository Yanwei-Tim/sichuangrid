...
<style>
	...
	#reportName , #selectedMonthForTown{
		text-align:center;border:0px;height:33.0pt;font-size:25px; font-weight:bold;
	}
</style>
<table class="tb" border=0 cellpadding=0 cellspacing=0 style='border-collapse:collapse;table-layout:fixed;width:750px'>
	<s:if test='"add".equals(mode)'>
		<tr>
			<td colspan=10 height=44 id="reportName" class=head >${dailyYear.yearDate}年<span id="selectedMonthForTown"></span>${organization.orgName}矛盾纠纷排查情况表</td>
		</tr>
	</s:if>
	<s:else>
		<tr>
...
<table class="tb" border=0 cellpadding=0 cellspacing=0 style='border-collapse:collapse;table-layout:fixed;width:750px'>
	<tr  height=43 style='height:32.25pt'>
		...
		<td colspan=4 class=head >层<span >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span>级</td>
	</tr>
	...
	<div class="tableCtt">
	<table id='tb' class="tb" border=0 cellpadding=0 cellspacing=0 style='border-collapse:collapse;table-layout:fixed;width:750px'>
	<tr id='c_0_' >
		<td width="32" height=26 class=order >1</td>
		<td colspan=2 rowspan=5 class=label style='width:176.5pt'>总<span
	...
	<tr id='c_0_' >
		...
		<td name="totalTwonDealrate" class="toDataJosn" countName="totalUnDealrate-data" countTarget="totalUnDealrate">${disputEsexamine.totalTwonDealrate}</td>
	...
	<tr id='c_0_' >
		...
		<td name="totalTwonFinishrate" class="toDataJosn" countName="totalUnFinishrate-data" countTarget="totalUnFinishrate">${disputEsexamine.totalTwonFinishrate}</td>
	...
	<tr id='c_413_' >
		...
		<td id='c_417_99_issuecount' class="toDataJosn"  name="ltdUnDealcount" countName="ltdUnDealcount">${disputEsexamine.ltdUnDealcount}</td>
		<td id='c_417_100_issuecount' class="toDataJosn" name="ltdYearDealcount">${disputEsexamine.ltdYearDealcount}</td>
	</tr>
	<tr id='c_414_' >
		...
		<td id='c_418_99_issuecount' class="toDataJosn"  name="environUnIssuecount" countName="environUnIssuecount">${disputEsexamine.environUnIssuecount}</td>
		<td id='c_418_100_issuecount' class="toDataJosn" name="environYearIssuecount">${disputEsexamine.environYearIssuecount}</td>
	</tr>
	<tr id='c_414_' >
	...
	<tr id='c_417_' >
		...
		<td id='c_417_99_issuecount' class="toDataJosn"  name="schoolUnIssuecount" countName="schoolUnIssuecount">${disputEsexamine.schoolUnIssuecount}</td>
		<td id='c_417_100_issuecount' class="toDataJosn" name="schoolYearIssuecount">${disputEsexamine.schoolYearIssuecount}</td>
	</tr>
	<tr id='c_417_' >
	...
	<tr id='c_429_' >
		...
		<td name="imptUnImptDealcount" countName="imptUnImptDealcount" class="toDataJosn" >${disputEsexamine.imptUnImptDealcount}</td>
	...
	<tr height=42 >
		...
		<td colspan=7 class=label>各市于每季度第一个月的10日前将此报表上报省综治办；县（市、区）于次月5日前上报市综治办。</td>
	</tr>
	</table>
	</div>
	<table id='tb' class="tb" border=0 cellpadding=0 cellspacing=0 style='border-collapse:collapse;table-layout:fixed;width:563pt'>
	<tr >
...
var selectLength = $("#reportTime").find("option").length;
	...
	function countNianDuLeiJi(selfDoc,inputKeyDownValue){
		...
		var rateCount=parseFloat(value)-parseFloat(inputKeyDownValue);
		var tdDoc = selfDoc.parent();
	...
	function inputKeyOut(selfDoc){
		...
		var issueCounts=["totalTwonIssuecount","totalVilIssuecount","totalUnIssuecount","totalYearIssuecount"];
		var dealCounts=["totalTwonDealcount","totalVilDealcount","totalUnDealcount","totalYearDealcount"];
		var dealRates=["totalTwonDealrate","totalVilDealrate","totalUnDealrate","totalYearDealrate"];
		var finishCounts=["totalTwonFinishcount","totalVilFinishcount","totalUnFinishcount","totalYearFinishcount"];
		var finishRates=["totalTwonFinishrate","totalVilFinishrate","totalUnFinishrate","totalYearFinishrate"];
		
		var imptIssueCounts=["imptTwonImptIssuecount","imptVilImptIssuecount","imptUnImptIssuecount","imptYearImptIssuecount"];
		var imptFinishCounts=["imptTwonImptFinishcount","imptVilImptFinishcount","imptUnImptFinishcount","imptYearImptFinishcount"];
		var imptFinishRates=["imptTwonImptFinishrate","imptVilImptFinishrate","imptUnImptFinishrate","imptYearImptFinishrate"];
		var imptFinishDealcount= ["imptTwonImptDealcount","imptVilImptDealcount","imptUnImptDealcount","imptYearImptDealcount"];
	...
	function paddingTdData(data){
		...
		$.each(toDataJosnTds,function(i){
			if(data==null){
				...
				if($(toDataJosnTds[i]).attr("name")=="totalVilDealrate"){
...
function onSelectChanged(){
	<s:if test="reportTypeInternalId!=@com.tianque.domain.property.DirectoryReportType@YEAR_REPORT">
	$("#selectedMonthForTown").text($("#reportTime").find("option:selected").text());
	</s:if>
	$.ajax({
...
function countChenggongLv(selfDoc,issueCounts,dealCounts,dealRates){
	...
	for(var i=0;i<issueCounts.length;i++){
		...
		}else if(value==""||tdDoc.attr("name")==dealCounts[i]){
			if(isNaN($("td[name='"+issueCounts[i]+"']").text())||parseFloat($("td[name='"+issueCounts[i]+"']").text())==0){
				$("td[name='"+dealRates[i]+"']").text(0);
			}else{
