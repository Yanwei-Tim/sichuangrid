...
var heightWhenAdd=400;
var currentOrgId = getCurrentOrgId();
var dailyId = '${dailyDirectoryId}';
$(document).ready(function(){
	var gridoption = {
		url:"${path}/workingRecord/securityPropagandaManage/securityPropagandaList.action?organization.id="+currentOrgId+"&dailyDirectoryId="+dailyId,
		datatype: "json",
		colNames:['id','宣传情况','附件','录入时间'],
	   	colModel:[
	...
	$("#add").click(function(event){
		...
		if(!selectedNode.attributes.leaf){
			...
			modal : true,
			url:"${path}/workingRecord/securityPropagandaManage/dispatch.action?mode=add"+"&dailyDirectoryId="+dailyDirectoryId,
			buttons :{
				"保存" : function(){
	...
	$("#update").click(function(event){
		...
		$("#dailyLogMaintanceDialog").createDialog({
			...
			url:"${path}/workingRecord/securityPropagandaManage/dispatch.action?mode=edit&securityPropaganda.id="+selectedId,
			buttons : {
				"保存" : function(){
					$("#dailyLog-form").submit();
				},
	...
	$("#reload").click(function(){
		$("#dailyLogList").setGridParam({
			url:"${path}/workingRecord/securityPropagandaManage/securityPropagandaList.action?organization.id="+currentOrgId,
			postData:{
...
function doubleClickTable(selectedId){
	$("#dailyLogMaintanceDialog").createDialog({
		...
		modal : true,
		url:"${path}/workingRecord/securityPropagandaManage/dispatch.action?mode=view&securityPropaganda.id="+selectedId,
		buttons :{
			"关闭" : function(){
				$(this).dialog("close");
			}
...
function showDailyLogAttachFile(){
	$.each($(".popUpMore"), function(i, n){
		$.ajax({
			url:"${path}/workingRecord/securityPropagandaManage/getSecurityPropagandaById.action?securityPropaganda.id="+$(n).attr("securityPropagandaId")+"&organization.id="+currentOrgId,
			success:function(dailylog){
				var popMoreData = [];
				for(var j = 0; j < dailylog.dailyAttachFiles.length; j++){
					popMoreData[j]={id:dailylog.dailyAttachFiles[j].id, url:'${path}/workingRecord/securityPropagandaManage/downloadDailyAttachFile.action?dailyAttachFile.id='+dailylog.dailyAttachFiles[j].id, text:dailylog.dailyAttachFiles[j].fileName,clickFun:function(){}};
				}
...
function formatterAttachFile(el,options,rowData){
	if(rowData.dailyAttachFiles.length>0){
		return "<div style='clear:both' align='center'><a href='javascript:;' id='securityPropaganda_"+rowData.id+"' style='color:#666666' class='popUpMore' securityPropagandaId='"+rowData.id+"' >附件>></a></div>";
	}
	return "";
}
...
function deleteDailyLog(selectedIds){
	$.ajax({
		url:'${path}/workingRecord/securityPropagandaManage/deleteSecurityPropaganda.action?securityPropagandaIds='+selectedIds+"&organization.id="+currentOrgId,
