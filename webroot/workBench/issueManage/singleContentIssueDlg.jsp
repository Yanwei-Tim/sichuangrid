...
<div id="dialog-form" title="事件处理领导批示" class="container container_24">
	<form id="singleContentDealForm" method="post" action="${path}/issues/issueManage/dealIssue.action">
			...
			<pop:token />
		    <div class="grid_5 lable-right"><em class="form-req">*</em>
			...
			<div class="grid_19">
				<textarea name="operation.content" class="xheditor-mfull" rows="12" cols="80" style="width: 99%" id="content" ></textarea>
			</div>
			<div class='clearLine'>&nbsp;</div>
			...
			$(form).ajaxSubmit({
				success:function(data){
					...
					if($("#DialogName").val()=='maxDeal'){
						$("#maxJurisdictionsList").setRowData($("#keyId").val(),data);
						$("#maxJurisdictionsList").setSelection(data.issueStepId);
						$("#maxIssueDialogforbench").dialog("close");
					}else{
						$("#jurisdictionsList").setRowData($("#keyId").val(),data);
