...
<div>
    <form action="${path}/serviceList/businessManageManage/replyBusinessManage.action?" method="post" id="maintainForm">
	...
	<div class="container container_24"> 
		...
		<div class="grid_8 lable-left">
			<input name='businessManage.reply.replyDate' value='<s:date name="reply.replyDate" format="yyyy-MM-dd HH:mm:ss"/>'  readonly="readonly" class="form-txt"/>
		</div>
		...
		<div class="grid_8 lable-left">
			<input name='businessManage.reply.replyPeople' value='${reply.replyPeople}'  readonly="readonly" class="form-txt"/>
		</div>
		...
		<div class="grid_20 heightAuto"  >
            <textarea rows="4" name="businessManage.reply.replyContent" maxlength="300" cols="" class="form-txt" <s:if test='"viewReply".equals(mode)'>disabled="disabled"</s:if> >${reply.replyContent}</textarea>
    	</div>
...
<s:if test="reply.replyAttchs!=null && reply.replyAttchs.size > 0">
	<s:iterator value="reply.replyAttchs" var="file">
	    var itemHtml = '<div id="item'+'${file.id}'
			+'" class="uploadifyQueueItem completed">'+'<a href="'+'${path}/serviceList/businessManageManage/downLoadReplyActualFile.action?attachFileId=${file.id}'
