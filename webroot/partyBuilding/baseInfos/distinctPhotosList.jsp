...
<ul class="photoList">
<s:iterator value="districtBasiccase.caseImageUploads" var="caseImageUpload">
<li class="param">
	...
	var imgId = ${caseImageUpload.id};
		$("#editPhotos_${caseImageUpload.id }").click(function(event){
			$("#districtBasicCaseDialog").createDialog({
				...
				height: 230,
				title:"编辑图片",
				url:"${path}/partyBuilding/baseInfos/maintainPhotosByDistrict.jsp?imgId="+imgId+"&orgId="+getCurrentOrgId()+"&type=second",
				buttons:{
					"保存" : function(event){
		   				$("#maintainForm").submit();
			   		},
		...
		$("#addPhotos").click(function(event){
	 		$("#districtBasicCaseDialog").createDialog({
	 				...
	 				height: 230,
	 				title:"编辑图片",
	 				url:"${path}/partyBuilding/baseInfos/maintainPhotosByDistrict.jsp?orgId="+getCurrentOrgId()+"&type=second",
	 				buttons:{
	 					"保存" : function(event){
	 		   				$("#maintainForm").submit();
	 			   		},
