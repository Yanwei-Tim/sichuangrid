...
$(function(){
	...
	$("#add").unbind().click(function(){
		...
		if(checkLayerDataIsDrawn(parentOrgId)){
			...
			$("#systemMap").TqMap("drawFeatureMeasure",{
				type:"polygon",
				featureClickMapEnd:splitPolygonEndFunc//画面结束
			});
		}
	});
	...
	$("#delete").unbind("click").click(function(){
		...
		$.confirm({
			...
			okFunc: function() {
				deleteFeature($("#currentOrgId").val(),1);
			}
		});
	});
		...
		$.confirm({
			...
			okFunc: function() {
				...
				if(layerData!=null&&layerData!="" ){
					...
					deleteFeature($("#currentOrgId").val(),0);
				}
	...
	function fullScreen(){//全屏
		...
		var imap = $("#"+option.mapId).TqMap("get","map")
...
function splitPolygonEndFunc(event){
	var parentOrgId = $("#parentOrgId").val(),
	...
	var layerPoints = event.geometry+"";
	if(!checkPolygonsInPolygon(childFeatures,event)){
		$.messageBox({message:"区域必须包含下级的管辖区域，请重新划分区域!",level:"error"});
		return false;
	}
	getPolygonIntersection(parentFeature,event);
}
...
// 	}
	if(currentFeature){
		getPolygonIntersection(parentFeature,currentFeature);
	}else{
		 $.messageBox({message:"修改地图区域并保存成功!"});
		 afterChangNode(currentOrgNode);
	}
}

//修改图形时拖动区域边点事件
function dragFeatureEndFunc(feature,lonlat){
	var currentOrgId = $("#currentOrgId").val();
	var childFeatures = $("#systemMap").TqMap("getFeaturesBy",{ key:"data.parentOrgId", value:currentOrgId});
//     for(var i=0;i<childFeatures.length;i++){
//     	if(checkPointInPolygonPoints({x:lonlat.lon,y:lonlat.lat},childFeatures[i].geometry+"")){
// 			$.messageBox({message:"区域必须包含下级的管辖区域，请重新划分区域!",level:"error"});
...
function mapClick(e){
	var imap = $("#systemMap").TqMap("get","map");
...
function deleteFeature(orgId,type){
	var url = "/system/gis2DLayerManage/deleteSameLevelLayerByOrgId.action";//本级删除
	if(type==1){//下辖删除
		url = "/system/gis2DLayerManage/deleteGis2DLayerByOrganizationId.action";
	}
	$.ajax({
		url:PATH+ url + "?organization.id=" + orgId,
		success: function(data) {
			if(data==true){
				$.messageBox({
	                message:"辖区删除成功"
...
function addEndFunc(feature,geometry){
	$("#systemMap").TqMap("unregisterEvent",{eventName:"mousemove",func:mouseTipFun});
	$("#systemMap").TqMap("unregisterEvent",{eventName:"mousemove",func:markerMousemove});
	layerPoints = geometry+"";
	$("#systemMap").TqMap("deactivateMeasureControl");
...
function getPolygonIntersection(parentFeature,feature){
	if(parentFeature && feature){
		$("#systemMap").TqMap("addLoadingPanel");
		var intersectionMode = "threadSu";
		$.ajax({
			async:true,
			type:"post",
			url:PATH+"/gisUtilManage/intersectPolygon.action",
			data:{
				gisType:TQMap.gisType,
				orgId:$("#parentOrgId").val(),
				points:feature.geometry+"",
				mode:intersectionMode
			},
			success:function(data){
				if("threadSu" === intersectionMode){
					getPolygonById(feature,data);
				}else{
					if(!data || data.indexOf("POLYGON")<0){
						$.messageBox({message:(data?data:"面切分的结果为空集合，请重新切分!")});
						onLayerOrgChanged();
						return;
					}
					addEndFunc(feature,data);
					$("#systemMap").TqMap("removeLoadingPanel");
				}
			}
		})
	}else if(feature){
		addEndFunc(feature,feature.geometry+"");
	}
}

function getPolygonById(feature,id){
	if(id==null){
		$.messageBox({message:"面切分ID为空，请重新划分区域或联系管理员!"});
		onLayerOrgChanged();
		return null;
	}
	$.ajax({
		url:PATH+"/gisUtilManage/getPolygonById.action?keyId="+id,
		success:function(data){
			if(data!=null){
				if(!data || data.indexOf("POLYGON")<0){
					$.messageBox({message:(data?data:"面切分的结果为空集合，请重新切分!")});
...
function getGis2DLayerDataByOrgId(orgId){
	var result="";
	$.ajax({
		method:'post',
		async:false,
