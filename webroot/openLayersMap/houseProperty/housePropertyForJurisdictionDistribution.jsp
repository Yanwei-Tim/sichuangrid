...
function showHousePropertyLayerStaticInfo(){
	...
	if("<s:property value='#parameters.flag'/>"){//判断是否右边列表点楼宇 true则为是的情况
		...
		boundHouseList(orgId,housePropertyType,modeType);
		$("#jurisdictionDistribution").click();
		return;
	}
}

function boundHouseList(orgId,housePropertyType,modeType){
	$.ajax({
		...
		success:function(data){
			...
			for(var i=0;i<data.length;i++){
				var color = loadFeature(data[i],i,housePropertyType,modeType);
				if(data[i].lon!=null && data[i].lat!=null){
					var pop=$("#map").TqMap("addPopupText",{
						 ...
						 popupTextId:i+1,
						 data:{orgId:data[i].organization.id,typeName:housePropertyType,modeType:modeType},
						 autoSize:true,
						 isBgImg:true
					});
					$("#map").TqMap("registerEvent",{objectName:"popupText",eventName:"click",func:houseJurisdictionDetail});
...
function houseJurisdictionDetail(e){
	...
	var mainTableName="<s:property value='@com.tianque.openLayersMap.util.GisGlobalValueMap@HOUSEINFO_TABLENAME'/>";
