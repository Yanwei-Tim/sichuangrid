...
<script type="text/javascript">
	...
	var useractivatereportlist_url = "/userActivateReportManage/getUserActivateReportList.action?year=";
	var useractivatereportsort_url = "/userActivateReportManage/userActivateReportSort.action?year=";
	var createuseractivatereportlist_url = "${path}/userActivateReportManage/createUserActivateReportList.action?year=";
var grid = null;
var sortName="";
var sortColumn="";
...
function getUserActivateReportList(orgLevelDistance){
	var url = useractivatereportlist_url+$("#year").val()+"&month="+$("#month").val();
...
$(document).ready(function(){
	...
	var columns = [
		...
		sortName = name;
		sortColumn=sort;
		var url = useractivatereportsort_url+$("#year").val()+"&month="+$("#month").val()+"&sortName="+name+"&sort="+sort+"&orgLevelDistance="+$("#type").val();
		$.ajax({
			url:url,
			success:function(data){
				if(data == null){
					$.messageBox({
						message: "网格化服务管理工作情况通报表（一）排序失败",
						level: "error"
					});
					return;
				}
	...
	var nowMonth=nowDate.getMonth()+1;
		...
		$.ajax({
			url:createuseractivatereportlist_url+$("#year").val()+"&month="+$("#month").val(),
	...
	$(".print").click(function(){
		var url = "${path}/statAnalyse/evaluation/userActivateReportListPrint.ftl?year="+$("#year").val()+"&month="+$("#month").val()+"&sortName="+sortName+"&sort="+sortColumn+"&orgLevelDistance="+$("#type").val();
