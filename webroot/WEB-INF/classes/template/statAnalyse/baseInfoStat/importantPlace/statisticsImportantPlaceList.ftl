...
$(document).ready(function(){
	...
	$(".print").click(function(){
		$("#placeStuationDlg").createDialog({
			...
			height:450,
			title:"打印重点场所总况",
			url:"${(path)!}/statAnalyse/baseInfoStat/importantPlace/importPalceSituationPrint.ftl?parentOrgId="+getCurrentOrgId()+"&year="+$("#queryYear").val()+"&month="+$("#queryMonth").val()+"&typeTableName=IMPORTANTPLACE",
			buttons: {
			   "打印" : function(){
				print();
		  	   },
