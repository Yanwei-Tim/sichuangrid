...
var specialGroupGrid = null;
var orgId=getCurrentOrgId();
function getList(){
	$.ajax({
		url:'${path }/plugin/taskListManage/common/getSpecialGroupSumAndVisitList.action?orgId='+orgId+'&searchType='+$("#searchType").val()+'&year='+$("#year").val()+'&month='+$("#month").val()+'&week='+$("#week").val(),
		success:function(data){
			specialGroupGrid.bindData(data);
		}
	})
}
...
$(document).ready(function(){
	...
	var context = {};
	specialGroupGrid = new SigmaReport("<@s.property value='#parameters.type'/>Gridbox",context,columns, "SigmaReportPrint","printTable_title");
	getList();
})
</script>
