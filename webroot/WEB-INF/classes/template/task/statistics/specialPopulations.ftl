...
<div class="zt_tabs_style">
	<div id="chartsTabs">
			<ul>
				<li id="chartsTabs1" class="taskTit"><a href="${path }/task/statistics/specialPopulationsColumn.ftl">区域分布图</a></li>
				<li id="chartsTabs2" class="taskTit"><a href="${path }/task/statistics/specialPopulationsPie.ftl">类型分布图</a></li>
				<li id="chartsTabs3" class="taskTit"><a href="${path }/task/statistics/specialPopulationsTrend.ftl">趋势图</a></li>
				<#--<li id="chartsTabs4" class="taskTit"><a href="${path }/task/statistics/specialPopulationsList.ftl">特殊人群报表</a></li>-->
			</ul>
		</div>
	</div>
</div>
...
function checkBoxType(){
	var bo = true;//判断循环第一次
	var checkValue = "";
	<@s.iterator value="@com.tianque.plugin.statistics.constants.TypeConstants@specialMapByName">
		if(bo){
			bo = false;
...
function checkBoxIsChecked(){
	$("[name='checkNameColumn']").click(function(){
		if($(this).val() != "special"){
			$("#specialColumn").removeAttr("checked");
			changeCheckBox();
		}else{
			changeCheckBox(0);
		}
...
$(document).ready(function(){
	...
	$("input[name='statusValuePie']").click(function(){
		if($(this).attr("checked")){//选中状态
			...
			$("[name='checkNameColumn']:checked").each(function(){
				if($(this).val() == "special" || clickNum > 1){
					statusClick(statusVal);
					return;
				}
				clickNum++;
