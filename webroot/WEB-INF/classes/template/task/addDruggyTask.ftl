...
<div class="container container_24">
	...
	<div class="grid_5">
		<input type="text" name="druggyTask.familyTel" maxlength="11" id="place" value="${(druggyTask.familyTel)!}"  style="width:99%"  class="form-txt {required:true,phoneAndMobile:true,messages:{required:'家属联系电话不能为空',phoneAndMobile:$.format('请输入以固定电话：028-87653333或者手机：15102888888为格式的号码')}}" />
	</div>
	<div class='clearLine'>&nbsp;</div>
	...
	<div class="grid_15 heightAuto" style="margin-top:5px;">
		<input type="text" id="helpPeople" name="druggyTask.helpPeople" style="height:70px;"  class='form-txt' />
	     ...
	     <div class="grid_5">
	          <select  name="druggyTask.lifeResource.id" id="lifeResource"  class="form-select {required:true,messages:{required:'生活来源方式不能为空'}}">
	...
	<div class="grid_15 ">
		<input type="text" name="druggyTask.place" maxlength="50" id="place" value="${(population.currentAddress)!}" style="width:99%"  class="form-txt {required:true,exculdeParticalChar:true,messages:{required:'地点不能为空',exculdeParticalChar:'不能输入非法字符'}}"/>
	</div>
	<div class='clearLine'>&nbsp;</div>
	...
	<div class="grid_15 ">
		<input type="radio" name="druggyTask.hasException" checked="checked" value="1" onclick="s(this.value)" />有 <input name="druggyTask.hasException" type="radio" value="0" onclick="s(this.value)" />无
	</div>
	<div class='clearLine'>&nbsp;</div>
