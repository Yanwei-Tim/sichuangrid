...
<script type="text/javascript">
		...
		jQuery(document).ready(function() {
			var tableHeight=($(".ui-layout-center").height()-$(".btnbanner").height()-190)/2;
			...
			var layout=function(){
				...
				var wrapHeight=($(".ui-layout-center").height()-$("#thisCrumbs").height()-130)/2;
		...
		function onOrgChanged(){
			$("#areaDatatable").setGridParam({
				url:"${path}/baseinfo/leaderViewManage/personGeneralCondition.action",
				datatype:"json",
				page:1,
		...
		function getPersonMonthGeneralConditionData(){
			$("#personMonthGeneralConditionTable").setGridParam({
				url:"${path}/baseinfo/leaderViewManage/monthGeneralCondition.action",
				datatype:"json",
				page:1,
				loadComplete:function(){
			...
			$("#personMonthGeneralConditionTable").setPostData({
		    	"orgId":getCurrentOrgId(),
		    	"tableType":"all_attention_population"
		   	});
