...
<div class="content">
	<div class="ui-corner-all contentNav" id="nav">
			...
			<select id="teamMemberId" class="basic-input" name="serviceMemberVo.teamMember">
				<option value="" selected>全部</option>
				<option value="1">服务团队成员</option>
				<option value="0">监管(护)人员</option>
			</select>
		<span class="lineBetween "></span>
		<@pop.JugePermissionTag ename="addServiceTeamMember">
		<a id="addServiceTeamMember" href="javascript:void(0)"><span>新增团队成员</span></a>
		</@pop.JugePermissionTag>
		<a id="addGuardersForObject"  href="javascript:void(0)"><span>新增监护人员</span></a>
...
<@pop.formatterProperty name="position" domainName="@com.tianque.plugin.serviceTeam.util.Constants$ServiceTeamPositionPropertyTypes@TEAMPOSITION" />
	...
	$("#serviceTeamMemberList").jqGridFunction({
		...
		colModel:[
	      ...
	      {name:"position",label:'职务(身份)',sortable:false,formatter:positionOrRelationFormatter,width:80,align:"center"},
		  {name:'teamName',label:'所在团队',sortable:false,width:150},
	      {name:'mobile',label:'联系手机',sortable:false, width:150},
	...
	$("#addServiceTeamMember").click(function(event){
		$("#_serviceTeamMemberDialog").createDialog({
			...
			width: 700,
			height: 450,
			url:'${path}/plugin/serviceTeam/router/routerManage/maintainServiceMemberForPopulation.action?mode=add&dailogName=_serviceTeamMemberDialog&serviceTeamMemberVo.org.id='+getCurrentOrgId(),
			buttons: {			
				"关闭" : function(){
		        	$(this).dialog("close");
	...
	$("#reloadTeamMemberList").click(function(){ 
		...
		$("#teamMemberId").val($("#teamMemberId").find(":eq(0)").val());
		loadTeamMembers();
	});
	
	//加载服务成员
	function loadTeamMembers() {
	var personData = $("#"+$('#module').val()+"List").getRowData(getSelectedIdWhenUpdate);	
		var listParam = null;
		
			listParam = {
				'serviceMemberVo.objectType':$("#_populationType_").val(),
				'serviceMemberVo.objectId':personData.id,
				'serviceMemberVo.onDuty':$("#onDutyId").val(),
				'serviceMemberVo.teamMember':$("#teamMemberId").val()
			};
