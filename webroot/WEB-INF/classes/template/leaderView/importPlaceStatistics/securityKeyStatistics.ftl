...
var title = "治安重点";
		jQuery(document).ready(function() {		
			...
			$("#searchHighA").click(function(){
				$("#statisticsDialog").createDialog({
					...
					modal:true,
					url: '${(path)!}/baseinfo/siteinfo/enterprise/searchSecurityCondition.jsp?keyType='+"securityKey",
					buttons: {
						"查询" : function(event){
		...
		function getDialogParam(isSearch){
			return {
				...
				title: title+'信息',
				modal:true,
				url: '${(path)!}/baseinfo/siteinfo/enterprise/securityEnterpriseListForStatistics.jsp?isSearch='+isSearch+'&keyType='+"securityKey",
				buttons: {
			   		"关闭" : function(){
			        	$(this).dialog("close");
			   		}
