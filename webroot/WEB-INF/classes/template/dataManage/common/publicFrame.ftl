	...
	<div class="btnbanner btnbannerData">
		<div class="ui-widget autosearch">
			<#if bigType=="population">
				<input type="text" class="basic-input" value="请输入姓名或身份证号码" id="searchCurrentlyAddress" class="searchtxt" maxlength="18" onblur="value=(this.value=='')?'请输入姓名或身份证号码':this.value;" onfocus="value=(this.value=='请输入姓名或身份证号码')?'':this.value;"/>
			<#elseif bigType=="dustbin">
				<input type="text" class="basic-input" value="请输入部件标识码" id="searchCurrentlyAddress" class="searchtxt" maxlength="18" onblur="value=(this.value=='')?'请输入部件标识码':this.value;" onfocus="value=(this.value=='请输入部件标识码')?'':this.value;"/>
			<#elseif bigType=="builddatas">
				<input type="text" class="basic-input" value="请输入楼宇名称或地址" id="searchCurrentlyAddress" class="searchtxt" maxlength="18" onblur="value=(this.value=='')?'请输入楼宇名称或地址':this.value;" onfocus="value=(this.value=='请输入楼宇名称或地址')?'':this.value;"/>
			<#else>
				<input type="text" class="basic-input" value="请输入<#if bigType=="location">场所名称或<#else>房屋</#if>地址" id="searchCurrentlyAddress" class="searchtxt" maxlength="18" onblur="value=(this.value=='')?'请输入<#if bigType=="location">场所名称或<#else>房屋</#if>地址':this.value;" onfocus="value=(this.value=='请输入<#if bigType=="location">场所名称或<#else>房屋</#if>地址')?'':this.value;"/>
			</#if>
...
function onOrgChanged(){
	...
	if("请输入姓名或身份证号码"==$("#searchCurrentlyAddress").val() ||
		"请输入场所名称或地址"==$("#searchCurrentlyAddress").val()||
		"请输入房屋地址"==$("#searchCurrentlyAddress").val() ||
		"请输入部件标识码"==$("#searchCurrentlyAddress").val()||
		"请输入楼宇名称或地址"==$("#searchCurrentlyAddress").val()){
		initParam = {
...
$(document).ready(function(){
	...
	            dataJson[data[i].name] = data[i].value;
		...
		$("#${lowerCaseModuleName}List").setPostData($.extend({"searchVo.searchType":0,"mode":modes},dataJson));
		$("#${lowerCaseModuleName}List").trigger("reloadGrid");
	}
	...
	var lastGridOption = {
		colModel:[
	        ...
	        {name:"claimDetail.claimState",sortable:true,width:100, formatter : claimStateFormatter,label:"认领状态",align:"center"},
	...
	$("#import").click(function(event){
		$("#noticeDialog").createDialog({
			...
			height: 230,
			title:"数据导入",
			url:"${path}/common/import.jsp?isNew=1&dataType=${lowerCaseModuleName}&dialog=noticeDialog&startRow=${(importStartRow)!}&templates=" + getImportTemplateKey("${upperCaseModuleName}")+"&listName=${lowerCaseModuleName}List",
			buttons:{
				"导入" : function(event){
					$("#mForm").submit();
				},
...
function updateOperator(event,selectedId){
		...
		if(ent.claimState==10||ent.claimState=='10'){
			 $.messageBox({level:'warn',message:"该${moduleCName}已经认领，不能修改!"});
			 return;
		}
	...
	function hasIdCardNo(){
		...
		<#--如果是未落户人员就直接返回true-->
		if("unsettledPopulationTemp"=='${(lowerCaseModuleName)}'){
			return true;
		}
			...
			okFunc: function() {
				$.ajax({
					...
					success:function(responseData){
					    ...
					    $.messageBox({message:"已经成功撤销认领${moduleCName}!"});
					    onOrgChanged();
					}
				});
		    }
	...
	$.confirm({
		...
		okFunc: function() {
			$.ajax({
				...
				success:function(data){
					if(null==data){
				    	$.messageBox({message:"已经成功删除该${moduleCName}!"});
					    disableButtons();
						$("#${lowerCaseModuleName}List").trigger("reloadGrid");
				    }else{
...
function getSearchName(){
	var searchName="";
	<#if bigType=="population">
		searchName="请输入姓名或身份证号码";
	<#elseif bigType=="location">
		searchName="请输入场所名称或地址";
	<#elseif bigType=="house">
		searchName="请输入房屋地址";
	<#elseif bigType=="dustbin">
		searchName="请输入部件标识码";
	<#elseif bigType=="builddatas">
		searchName="请输入楼宇名称或地址";
	<#else>
		searchName="bigType未设置";
	</#if>
	return searchName;
}
