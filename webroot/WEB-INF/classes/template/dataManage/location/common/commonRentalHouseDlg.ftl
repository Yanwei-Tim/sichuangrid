...
<@s.include value="/includes/baseInclude.jsp"/>
			...
			<div class="grid_7">
				<input type="text" name="houseInfo.registDate" id="registerDate" maxlength="32"  value='<@s.date name="houseInfo.registDate" format="yyyy-MM-dd" />'
			...
			<div class="grid_7">
				<input type="text" name="houseInfo.lessorDate" id="lessorDate" maxlength="50" value="<@s.date name="houseInfo.lessorDate" format="yyyy-MM-dd" />" class="form-txt {lessorDate:true,messages:{lessorDate:'出租时间小于登记日期'}}" readonly/>
			</div>
				<div class='clearLine'>&nbsp;</div>
			<div class="grid_4 lable-right">
			...
			<div class="grid_7">
				<input type="text" name="houseInfo.rentMonth" id="rentMonth" maxlength="9" value="${(houseInfo.rentMonth)!}"
					class="form-txt {number:true,messages:{number:'请输入合法的数字'}}" />
			</div>
			...
			<div class="grid_20">
				<input type="checkbox" id="isSignGuarantee" name="houseInfo.isSignGuarantee" value="1"   <@s.if test='1==houseInfo.isSignGuarantee'>checked="checked"</@s.if> />
				<label class="form-check-text">是否签订治安责任保证书 </label>&nbsp;

				<input type="checkbox" id="isSafetyChannel" name="houseInfo.isSafetyChannel" value="1"   <@s.if test='1==houseInfo.isSafetyChannel'>checked="checked"</@s.if> />
				<label class="form-check-text">有无安全通道</label>&nbsp;

				<input type="checkbox" id="isFireChannels" name="houseInfo.isFireChannels" value="1"   <@s.if test='1==houseInfo.isFireChannels'>checked="checked"</@s.if> />
		...
		maxDate:'+0d',
	   	onSelect:function(dateText, inst) {
			if(dateText!=null&&dateText!=''){
				var dateUnit=dateText.split('-');
				var date=new Date(dateUnit[0],dateUnit[1]-1,dateUnit[2]);
				$("#registerDate").datepicker("option", "maxDate",date);
			}
		}
	});
