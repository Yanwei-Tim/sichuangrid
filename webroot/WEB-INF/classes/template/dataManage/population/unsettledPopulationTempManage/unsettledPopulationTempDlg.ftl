...
<div id="unsettledPopulation" class="container container_24">
		...
		<div class="grid_7">
	    	...
	    	<@s.if test="unsettledPopulation.id">readonly </@s.if>
	    	<@s.else>title="请输入15或18位由数字或字母X组成的身份证号码"</@s.else>
		...
		<div class="grid_7">
			...
			class="form-txt {required:true,exculdeParticalChar:true,minlength:2,messages:{required:'请输入姓名',exculdeParticalChar:'姓名只能输入字母，数字和中文字符',minlength:$.format('姓名至少需要输入{0}个字符')}}"/>
		</div>
		<div class='clearLine'></div>
		...
		<div class="grid_7">
			<input type="text" name="unsettledPopulation.workUnit" id="workUnit" class="form-txt" value="${unsettledPopulation.workUnit}" maxlength="50"/>
		</div>
		<div class='clearLine'></div>
        ...
        <div class='clearLine'>&nbsp;</div>
        <@s.include value="${path}/baseinfo/commonPopulation/commonSearchAddress.jsp">
			<@s.param name="name">unsettledPopulation</@s.param>
		</@s.include>
   		<div class='clearLine'>&nbsp;</div>
		...
		<div class="grid_18" style="height: 120px">
			<textarea rows="3" cols="" id="unsettledPopulation-remark" name="unsettledPopulation.remark" 
			class="form-txt {maxlength:300,messages:{maxlength:$.format('备注最多只能输入{0}个字符')}}" >${unsettledPopulation.remark}</textarea>
		</div>
		<div class='clearLine'>&nbsp;</div>
...
$(document).ready(function(){
	...
	$("#unsettledPopulation-idCardNo").blur(function(value){
		if(value=="" || value==null){
			//$("#unsettledPopulation-idCardNo").errorInput.style{
			//	border: none;
			//}
		}
	});
