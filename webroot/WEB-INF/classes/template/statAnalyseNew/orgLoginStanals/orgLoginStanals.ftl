...
function getmonth(){
	$.ajax({
		async: false,
		url: "${path }/stat/currentTimeNew/getCurrentTimeForMonthToSpecial.action?currenYear="+$("#statYear").val(),
...
$(document).ready(function(){
	...
	$("#produceDate").click(function(event){
		$("#MainDialog").createDialog({
			...
			height: 200,
			title: "部门登录信息数据重新生成",
			url: "${path}/statAnalyse/orgLoginStanalsManagerNew/dispatch.action?mode=produceDate"+"&orgId="+getCurrentOrgId(),
			buttons: {
		   		"生成" : function(event){
	...
	$("#export").click(function(){
		var url = '${path}/statAnalyse/orgLoginStanalsManagerNew/downloadOrgLoginStanals.action?orgId='+getCurrentOrgId()+'&nowYear='+$("#statYear").val()+'&nowMonth='+$("#statMonth").val()+'&internalId='+ $("#internalId").val();
		downloadFile(url);
	});
...
function reloadReportDate(){
	$.ajax({
		async: false,
		url: '${path}/statAnalyse/orgLoginStanalsManagerNew/findOrgLoginStanalsByOrgIdForListPage.action',
		data:{
