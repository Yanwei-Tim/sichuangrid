...
<div id="lettingHousePrint" style="overflow: auto;height:100%;width:100%">
<style type="text/css">
...
<div id="lettingHouseGridbox" class="SigmaReportPrint" style="overflow: auto;height:100%;width:100%">
</div>
</div>
<script type="text/javascript">
var lettingHouseGrid = null;
var orgName;
function getLettingHouse(){
	$.ajax({
		url: "${path}/baseinfo/statAnalysePlaceNew/findStatAnalysePlace.action?orgId="+'<@s.property value="#parameters.parentOrgId"/>'+"&keyType="+'<@s.property value="#parameters.keyType"/>'+'&year='+'<@s.property value="#parameters.year"/>'+'&month='+'<@s.property value="#parameters.month"/>',
		success:function(responseData){
			lettingHouseGrid.bindData(responseData);
		}
	});
}
...
$(document).ready(function(){
	$.ajax({
		...
		success:function(data){
			...
			$("#title_lettingHouseGridbox").children().remove();
			$("#title_lettingHouseGridbox").html(data.orgName+"出租房报表");
		}
	})
	var context = {};
	var columns = [
	...
	lettingHouseGrid = new SigmaReport("lettingHouseGridbox",context,columns, "SigmaReportPrint","printTable_title");
	getLettingHouse();
})
</script>
