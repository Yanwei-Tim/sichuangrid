...
function getStatisticsPopulationList(orgLevelDistance,populationType){
	var url = "/baseInfoStat/statisticsPopulationNew/getStatisticsPopulationList.action?orgId="+getCurrentOrgId()+"&year="+$("#year").val()+"&month="+$("#month").val()+"&populationType="+populationType;
...
$(document).ready(function(){
	...
	$("#createStatistic").click(function(){
		var populationType='<@s.property value="#parameters.populationType[0]"/>';
		$.ajax({
			url:'/baseInfoStat/statisticsPopulationNew/createStatisticsPopulationList.action?orgId='+getCurrentOrgId()+"&year="+$("#year").val()+"&month="+$("#month").val()+"&populationType="+populationType,
	...
	$(".print").click(function(){
		var populationType='<@s.property value="#parameters.populationType[0]"/>';
		var url = "${path}/statAnalyseNew/baseInfoStat/population/populationSituationPrint.ftl?parentOrgId="+getCurrentOrgId()+"&year="+$("#year").val()+"&month="+$("#month").val()+"&populationType="+populationType;
		if($("#type").val() == '2'){
