...
<div id="dialog-form" title="新增" class="container container_24">
	<div id="tabs" style="height:400px;">
	   	<div id="positiveInfoDiv" class="container container_24">
	  		<form id="positiveInfoForm" method="post" action="${path}/baseinfo/qPersonnelManage/addQPersonnel.action" >
	  		<pop:token />
			...
			<jsp:include page="/baseinfo/commonPopulation/populationInfo/qPersonnel.jsp"></jsp:include>	
			</form>
	  	</div>
	</div>
</div>
			...
			$(form).ajaxSubmit({
				...
				success:function(data){
					...
					if("add"=="<s:property value='#parameters.mode[0]'/>"){
						if($("#isLock").val()!='1'){
	    				 	$("#qPersonnelList").addRowData(data.id,data,"first");
		                	$("#qPersonnelList").setSelection(data.id);
		                }
	                ...
	                if("edit"=="<s:property value='#parameters.mode[0]'/>"){
	                	 ...
	                	 if(data.death){
               				if($("#isLock").val()=='0') {
               					$("#qPersonnelList").delRowData(data.id);
							}else {
								 $("#qPersonnelList").setRowData(data.id,data);
								 $("#"+data.id+"").css('color','#778899');
							}
						}else{
							$("#qPersonnelList").setRowData(data.id,data);
						}
						 $("#qPersonnelList").trigger("reloadGrid");
