...
<%@ include file="/includes/baseInclude.jsp" %>
	...
	<input id="populationType" type="hidden" name="population.populationType" value="${locationType}" />
		<div id=tabs>
			<ul>
				<li><a href="${path}/baseinfo/internetBarManage/viewPublicPlace.action?location.id=${location.encryptId}">网吧</a> </li>
	...
	$(function() {
		$( "#tabs" ).tabs();
		/*$.ajax({
			url:"${path}/baseinfo/internetBarManage/viewPublicPlace.action?location.id=${location.id}",
			success:function(data){
				$("#internetBarInfo").html("");
				$("#internetBarInfo").html(data);	
			}
			});	*/
	});
	</script>