...
<input name="isSubmit" id="isSubmit" type="hidden" value="">
		<div class="grid_5 lable-right">
 		...
 		<s:if test='"actualHouseMaintanceDialog".equals(#parameters.dailogName[0])'>
 		<div class="grid_4 lable-right">
 			<input type="checkbox" id="isRentalHouse"  name="houseInfo.isRentalHouse" value="true" <s:if test="true==houseInfo.isRentalHouse">checked="checked"</s:if>/>
 		...
 		<div class="grid_5 lable-right">
			...
			<div class="grid_7">
				<input type="text"  id="_address"  <s:if test="isUseFrom==null">readonly</s:if> name="houseInfo.address"  maxlength="50"  value="${houseInfo.address }"
			...
			<div class="grid_8">
				<input type="text" name="houseInfo.houseAddress" id="houseAddress" maxlength="60" value="${houseInfo.houseAddress}"
					class="form-txt {exculdeParticalChar:true,minlength:2,maxlength:60,isLawful:true,messages:{exculdeParticalChar:'不能输入非法字符',minlength:'房产证地址至少需要输入2个字符',maxlength:'房产证地址最多需要输入60个字符',isLawful:'您输入了非法脚本请重新输入!'}}" />
			</div>
			<div class='clearLine'>&nbsp;</div>
			<div class="grid_5 lable-right">
			...
			<div class="grid_8">
				<input type="text" name="houseInfo.houseOwner" id="houseOwner" maxlength="30" value="${houseInfo.houseOwner}"
			...
			<div class="grid_5 lable-right">
				<label class="form-lb1"><s:if test="'rentalHouseMaintanceDialog'==#parameters.dailogName[0]">出租房面积</s:if><s:else>本户建筑面积</s:else>：</label>
			</div>
			<div class="grid_6">
				<input type="text" name="houseInfo.houseArea" id="houseArea" maxlength="15" value="${houseInfo.houseArea}"
					class="form-txt {decimal:true,range:[0,9999999.99],messages:{decimal:'请输入非负数 ，保留两位小数点',range: '请输入0至9999999.99之间的非负数 '}}" maxlength="10" />
			</div><div class="grid_1">m<sup>2</sup></div>
			<div class="grid_4 lable-right">
				<label class="form-lb1"><s:if test="'rentalHouseMaintanceDialog'==#parameters.dailogName[0]">出租房结构</s:if><s:else>房屋结构</s:else>：</label>
			</div>
			<div class="grid_8">
			...
			<div class="grid_8">
				<input type="text" name="houseInfo.houseRightNumber" id="houseRightNumber" maxlength="30" value="${houseInfo.houseRightNumber}"
			...
			<div class="grid_19">
				<textarea rows="4" name="houseInfo.remark" id="remark" class="form-txt {minlength:2,maxlength:300,isLawful:true,messages:{minlength:'备注至少需要输入2个字符',maxlength:'备注最多需要输入300个字符',isLawful:'您输入了非法脚本请重新输入!'}}" style="width: 99%">${houseInfo.remark }</textarea>
			</div>
	...
	function procRentalHouseTab(doc){
		if(doc.attr("checked")){
			...
			$("#"+dialogName).addTabToDialog(
					{
						title:"出租房",
						url:'${path}/baseinfo/rentalHouseManage/prepareDispatchByEncrypt.action?orgId='+currentOrgId+'&dailogName=<s:property value="#parameters.dailogName[0]"/>&mode=${mode}&houseInfo.houseCode=${houseInfo.houseCode}&houseInfo.organization.id=${houseInfo.organization.id}',
						index:1
					}
				)
				if("${id}" == null){
