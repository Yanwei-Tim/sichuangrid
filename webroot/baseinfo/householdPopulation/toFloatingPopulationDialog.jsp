...
<div >
	<table id="toFloatingPopulationList" ></table>
</div>		
...
function deathFloatingPopulationPrompt(rowDatas){
	...
	for(var i = 0; i < rowDatas.length; i++){
		...
		if(rowDatas[i].death=="true"){
			deathMassage += "<p>"+rowDatas[i].name+"，"+idCardNo+"</p>";
		}else{
			massageInfo += "<p>"+rowDatas[i].name+"，"+idCardNo+"。</p>"
			populationIds += (rowDatas[i].id+",");
		}
	}
	if(deathMassage != null && deathMassage != ""){
		$.messageBox({message:"以下人员死亡："+deathMassage+"不能进行操作!",level:"warn"});
		return;
	}
	for(var i = 0; i < rowDatas.length; i++){
		var idCardNo = rowDatas[i].idCardNo;
...
function toFloatingPopulations(){
	var selectedFloatingPopulationId = getSelectedIdsByTabId("toFloatingPopulationList");
	var seldatas = new Array();
	for(var i = 0; i < selectedFloatingPopulationId.length; i ++ ){
		var seldata = $("#toFloatingPopulationList").getRowData(selectedFloatingPopulationId[i]);
		seldatas[i] = seldata;
	}
	...
	$.confirm({
		title:"转为流动人口提示",
		message:"是否把“户籍人口：<div style='text-align:left'>"+massageInfo+"</div> 转为流动人口？",
		okFunc: function(){
			$.dialogLoading("open");
			$.ajax({
				url:PATH+'/baseinfo/householdStaff/toFloatingPopulationByIds.action?&populationIds='+populationIds,
				success:function(data){
					$.dialogLoading("close");
					$("#householdStaffList").trigger("reloadGrid");
					$("#toFloatingPopulationList").trigger("reloadGrid");
					if(data == null || data.toString() == "true"){
						$.messageBox({ message:"转为流动人口成功!"});
						$("#toFloatingPopulationDialog").dialog("close");
					}else{
						...
						if(dataMassage != null && dataMassage != ""){
							...
							for(var i = 0; i < idAndCards.length; i++){
								if(idAndCards[i] != null && idAndCards[i] != ""){
									...
									$("#toFloatingPopulationList").find("#"+id).css("background", "#FF8888");
									$("#toFloatingPopulationList").setSelection(id,true);
								}
							}
						}
...
$(document).ready(function(){
	...
	jQuery("#toFloatingPopulationList").jqGridFunction({
			datatype: "local",
		 	colModel:[
	...
	var selectedId = getSelectedIdsByTabId("householdStaffList");
	for(var i = 0; i < selectedId.length; i ++ ){
		var seldata = $("#householdStaffList").getRowData(selectedId[i]);
		$("#toFloatingPopulationList").addRowData(seldata.id,seldata);
		$("#toFloatingPopulationList").setSelection(seldata.id,true);
	}
});

function setHouseInfoOperateButton(){
	var selectedCounts = getActualjqGridMultiSelectCount("toFloatingPopulationList");
	var count = $("#toFloatingPopulationList").jqGrid("getGridParam","records");
	if(selectedCounts == count && count > 0){
		jqGridMultiSelectState("toFloatingPopulationList", true);
	}else{
		jqGridMultiSelectState("toFloatingPopulationList", false);
	}
}
</script>
