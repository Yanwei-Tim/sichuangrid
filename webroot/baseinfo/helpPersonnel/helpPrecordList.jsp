...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#helpPrecordList").jqGridFunction({
			...
			colModel:[
				...
				{name:"helpTime", sortable:false,index:"floorpersonsTime", label: '帮教时间',width:100},
				{name:"address", sortable:false, label:'帮教地点', width:100},
				{name:"helpPersonnelName", sortable:false, label: '帮教人员',width:200},
				{name:"events", sortable:false, label: '帮教情况'}
			],
		...
		$("#addHelpPrecord").click(function(){
			...
			$("#helpPrecordDialog").createDialog({
				...
				height: 400,
				title:'新增帮教情况',
				url:'${path}/baseinfo/helpPrecord/dispatch.action?personnelId='+domainId+'&personnelType=<s:property value="#parameters.keyType"/>&mode=add&precordType=help',
				buttons: {
					"保存" : function(event){
		...
		$("#updateHelpPrecord").click(function(){
			...
			$("#helpPrecordDialog").createDialog({
				...
				height: 400,
				title:'修改帮教情况',
				url:'${path}/baseinfo/helpPrecord/dispatch.action?helpPrecord.id='+selectedId+'&mode=edit&precordType=help',
				buttons: {
					"保存" : function(event){
				      $("#helpPrecordForm").submit();
				   },
		...
		$("#deleteHelpPrecord").click(function(){
			...
			$.confirm({
				title:"确认删除",
				message:"该帮教情况信息删除后，将无法恢复,您确认删除该帮教情况吗?",
				width: 300,
	...
	function viewHelpPrecord(){
		...
		$("#helpPrecordDialog").createDialog({
			...
			height: 400,
			title:'查看帮教情况',
			url:'${path}/baseinfo/helpPrecord/dispatch.action?helpPrecord.id='+selectedId+'&mode=view&precordType=help',
			buttons: {
			   "关闭" : function(){
			        $(this).dialog("close");
			   }
			...
			success:function(responseData){
				if(responseData){
					if(responseData == true){
						...
						$.messageBox({message:"帮教情况删除成功！"});
						disableHelpPrecordButton();
					}else{
						$.messageBox({message:"帮教情况删除失败！"});
					}
				}
			}
		});
