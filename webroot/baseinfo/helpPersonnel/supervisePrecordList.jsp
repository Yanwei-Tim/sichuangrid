...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#helpPrecordList").jqGridFunction({
			...
			ondblClickRow:viewSuperviseHelpPrecord,
		...
		$("#addHelpPrecord").click(function(){
			...
			$("#helpPrecordDialog").createDialog({
				...
				height: 400,
				title:'新增监护情况',
				url:'${path}/baseinfo/helpPrecord/dispatch.action?personnelId='+domainId+'&personnelType=<s:property value="#parameters.keyType"/>&mode=add&precordType=supervise',
				buttons: {
					"保存" : function(event){
		...
		$("#updateHelpPrecord").click(function(){
			...
			$("#helpPrecordDialog").createDialog({
				...
				height: 400,
				title:'修改监护情况',
				url:'${path}/baseinfo/helpPrecord/dispatch.action?helpPrecord.id='+selectedId+'&mode=edit&precordType=supervise',
				buttons: {
					"保存" : function(event){
				      $("#helpPrecordForm").submit();
				   },
	...
	function viewSuperviseHelpPrecord(){
		var selectedId = $("#helpPrecordList").getGridParam("selrow");
		if(selectedId==null){
		  return;
		}
		$("#helpPrecordDialog").createDialog({
			...
			height: 400,
			title:'查看监护情况',
			url:'${path}/baseinfo/helpPrecord/dispatch.action?helpPrecord.id='+selectedId+'&mode=view&precordType=supervise',
			buttons: {
			   "关闭" : function(){
			        $(this).dialog("close");
			   }
