...
<div id="dialog-form" title="" class="container container_24">
	...
	<pop:token />
		...
		<div class="grid_6 lable-right">
			<label class="form-lbl">监护人员：</label>
		</div>
...
<script type="text/javascript">
	...
	<s:if test='"add".equals(mode)'>
		$("#helpPrecordForm").attr("action","${path}/baseinfo/elderlyPeopleHelpPrecord/addHelpPrecord.action");
	</s:if>
	<s:if test='"edit".equals(mode)'>
		$("#helpPrecordForm").attr("action","${path}/baseinfo/elderlyPeopleHelpPrecord/updateHelpPrecord.action");
	</s:if>
		$("#helpPrecordForm").formValidate({
			...
			messages:{
				...
				"helpPrecord.address":{
					required:"监护地址不能为空",
					maxlength:$.format("监管地址最多输入{0}个字符")
				}
				,
				"helpPrecord.events":{
