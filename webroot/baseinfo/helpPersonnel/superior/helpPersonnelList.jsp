...
<%@ include file="/includes/baseInclude.jsp"%>
	...
	<input type="hidden" id="superiorKeyType" name="" value="<s:property value="#parameters.superiorKeyType"/>"/>
...
<script type="text/javascript">
	var superiorKeyType=$("#superiorKeyType").val();
	...
	$(document).ready(function(){
		...
		$("#helpPersonnelList").jqGridFunction({
			...
			ondblClickRow: viewsuperiorHelp,
		...
		$("#addHelp").click(function(){
			...
			$("#helpPersonnelDialog").createDialog({
				...
				height: 250,
				title:'新增综治人员信息',
				url:'${path}/baseinfo/helpPersonnel/dispatch.action?personnelId='+domainId+'&personnelType=<s:property value="#parameters.superiorKeyType"/>&mode=add&personType=superiorHelp',
				buttons: {
					"保存" : function(event){
		...
		$("#updateHelp").click(function(){
			...
			$("#helpPersonnelDialog").createDialog({
				...
				height: 250,
				title:'修改综治人员信息',
				url:'${path}/baseinfo/helpPersonnel/dispatch.action?helpPersonnel.id='+id[0]+'&mode=edit&personType=superiorHelp',
				buttons: {
					"保存" : function(event){
				      $("#helpPersonnelForm").submit();
				   },
		...
		$("#logoutHelp").click(function(){
			...
			$.confirm({
				title:"确认注销",
				message:"该综治人员信息注销后，将无法恢复,您确认注销该人员信息吗?",
				width: 300,
				okFunc: function(){
				$.ajax({
	...
	function viewsuperiorHelp(){
		...
		$("#helpPersonnelDialog").createDialog({
			...
			height: 250,
			title:'查看综治人员信息',
			url:'${path}/baseinfo/helpPersonnel/dispatch.action?helpPersonnel.id='+id[0]+'&mode=view&personType=superiorHelp',
			buttons: {
			   "关闭" : function(){
			        $(this).dialog("close");
			   }
	...
	function isHelpPersonnel(){
		...
		$.ajax({
			...
			data:{
				...
				"personnelType":$("#superiorKeyType").val()
			},
			success:function(responseData){
