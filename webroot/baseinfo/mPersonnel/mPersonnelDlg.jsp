...
<div id="dialog-form" title="新增" class="container container_24">
	<div id="tabs" style="height:400px;">
	   	<div id="positiveInfoDiv" class="container container_24">
	  		<form id="positiveInfoForm" method="post" action="${path}/baseinfo/mPersonnelManage/addMPersonnel.action" >
	  		<pop:token />
			...
			<jsp:include page="/baseinfo/commonPopulation/populationInfo/mPersonnel.jsp"></jsp:include>	
			</form>
	  	</div>
	</div>
</div>
			...
			$(form).ajaxSubmit({
				...
				success:function(data){
					...
					if("add"=="<s:property value='#parameters.mode[0]'/>"){
						if($("#isLock").val()!='1'){
	    				 	$("#mpersonnelList").addRowData(data.id,data,"first");
		                	$("#mpersonnelList").setSelection(data.id);
		                }
	                ...
	                if("edit"=="<s:property value='#parameters.mode[0]'/>"){
	                	 ...
	                	 if(data.death){
               				if($("#isLock").val()=='0') {
               					$("#mpersonnelList").delRowData(data.id);
							}else {
								 $("#mpersonnelList").setRowData(data.id,data);
								 $("#"+data.id+"").css('color','#778899');
							}
						}else{
							$("#mpersonnelList").setRowData(data.id,data);
						}
						 $("#mpersonnelList").trigger("reloadGrid");
