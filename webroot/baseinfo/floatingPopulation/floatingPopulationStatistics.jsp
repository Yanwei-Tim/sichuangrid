...
<div id="floatingPopulationListDiv">
...
function nameFont(el,options,rowData){
	if(rowData.death == true || rowData.death == "true"){
		return "<pop:JugePermissionTag ename="viewFloatingPopulation"><a href='javascript:viewFloatingPopulationInfo("+rowData.id+")'><font color='red'>"+rowData.name+"</font></a></pop:JugePermissionTag>";
	}
	if(rowData.logOut==1||rowData.logOut=='1'){
		return "<pop:JugePermissionTag ename="viewFloatingPopulation"><a href='javascript:viewFloatingPopulationInfo("+rowData.id+")'><font color='#778899'>"+rowData.name+"</font></a></pop:JugePermissionTag>";
	}
	return "<pop:JugePermissionTag ename="viewFloatingPopulation"><a href='javascript:viewFloatingPopulationInfo("+rowData.id+")'><font color='#6633FF'>"+rowData.name+"</font></a></pop:JugePermissionTag>";
}
...
$(document).ready(function(){
	...
	function operateFormatter(el, options, rowData){
		return "<pop:JugePermissionTag ename="viewFloatingPopulation"><a href='javascript:viewFloatingPopulationInfo("+rowData.id+")'><U><font color='#6633FF'>查看</font></U></a></pop:JugePermissionTag>";
	}
	...
	$("#floatingPopulationList").jqGridFunction({
		...
		ondblClickRow: viewFloatingPopulationInfo,
		</pop:JugePermissionTag>
...
function viewFloatingPopulationInfo(rowid){
	if(rowid==null){
 		return;
	}
	var floatingPopulation =  $("#floatingPopulationList").getRowData(rowid);
	$("#viewFloatingPopulationDialog").createDialog({
		width: dialogWidth,
		height: dialogHeight,
		title:'查看'+title+'信息',
		modal : true,
		url:'${path}/baseinfo/floatingPopulationManage/dispathByEncrypt.action?mode=view&population.id='+floatingPopulation.encryptId,
		buttons: {
		   "关闭" : function(){
		        $(this).dialog("close");
		   }
