...
var title = "危险化学品单位";	
		jQuery(document).ready(function() {		
			...
			$("#searchHighA").click(function(){
				$("#dangerousChemicalsUnitDialog").createDialog({
					width: 650,
					height: 400,
					title: title+'查询-请输入查询条件',
					modal:true,
					url: '${path}/baseinfo/siteinfo/dangerousChemicalsUnit/searchDangerousChemicalsUnitDlg.jsp',
					buttons: {
						"查询" : function(event){
			...
			$("#searchByIdcardA").click(function(){
				...
				if("请输入危险化学品单位名称"==$("#searchByPlaceName").val()){
								$.notice({
									level: 'alert',
								    okbtnName: "确定",
								    title:'提示',
								    message: "请输入请输入危险化学品单位名称"
								});
								return;
							}
			...
			$("#refreshOrgTree1").click(function(){
				...
				$("#searchByPlaceName").attr("value","请输入危险化学品单位名称");
			});
			var layout=function(){
		...
		function getDialogParam(isSearch){
			return {
				...
				title: title+'信息',
				modal:true,
				url: '${path}/baseinfo/siteinfo/dangerousChemicalsUnit/dangerousChemicalsUnitListForStat.jsp?isSearch='+isSearch,
				buttons: {
			   		"关闭" : function(){
			        	$(this).dialog("close");
			   		}
		...
		function onOrgChanged(){
			...
			$("#areaDatatable").setPostData({
		    	...
		    	"tableName":"dangerousChemicalsUnit"
		   	});
...
<div class="btnbanner">
	...
	<div class="ui-widget autosearch">
		<input type="text" value="请输入危险化学品单位名称" class="searchtxt" id="searchByPlaceName" style="width:215px;" maxlength="18" onfocus="if(this.value == '请输入危险化学品单位名称') this.value = '';" onblur="if(this.value == '') this.value = '请输入危险化学品单位名称';">
...
<div id="dangerousChemicalsUnitDialog"></div>
