...
var title = "公共复杂场所";	
		jQuery(document).ready(function() {		
			...
			$("#searchHighA").click(function(){
				$("#publicComplexPlacesDialog").createDialog({
					width: 700,
					height: 350,
					title: title+'查询-请输入查询条件',
					modal:true,
					url: '${path}/baseinfo/publicComplexPlacesManage/dispatchOperate.action?mode=search&organizationId='+getCurrentOrgId(),
					buttons: {
						"查询" : function(event){
			...
			$("#searchByIdcardA").click(function(){
				...
				if("请输入公共复杂场所名称"==$("#searchByPlaceName").val()){
					$.notice({
						level: 'alert',
					    okbtnName: "确定",
					    title:'提示',
			...
			$("#refreshOrgTree1").click(function(){
				...
				$("#searchByPlaceName").attr("value","请输入公共复杂场所名称");
			});
			var layout=function(){
		...
		function getDialogParam(isSearch){
			return {
				...
				title: title+'信息',
				modal:true,
				url: '${path}/baseinfo/siteinfo/publicComplexPlaces/publicComplexPlacesListForStat.jsp?isSearch='+isSearch,
				buttons: {
			   		"关闭" : function(){
			        	$(this).dialog("close");
			   		}
...
<div class="btnbanner">
	...
	<div class="ui-widget autosearch">
		<input type="text" value="请输入公共复杂场所名称" class="searchtxt" id="searchByPlaceName" style="width:215px;" maxlength="18" onfocus="if(this.value == '请输入公共复杂场所名称') this.value = '';" onblur="if(this.value == '') this.value = '请输入公共复杂场所名称';">
...
<div id="publicComplexPlacesDialog"></div>
