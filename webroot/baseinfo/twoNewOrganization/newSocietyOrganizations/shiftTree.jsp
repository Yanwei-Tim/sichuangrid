...
<form id="maintainShiftForm" method="post" action="${path}/baseinfo/newSocietyOrganizationsManage/shiftPerson.action">
<pop:token />
	...
	<input name="newSocietyOrganizations.organization.id" id="shiftOrgId" value="" type="text" style="width:0px;border:0px;height:0px;overflow:hidden;position:absolute;top:25px;left:70px;">
	<input name="locationIds" id="newSocietyFederationIds" value="${locationIds}" type="hidden">
</form >
...
$(document).ready(function(){
	...
	$("#maintainShiftForm").formValidate({
         ...
         $(form).ajaxSubmit({
             success: function(data){
                     if(data){
     					...
     					$("#newSocietyOrganizationsDialog").dialog("close");
		...
		rules:{"newSocietyOrganization.organization.id":{isGridOrganization:true,isModernOrg:true}},
		messages:{"newSocietyOrganization.organization.id":{isGridOrganization:"数据只能转移得到网格",isModernOrg:"数据不能转移到当前网格"}}
	});
});
</script>