$.bflow=function(options){
	...
	var createNode=function(id,x, y, width, height,r){
		var thatRect=that.rect(x, y, width, height,r).attr({fill: '90-#FFE084:10-#FFF4D2', "stroke-width":1,stroke:'#FFD65F', cursor: "hand"}).mouseover(function(){//绑定指向台账
			if(this.glows!=undefined){
				this.glows.show();
			}else{
				this.glows=this.glow({color:'#FFD65F',width:7});
			}
		}).mouseout(function(){//绑定鼠标移出台账
			this.glows.hide();
		});
		thatRect.id=id;
	...
	var createText=function(id,x,y,name){
		var thatText=that.text(x, y, name).attr({font: 'bold 12px "microsoft yahei", Arial',fill: '#B22D00',cursor:'hand'}).mouseover(function(){//绑定指向台账
			var thisDate={
				id:this.id,
				type:this.nodeType
			}
	...
	var calculateIndex=function(data){
		...
		for(var i=0;i<data.length;i++){
			...
			if(i!=0){
				...
				if(data[i-1].relation=='实质性办结'){
					index=conculationJieAn(data,i);
					data[i].index=index;
				}if(data[i-1].relation=='程序性办结'){
					index=conculationJieAn(data,i);
					data[i].index=index;
				}if(data[i-1].relation=='阶段性办结'){
					index=conculationJieAn(data,i);
					data[i].index=index;
				}
				if(data[i-1].relation=='申报'){
					index=conculation(data,i);
					data[i].index=index;
				}
				if(data[i-1].relation=='转办'){
					index=conculation(data,i);
					data[i].index=index;
				}
			}
	...
	var createNodes=function(holder,thisData,maxOrgLevel){
		...
		var box=createNode(thisData.id,x, y, dfop.boxWidth, dfop.boxHeight, 3).click(function(){
			...
			clearTimeout(window._holderPopTimerOut);
			var that=this;
			holder.popup({thatDom:that,url:"/threeRecordsIssues/issueManage/viewIssueDealLog.action?issueMap.id="+thisData.id+"&issueMap.orgId="+thisData.orgId});
