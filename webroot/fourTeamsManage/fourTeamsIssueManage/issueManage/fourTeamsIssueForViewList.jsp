...
<div class="content">
	<jsp:include page="${path}/fourTeamsManage/fourTeamsIssueManage/IssueManage/fourTeamsIssueForViewButtons.jsp"/>
...
$(document).ready(function(){
	function dealStateFormatter(el, options, rowData){
		if(viewType.value=='done'){//对于已办事件状态的处理
			if(rowData.status==1){
				return "办理中";
	...
	function rendUrgent(el, options, rowData){
		...
		if(rowData.dealState != null){
			if(110 == rowData.dealState){
				...
				iconArray += "<div class='myStyle'><strong class='dealState complete-state' title='已完成'></strong></div>";
			}else {
	...
	$("#issueList").jqGridFunction({
		...
		colNames:['issueId','issueStepId','encryptIdByIssueId','encryptIdByIssueStepId','publicltycass','topState','earingWarn'
		          <s:if test="!(#request.viewType.equals(@com.tianque.issue.constants.IssueViewType@COMPLETED)) && !(#request.viewType.equals(@com.tianque.issue.constants.IssueViewType@DONE))">,'督办'</s:if>,'supervisionState','预警','urgent','事件名称'
		...
		colModel:[
			...
			{name:'earingWarn',hidden:true},
			 <s:if test="!(#request.viewType.equals(@com.tianque.issue.constants.IssueViewType@COMPLETED)) && !(#request.viewType.equals(@com.tianque.issue.constants.IssueViewType@DONE))">
			{name:'supervisionStateShow',index:'isteps.superviselevel',sortable:true,formatter:rendSupervise,width:120},
			</s:if>
			{name:'supervisionState',index:'isteps.superviselevel',hidden:true},
			{name:'urgentShow',index:'iu.urgent',sortable:true,formatter:rendUrgent,width:80},
			...
			{name:'currentOrgDisplayName',index:'iu.currentOrgDisplayName',sortable:true},
			{name:'occurOrg.orgName',index:'iu.occurOrg',sortable:true,width:200},
			{name:'dealTime',index:'cstep.lastdealdate',sortable:true,width:100}
...
function viewIssue(){
	if (hasRowSelected() && $("#view").isButtonEnabled()){
		...
		$("#issueDialog").createDialog({
			...
			height: 500,
			title: "查看事件详情",
			url: "${path}/fourTeamsIssueManage/dispatch.action?mode=viewNew&keyId="+selectedIssue.encryptIdByIssueId,
			buttons: {
		   		"关闭" : function(){
		        	$(this).dialog("close");
		   		}
