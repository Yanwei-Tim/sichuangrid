...
$(document).ready(function() {
	...
	$("#searchType").change(function(){
		if($("#searchType").val()=="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_MONTH'/>"){
			$("#quarterDiv").hide();
			$("#monthDiv").show();
		}else if($("#searchType").val()=="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_QUARTER'/>"){
			$("#quarterDiv").show();
			$("#monthDiv").hide();
		}else if($("#searchType").val()=="<s:property value='@com.tianque.plugin.analysisNew.util.IssueAnalysisSearchType@SEARCH_BY_YEAR'/>"){
			$("#quarterDiv").hide();
			$("#monthDiv").hide();
		}
	});
...
function onOrgChanged(){
	$("#positiveinfoPie").pieChart({
		url:"${path}/issueAnalysisChartManageNew/getStatisticPie.action?orgId="+getCurrentOrgId()+"&year="+$("#queryYear").val()+"&month="+$("#queryMonth").val()+"&searchType="+$("#searchType").val()+"&quarter="+$("#quarter").val()+"&issueTypeDomainId="+$("#issueDomain").val(),
		moduleName:"事件处理统计",
...
function getQuarter(){
	$.ajax({
		async: false,
		url: "${path }/stat/currentTimeNew/getCurrentTimeForQuarter.action?currenYear="+$("#queryYear").val(),
		success:function(responseData){
			for(var i = 0;i<responseData.length;i++){
