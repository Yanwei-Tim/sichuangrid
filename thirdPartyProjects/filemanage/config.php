...
define('ACCESS_KEY',"Kent is,superstar? I do not think so!");
...
function CheckAuthentication(){
   // return true;
    ...
    if( isset($permission['logined']) and $permission['logined'])
		return true;
	return false;
}
...
function getPermission(){
		...
		//accesskey 是信任关系
		//sid则是客户端带过来的sessionid
		$url = 'http://localhost:8080/singleLogin/filePermission.action?accessKey='.md5(ACCESS_KEY).'&sid='.$sid;
		//error_log($url);
        $handler = fopen($url,'r');
        while(!feof($handler)){
			$jsonResult.=fread($handler,512);
         }
		fclose($handler);
		$config['permission'] = json_decode($jsonResult, true);
		//error_log($jsonResult);
...
Example: 'maxSize' => "8M",
*/
$config['DefaultResourceTypes'] = '全国';
...
$config['ResourceType'][] = Array(
		...
		'maxSize' => "8M",
		'allowedExtensions' => '7z,aiff,asf,avi,bmp,csv,doc,docx,fla,flv,gif,gz,gzip,rar,jpeg,jpg,mid,mov,mp3,mp4,mpc,mpeg,mpg,ods,odt,pdf,png,ppt,pptx,pxd,qt,ram,rar,rm,rmi,rmvb,rtf,sdc,sitd,swf,sxc,sxw,tar,tgz,tif,tiff,txt,vsd,wav,wma,wmv,xls,xlsx,zip',
		'deniedExtensions' => '');
...
*/
$config['ResourceType'][] = Array(
		'name' => '全国',				// Single quotes not allowed
		'url' => $baseUrl . 'files',
		'directory' => $baseDir . 'files',
		'maxSize' => "8M",
		'allowedExtensions' => '7z,aiff,asf,avi,bmp,csv,doc,docx,fla,flv,gif,gz,gzip,rar,jpeg,jpg,mid,mov,mp3,mp4,mpc,mpeg,mpg,ods,odt,pdf,png,ppt,pptx,pxd,qt,ram,rar,rm,rmi,rmvb,rtf,sdc,sitd,swf,sxc,sxw,tar,tgz,tif,tiff,txt,vsd,wav,wma,wmv,xls,xlsx,zip',
...
include_once "plugins/customacl/plugin.php";
