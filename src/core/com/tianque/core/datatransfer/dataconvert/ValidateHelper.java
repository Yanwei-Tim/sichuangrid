...
public class ValidateHelper {
	...
	public boolean illegalOrganizationName(Organization parent, String orgName) {
		...
		if (vlaue != -1) {
			...
			if (results != null && results.size() == 1) {
				results = organizationDubboService.findOrganizationsByOrgNameAndParentId(
	...
	public boolean illegalDate(String strDate) {
		...
		if (date == null) {
			...
			if(date==null){
				date = CalendarUtil.parseDate(strDate, "yyyy-MM-dd HH:mm:ss");
				if (null != date && getPrevFourString(strDate, "-") < 1900) {
					return true;
				}
			}
	...
	public boolean illegalDateDetail(String strDate) {
		if (!StringUtil.isStringAvaliable(strDate)) {
			return false;
		}
		Date date = null;
	...
	public boolean isUbordinateRelationship(Long downOrgId, Long upOrgId) {
		Organization downOrg = organizationDubboService.getSimpleOrgById(downOrgId);
		Organization upOrg = organizationDubboService.getSimpleOrgById(upOrgId);
		...
		if (upOrg != null
			...
			upOrg = organizationDubboService.getParentOrgByOrgTypeAndChildOrgId(
