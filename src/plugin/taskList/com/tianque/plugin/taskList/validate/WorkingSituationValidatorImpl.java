...
@Component("workingSituationValidator")
public class WorkingSituationValidatorImpl implements
DomainValidator<WorkingSituation>{
	...
	@Override
	public ValidateResult validate(WorkingSituation domain) {
		...
		if(domain.getOccurrenceDate()==null){
			validateResult.addErrorMessage("发生时间不能为空！");
		}
		...
		if(validateHelper.emptyString(domain.getPoliceName())){
			validateResult.addErrorMessage("民警姓名必须输入！");
		}
		if (domain.getPoliceName()!=null
				&& validateHelper.illegalStringLength(2, 10,
						domain.getPoliceName())) {
			validateResult.addErrorMessage("民警姓名只能输入2-10个字");
		}
		
		if (!workingContentIsNotNull(domain.getWorkcontent())) {
			validateResult.addErrorMessage("工作内容必须输入！");
		}
	...
	private boolean workingContentIsNotNull(PropertyDict workingContent) {
		if (null == workingContent || null == workingContent.getId()) {
			return false;
		}
		return true;
	}
