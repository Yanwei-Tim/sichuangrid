		...
		try {
			...
			if (Constants.GET.equals(method)) {
				...
				if ("LOCATION".equals(event)) {// 微信用户地理位置通知
					redCuffTeamService.bindingRedCuffTeamCoordinate(
							messageMap.get("Latitude"),
							messageMap.get("Longitude"),
							messageMap.get("Precision"),
							messageMap.get("FromUserName"));
				} else if ("MASSSENDJOBFINISH".equals(event)) {// 群发消息结果回调通知
					String msgId = messageMap.get("MsgID");
					WeChatResponse weChatResponse = weChatResponseDao
							.getByMsgId(msgId);
					if (weChatResponse == null) {
						logger.error("微信群发消息未找到，微信服务器消息："
								+ JSON.toJSONString(messageMap));
						...
						weChatResponse.setTotalCount(Integer
								.parseInt(messageMap.get("TotalCount")));
						weChatResponse.setFilterCount(Integer
								.parseInt(messageMap.get("FilterCount")));
						weChatResponse.setSentCount(Integer.parseInt(messageMap
								.get("SentCount")));
						weChatResponse.setErrorCount(Integer
								.parseInt(messageMap.get("ErrorCount")));
					...
					if ("subscribe".equals(event)) {
						...
						if (fromUserName != null && toUserName != null) {
							...
							TencentUser tencentUser = tencentUserService
									.getTencentUserByWeChatUserId(toUserName);

							Fan fan = fanService.getFanByOpenIdAndWeChatUserId(
									fromUserName, toUserName);
							if (fan == null && tencentUser != null) {
								fanService.saveFan(messageMap, tencentUser);
							}
							redCuffTeam = redCuffTeamService
									.getRedCuffTeamByWechatno(fromUserName);
							if (redCuffTeam != null
									&& redCuffTeam.getSubTeamType() != null
								...
								if (tencentUser != null
									...
									WeChatGroup weChatGroup = weChatGroupService
											.getGroupListByNameAndWeChatUserId(
													toUserName, propertyDict
															.getDisplayName());
									if (weChatGroup != null) {
										if (weChatService.getFanByOpenId(
												fromUserName, tencentUser) != null) {
											weChatGroupService
													.removeUserForGroup(
															tencentUser,
															fromUserName,
					...
					out = response.getWriter();
					/** 接收消息（存入数据库） */
					String respMessage = weChatService
							.receiveMessage(messageMap);
					if (respMessage != null) {
			...
			if (out != null) {
				out.print(MessageUtil.textMessageToXml(createErrorMessage(
						toUserName, fromUserName, "亲，请求出了一丁点的问题， 请稍后在试！")));
			}
			return ERROR;
