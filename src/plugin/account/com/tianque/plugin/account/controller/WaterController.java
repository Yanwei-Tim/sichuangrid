...
@Scope("request")
@Namespace("/threeRecords/water")
@Controller("waterController")
public class WaterController extends BaseAction {
	...
	private WaterResources waterResource;
	...
	@Actions( { @Action(value = "dispatch", results = {
			@Result(name = "success", location = "/account/peopleAspiration/waterResourceViewDlg.jsp"),
			@Result(name = "viewNew", location = "/account/peopleAspiration/viewWaterResourceNewDlg.jsp"),
			@Result(name = "print", location = "/account/peopleAspiration/printWaterResourceDlg.jsp"),
			@Result(name = "search", location = "/account/peopleAspiration/searchWaterResourceDlg.jsp"),
		...
		if (DialogMode.ADD_MODE.equals(mode)) {
			...
			peopleAspirations = peopleAspirationDubboService
					.findPeopleAspirationAndFileOrLogAndLogFile(
							LedgerConstants.PEOPLEASPIRATION_WATER, keyId,
							false, false);
			waterResource = peopleAspirationDubboService
					.getWaterResourcesByAspirationId(keyId);
	 ...
	 * @return
	 */
	@Action(value = "addWaterResource", results = {
			@Result(name = "success", type = "json", params = { "root",
					"waterResource", "ignoreHierarchy", "false" }),
			@Result(name = "error", type = "json", params = { "root",
					"errorMessage" }) })
	public String addWaterResource() throws Exception {
		if (waterResource == null
				|| waterResource.getPeopleAspiration() == null) {
			this.errorMessage = "参数无效!";
			return ERROR;
		}
		waterResource = peopleAspirationDubboService
				.addWaterResources(waterResource);
		return SUCCESS;
	}

	@Action(value = "updateWaterResource", results = {
			@Result(name = "success", type = "json", params = { "root",
					"waterResource", "ignoreHierarchy", "false" }),
			@Result(name = "error", type = "json", params = { "root",
					"errorMessage" }) })
	public String updateWaterResource() throws Exception {
		if (waterResource == null || waterResource.getId() == null) {
			this.errorMessage = "参数无效!";
			return ERROR;
		}
		waterResource = peopleAspirationDubboService
				.updateWaterResources(waterResource);
		return SUCCESS;
	}

	@Action(value = "getWaterResource", results = { @Result(name = "success", location = "/account/peopleAspiration/waterResourceDlg.jsp") })
	public String viewWaterResource() throws Exception {
		if (keyId != null) {
			waterResource = peopleAspirationDubboService
					.getWaterResourcesByAspirationId(keyId);
		}
	 ...
	 * @return
	 */
	@Action(value = "deleteWaterSource", results = {
			@Result(name = "success", type = "json", params = { "root", "true" }),
			@Result(name = "error", type = "json", params = { "root",
					"errorMessage", "ignoreHierarchy", "false" }) })
	public String deleteWaterSource() throws Exception {
		if (!legalKeyIdParam()) {
			errorMessage = "参数无效!";
			return ERROR;
		}
		...
		AttachFileUtil.removeIssueAllAttachFiles(keyId, LedgerConstants.PEOPLEASPIRATION_WATER, threeRecordsIssueDubboService);
		peopleAspirationDubboService.deleteWaterLedgerAndStepById(keyId);
		return SUCCESS;
	}
	...
	public String getIssueCount(){
		...
		countMap = threeRecordsIssueDubboService.getIssueCount(keyId, Long.valueOf(LedgerConstants.PEOPLEASPIRATION_WATER), year, month);
		return SUCCESS;
	}
	...
	@Action(value = "existWaterResource", results = {
			@Result(name = "success", type = "json", params = { "root",
					"waterResource", "ignoreHierarchy", "false" }),
			@Result(name = "error", type = "json", params = { "root",
					"errorMessage" }) })
	public String existWaterResource() throws Exception {
		if (keyId != null) {
			waterResource = peopleAspirationDubboService
					.getWaterResourcesByAspirationId(keyId);
		}
	...
	private String findJurisdictionsNeedDo() throws Exception {
		...
		resolveIssueType();
		System.out.println(threeRecordsIssueDubboService);
	...
	@Action(value = "deleteAgriculture", results = {
			@Result(name = "success", type = "json", params = { "root", "true" }),
			@Result(name = "error", type = "json", params = { "root",
					"errorMessage", "ignoreHierarchy", "false" }) })
	public String deleteAgriculture() throws Exception {
		if (!legalKeyIdParam()) {
			errorMessage = "参数无效!";
			return ERROR;
		}
		...
		AttachFileUtil.removeIssueAllAttachFiles(keyId, LedgerConstants.PEOPLEASPIRATION_AGRICULTURE, threeRecordsIssueDubboService);
		peopleAspirationDubboService.deleteAgricultureLedgerAndStepById(keyId);
		return SUCCESS;
	}
	...
	public String exportExcel() throws Exception {
		...
		peopleAspirations = peopleAspirationDubboService
				.findPeopleAspirationAndFileOrLogAndLogFile(
						LedgerConstants.PEOPLEASPIRATION_WATER, keyId, true,
						true, true);
		waterResource = peopleAspirationDubboService
				.getWaterResourcesByAspirationId(keyId);
		inputStream = AccountExportHelper.exportDateToExcel(peopleAspirations,waterResource==null?new WaterResources():waterResource);
		downloadFileName = new String(("中江县民生信息登记卡（水利类）"+peopleAspirations.getSerialNumber()).getBytes("gbk"),
				"ISO8859-1") + ".xls";
		return SUCCESS;
	}
	...
	private boolean hasPermission(Long ledgerId, Long stepId) {
		...
		hasPermission = searchIssueStepDubboService.hasPermission(ledgerId,
				LedgerConstants.PEOPLEASPIRATION_WATER, stepId, userOrgCode);
	...
	private void resolveIssueType() {
		issueType = Long.valueOf(LedgerConstants.PEOPLEASPIRATION_WATER);
	}
	...
	public WaterResources getWaterResource() {
		return waterResource;
	}

	public void setWaterResource(WaterResources waterResource) {
