...
//	@PermissionFilter(ename = "addDifficultPeopleRecord")
	...
	public String exportExcel() throws Exception {
		...
		if(ledgerPoorPeople.getLedgerPoorPeopleMembers().size() < 4){
			for(int i=0;i<4-ledgerPoorPeople.getLedgerPoorPeopleMembers().size();i++){
				ledgerPoorPeople.getLedgerPoorPeopleMembers().add(new LedgerPoorPeopleMember());
			}
		}
		Map map=new HashMap();
		map.put("p", ledgerPoorPeople);
		map.put("issueDealLogs", issueDealLogs);
		map.put("last3Logs", getLast3Logs(issueDealLogs));
		map.put("feedbacks", feedbacks);
		inputStream = AccountExportHelper.exportLedgerPoorPeopleDateToExcel(map);
		downloadFileName = new String(("困难台账信息登记卡"+ledgerPoorPeople.getSerialNumber()).getBytes("gbk"),
				"ISO8859-1") + ".xls";
		return SUCCESS;
	}
