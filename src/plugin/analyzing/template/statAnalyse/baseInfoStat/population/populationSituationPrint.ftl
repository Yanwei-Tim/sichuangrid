...
<div id="populationSituationPrint"  style="overflow: auto;height:100%;width:100%">
   ...
   <div id="personSituationGridbox" class="SigmaReportPrint" style="overflow: auto;height:100%;width:100%">
   </div>
</div>
<div id="cssDiv"> 
 <style type="text/css">
...
$(document).ready(function(){
	$.ajax({
		...
		$("#personSituationGridbox").css({"overflow-y": "auto"});
		$("#personSituationGridbox").append($("#cssDiv").html());
	});
function importPersonelSituationPrint(){
	$.ajax({
		url:'/baseInfoStat/statisticsPopulation/getStatisticsPopulationList.action?orgLevelDistance='+'<@s.property value="#parameters.orgLevelDistance"/>'+'&orgId='+'<@s.property value="#parameters.parentOrgId"/>'+'&year='+'<@s.property value="#parameters.year"/>'+'&month='+'<@s.property value="#parameters.month"/>'+'&populationType='+'<@s.property value="#parameters.populationType"/>',
		success:function(data){
