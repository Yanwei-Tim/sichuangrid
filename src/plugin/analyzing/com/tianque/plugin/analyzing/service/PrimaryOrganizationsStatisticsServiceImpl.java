...
import com.tianque.plugin.analyzing.dao.PrimaryOrganizationsStatisticsDao;
...
@Service("primaryOrganizationsStatisticsService")
@Transactional
public class PrimaryOrganizationsStatisticsServiceImpl extends
		AbstractBaseService implements PrimaryOrganizationsStatisticsService {
	@Autowired
	private PropertyDictService propertyDictService;
	@Autowired
	private PropertyDomainService propertyDomainService;
	@Autowired
	private PrimaryOrganizationsStatisticsDao primaryOrganizationsStatisticsDao;
	@Autowired
	...
	private HighchartColumnVo getArealDistributionList(
		...
		List<Map<String, Object>> list = primaryOrganizationsStatisticsDao
		...
		statisticSearchVo
				.setPropertyDomainId(getPropertyDomainId(statisticSearchVo));
		List<Map<String, Object>> newSocietylist = primaryOrganizationsStatisticsDao
		...
		List<Map<String, Object>> newEconomiclist = primaryOrganizationsStatisticsDao
				.getNewSocietyStatisticList(statisticSearchVo);
		// 将三个list的数据整合
	...
	private HighchartColumnVo createHighchartColumnVoByList(
		...
		for (int i = 0; i < DictSize; i++) {
			...
			if (null != chartColumn.getSeries()) {
				String[] propertyDictLists = AnalyseUtil.groupMap
	...
	public HighchartColumnVo getArealDistributionListFromHistory(Long orgId,
		...
		try {
			...
			if (PopulationType.DEPARTMENTPARTY.equals(type)) {
				...
				tableService.createAnalyseTable(
						AnalyseUtil.PRIMARYORGANIZATIONSENAME,
						AnalyseUtil.PRIMARYORGANIZATIONSSQL, year, month);
			}
			StatisticSearchVo statisticSearchVo = null;
			statisticSearchVo = createStatisticSearchVo(type, orgId, "");
	...
	private HighchartColumnVo getArealDistributionListFromHistory(
		...
		if (PopulationType.DEPARTMENTPARTY.equals(type)) {
			list = departmentPartyService
					.queryDepartmentPartyListFromHistoryForList(statisticSearchVo);
		} else {
			list = primaryOrganizationsStatisticsDao
	...
	public List<PrimaryOrganizationDataColumnVo> getPrimaryOrganizationDataByYearMonthColumnVoList(
		...
		try {
			...
			// primaryOrganizationsStatisticsDao
			...
			// 查询下辖统计
			List<SearchPrimaryOrganizationDataColumnVo> searchPrimaryOrganizationDataColumnVo = primaryOrganizationsStatisticsDao
					.findPrimaryOrganizationDataColumnByDateTwo(year, month,
							orgId);
			// 查询本级数据
			List<SearchPrimaryOrganizationDataColumnVo> thisLevelPrimaryOrganizationDataColumnVo = primaryOrganizationsStatisticsDao
					.findPrimaryOrganizationDataColumnByOgrinternalCode(year,
							month, organization.getOrgInternalCode());

			listData = dealPrimaryOrganizationDataColumnVoTwo(
					thisLevelPrimaryOrganizationDataColumnVo,
					searchPrimaryOrganizationDataColumnVo, organizationsList);
	...
	private List<PrimaryOrganizationDataColumnVo> dealPrimaryOrganizationDataColumnVoTwo(
			List<SearchPrimaryOrganizationDataColumnVo> thisLevelPrimaryOrganizationDataColumnVo,
			List<SearchPrimaryOrganizationDataColumnVo> searchPrimaryOrganizationDataColumnVo,
			List<Organization> organizationsList) {
		if (organizationsList != null
				&& searchPrimaryOrganizationDataColumnVo.size() != 0
				&& organizationsList.size() != 0) {
			List<PrimaryOrganizationDataColumnVo> dealList = dealPrimaryOrganizationDataColumnVoListTwo(
					organizationsList, searchPrimaryOrganizationDataColumnVo);
			if (thisLevelPrimaryOrganizationDataColumnVo != null
					&& thisLevelPrimaryOrganizationDataColumnVo.size() != 0) {
				dealList.add(dataCalculationTwo(
						thisLevelPrimaryOrganizationDataColumnVo,
						new Organization()));
			} else {
				PrimaryOrganizationDataColumnVo columnVo = new PrimaryOrganizationDataColumnVo();
				Organization organization = new Organization();
				organization.setOrgName("本级");
				columnVo.setOrg(organization);
				dealList.add(columnVo);
			}
			// List<PrimaryOrganizationDataColumnVo> dealListForZk =
			// dealDataColumnVoListForZk(dealList);
			List<PrimaryOrganizationDataColumnVo> dealFinal = dealDataColumnVoListForHj(dealList);
			return dealFinal;
		} else {
			...
			if (thisLevelPrimaryOrganizationDataColumnVo != null
					&& thisLevelPrimaryOrganizationDataColumnVo.size() != 0) {
				list.add(dataCalculationTwo(
						thisLevelPrimaryOrganizationDataColumnVo,
						new Organization()));
			} else {
				PrimaryOrganizationDataColumnVo thisLevelcolumnVo = new PrimaryOrganizationDataColumnVo();
				Organization organization = new Organization();
				organization.setOrgName("本级");
				thisLevelcolumnVo.setOrg(organization);
				list.add(thisLevelcolumnVo);
			}
	...
	public List<PrimaryOrganizationDataColumnVo> dealPrimaryOrganizationDataColumnVoListTwo(
			List<Organization> organizationsList,
			List<SearchPrimaryOrganizationDataColumnVo> columnVoList) {
		List<PrimaryOrganizationDataColumnVo> list = new ArrayList<PrimaryOrganizationDataColumnVo>();
		for (Organization organization : organizationsList) {
			list.add(dataCalculationTwo(columnVoList, organization));
		}
		return list;
	}
	 ...
	 * 数据计算
	 */
	private PrimaryOrganizationDataColumnVo dataCalculationTwo(
			List<SearchPrimaryOrganizationDataColumnVo> columnVoList,
			Organization organization) {
		PrimaryOrganizationDataColumnVo dataColumnVo = null;
		int partyTeamCount = 0;// 基层党委
		...
		for (SearchPrimaryOrganizationDataColumnVo searchDataColumnVo : columnVoList) {
			dataColumnVo = new PrimaryOrganizationDataColumnVo();
			if ("本级".equals(organization.getOrgName())
					|| searchDataColumnVo.getOrgInternalCode().startsWith(
							orgInternalCode)) {
				// 判断是否是同一机构
				int basicOrgType = propertyDictService.getPropertyDictById(
						new Long(searchDataColumnVo.getTeamClazz()))
				...
				if (basicOrgType == BasicOrgType.PERMARY_PARTY
					...
					partyTeamMemberCount += searchDataColumnVo.getMemberNum();
					...
					// 群防群治队伍
					massesTeamCount += searchDataColumnVo.getTeamNum();
					massesTeamMemberCount += searchDataColumnVo.getMemberNum();
				} else if (basicOrgType == BasicOrgType.VOLUNTARY_TEAM) {
					// 社工志愿者队伍
					postulantTeamCount += searchDataColumnVo.getTeamNum();
					postulantTeamMemberCount += searchDataColumnVo
					...
					// 社会组织
					socialOrganizationCount += searchDataColumnVo.getTeamNum();
				} else if (basicOrgType == BasicOrgType.NEWECONOMICORGANIZATIONS) {
					// 非公有制经济组织
					newEconomicOrganizationsCount += searchDataColumnVo
							.getTeamNum();
					...
					// 群团组织
					massOrganizationCount += searchDataColumnVo.getTeamNum();
					massOrganizationMemberCount += searchDataColumnVo
							.getMemberNum();
				}
			}
			...
			dataColumnVo
					.setPartyCommitteeMemberCount(partyCommitteeMemberCount);
			dataColumnVo.setSocialOrganizationCount(socialOrganizationCount);
			dataColumnVo
					.setNewEconomicOrganizationsCount(newEconomicOrganizationsCount);
			dataColumnVo
					.setGovernmentDepartmentsCount(governmentDepartmentsCount);
			...
			dataColumnVo.setAutonmoyTeamMemberCount(autonmoyTeamMemberCount);
			dataColumnVo.setAutonomyTeamCount(autonomyTeamCount);
			dataColumnVo.setMassesTeamCount(massesTeamCount);
			dataColumnVo.setMassesTeamMemberCount(massesTeamMemberCount);
			dataColumnVo.setOrg(organization);
			...
			dataColumnVo.setPartyTeamMemberCount(partyTeamMemberCount);
			dataColumnVo.setPostulantTeamCount(postulantTeamCount);
			dataColumnVo.setPostulantTeamMemberCount(postulantTeamMemberCount);
		}
		return dataColumnVo;
	}
	...
	private HighchartDataColumnVo newGetHighchartDataColumnVo(
		...
		highchartDataColumnVo.setData(tendencyChartForPositiveinfo(time,
				orgInternalCode));
	...
	private List<HighchartDataColumnVo> getHighchartDataColumnVoListByType(
		...
		for (PropertyDict propertyDict : propertyDictList) {
			...
			for (int i = 0; i < len; i++) {
				...
				tableService.createAnalyseTable(
						AnalyseUtil.PRIMARYORGANIZATIONSENAME,
						AnalyseUtil.PRIMARYORGANIZATIONSSQL,
						Integer.parseInt(timeData[0]),
						Integer.parseInt(timeData[1]));
				column.getData()[i] = primaryOrganizationsStatisticsDao
						.countPrimaryOrgByMap(typeId.toString(), time[i],
								orgInternalCode);
			}
	...
	public void addPrimaryOrganizationsStat(String type, Integer year,
		...
		try {
			Boolean isCreate = tableService.createAnalyseTable(
					...
					AnalyseUtil.PRIMARYORGANIZATIONSSQL, year, month);
			if (!isCreate) {
				primaryOrganizationsStatisticsDao.deletePrimaryStatType(year,
						month);
			}
			...
			populationStat.setEndDate(CalendarUtil.getNextMonthStart(year,
					month));
			primaryOrganizationsStatisticsDao.addPrimaryOrganizationsStat(
					populationStat, type,
	...
	private void checkHistoryStatisticTable(int year, int month) {
		tableService.createAnalyseTable(AnalyseUtil.PRIMARYORGANIZATIONSENAME,
				AnalyseUtil.PRIMARYORGANIZATIONSSQL, year, month);
	}
	...
	private int[] tendencyChartForPositiveinfo(String[] time,
			String orgInternalCode) {
		int[] data = new int[12];
		for (int i = 0; i < time.length; i++) {
			Integer countSum = primaryOrganizationsStatisticsDao
					.countPrimaryOrgByMap(null, time[i], orgInternalCode);
			if (countSum != null) {
				data[i] = countSum;
			} else {
				data[i] = 0;
