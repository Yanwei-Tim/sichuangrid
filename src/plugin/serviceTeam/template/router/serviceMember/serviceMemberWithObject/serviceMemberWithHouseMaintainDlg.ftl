...
<div class="container container_24">
	...
	<@pop.token />
 		...
 		<input type="hidden" name="serviceTeamMemberVo.objectId" id="cobjectId" value="${(id)! }" /> 
		<input name="serviceTeamMemberVo.objectName" type="hidden" id="object_Name" value="${(name)!}"/>
		<input type="hidden" name="serviceTeamMemberVo.objectType" id="object__Type" value="${(populationType)! }" /> 
	...
	$.ajax({
		...
		url:"${path}/plugin/serviceTeam/router/routerManage/addObjectAndMemberRelation.action",
	   	data:{
			...
			"serviceMemberWithObject.objectType":$("#object__Type").val(),
			"serviceMemberWithObject.objectName":$("#object_Name").val(),
			"serviceMemberWithObject.objectId":${(id)! },
