package com.tianque.plugin.dataManage.location.rentalHouseTemp.controller;
...
@Namespace("/plugin/dataManage/rentalHouseTempManage")
@Transactional
@Scope("prototype")
@SuppressWarnings("serial")
@Controller("rentalHouseTempController")
public class RentalHouseTempController extends
		AbstractDataManageController<RentalHouseTemp, RentalHouse> {
	@Autowired
	private RentalHouseService rentalHouseService;
	@Autowired
	@Qualifier("rentalHouseTempService")
	private AbstractDataManageService locationDataManageService;

	@Resource(name = "rentalHouseTempService")
	...
	public RentalHouseTemp getTemp() {
		return temp;
	}

	public void setTemp(RentalHouseTemp temp) {
		this.temp = temp;
	}

	public RentalHouseTemp getHouseInfo() {
		return population;
	}
	...
	public void setHouseInfo(RentalHouseTemp houseInfo) {
		this.population = houseInfo;
	}

	@Override
	public List getCompareList(RentalHouseTemp temp, RentalHouse target) {
		/*
		 * List compareList = new ArrayList();
		 * Map map = new HashMap(); map.put("notNull", "true"); map.put("label",
		 * "房屋编号"); map.put("tempValue", "" + temp.getHouseCode());
		 * map.put("realValue", "" + target.getHouseCode()); map.put("argType",
		 * "str"); map.put("submitName", "houseCode"); compareList.add(map); map
		 * = new HashMap(); map.put("notNull", "true"); map.put("label",
		 * "房屋地址"); map.put("tempValue", "" + temp.getAddressType().getId() +
		 ...
		 * map.put("submitName", "remark"); compareList.add(map); map = new
		 * HashMap(); map.put("notNull", "false"); map.put("label", "出租用途");
		 * map.put("tempValue", temp.getUsage() == null ? "-1" : "" +
		 * temp.getUsage().getId()); map.put("realValue", target.getUsage() ==
		 * null ? "-1" : "" + target.getUsage().getId());
		 * map.put("PropertyDict",
		 * "@com.tianque.domain.property.PropertyTypes@LETTINGHOUSE_USAGE");
		 * map.put("argType", "PropertyDict"); map.put("submitName",
		 * "usage.id"); compareList.add(map); map = new HashMap();
		 * map.put("notNull", "false"); map.put("label", "租赁备案证号");
		 * map.put("tempValue", "" + temp.getHouseFileNum());
		 * map.put("realValue", "" + target.getHouseFileNum());
		 * map.put("argType", "str"); map.put("submitName", "houseFileNum");
		 * compareList.add(map); map = new HashMap(); map.put("notNull",
		 * "false"); map.put("label", "出租人类型"); map.put("tempValue",
		 * temp.getLessorType() == null ? "-1" : "" +
		 * temp.getLessorType().getId()); map.put("realValue",
		 * target.getLessorType() == null ? "-1" : "" +
		 * target.getLessorType().getId()); map.put("PropertyDict",
		 * "@com.tianque.domain.property.PropertyTypes@LESSOR_TYPE");
		 * map.put("argType", "PropertyDict"); map.put("submitName",
		 * "lessorType.id"); compareList.add(map); map = new HashMap();
		 * map.put("notNull", "true"); map.put("label", "出租人");
		 * map.put("tempValue", "" + temp.getRentalPerson());
		 * map.put("realValue", "" + target.getRentalPerson());
		 * map.put("argType", "str"); map.put("submitName", "rentalPerson");
		 * compareList.add(map); map = new HashMap(); map.put("notNull",
		 * "false"); map.put("label", "证件类型"); map.put("tempValue",
		 * temp.getRentalCertificateType() == null ? "-1" : "" +
		 * temp.getRentalCertificateType().getId()); map.put("realValue",
		 * target.getRentalCertificateType() == null ? "-1" : "" +
		 * target.getRentalCertificateType().getId()); map.put("PropertyDict",
		 * "@com.tianque.domain.property.PropertyTypes@LETTINGCERTIFICATE_TYPE"
		 * ); map.put("argType", "PropertyDict"); map.put("submitName",
		 * "rentalCertificateType.id"); compareList.add(map); map = new
		 * HashMap(); map.put("notNull", "false"); map.put("label", "证件号码");
		 * map.put("tempValue", "" + temp.getRentalCertificateNumbe());
		 * map.put("realValue", "" + target.getRentalCertificateNumbe());
		 * map.put("argType", "str"); map.put("submitName",
		 * "rentalCertificateNumbe"); compareList.add(map); map = new HashMap();
		 * map.put("notNull", "false"); map.put("label", "联系电话");
		 * map.put("tempValue", "" + temp.getRentalTelephone());
		 * map.put("realValue", "" + target.getRentalTelephone());
		 * map.put("argType", "str"); map.put("submitName", "rentalTelephone");
		 * compareList.add(map); map = new HashMap(); map.put("notNull",
		 * "false"); map.put("label", "联系手机"); map.put("tempValue", "" +
		 * temp.getRentalMobileNumber()); map.put("realValue", "" +
		 * target.getRentalMobileNumber()); map.put("argType", "str");
		 * map.put("submitName", "rentalMobileNumber"); compareList.add(map);
		 * map = new HashMap(); map.put("notNull", "false"); map.put("label",
		 * "地址"); map.put("tempValue", "" + temp.getLessorAddress());
		 * map.put("realValue", "" + target.getLessorAddress());
		 * map.put("argType", "str"); map.put("submitName", "lessorAddress");
		 * compareList.add(map); map.put("notNull", "true"); map.put("label",
		 * "隐患程度"); map.put("tempValue", temp.getHiddenDangerLevel() == null ?
		 * "-1" : "" + temp.getHiddenDangerLevel().getId());
		 * map.put("realValue", target.getHiddenDangerLevel() == null ? "-1" :
		 * "" + target.getHiddenDangerLevel().getId()); map.put("PropertyDict",
		 * "@com.tianque.domain.property.PropertyTypes@HIDDEN_TROUBLE_LEVEL");
		 * map.put("argType", "PropertyDict"); map.put("submitName",
		 * "hiddenDangerLevel.id"); compareList.add(map); map.put("notNull",
		 * "false"); map.put("label", "管理类别"); map.put("tempValue",
		 * temp.getMangerTypes() == null ? "-1" : "" +
		 * temp.getMangerTypes().getId()); map.put("realValue",
		 * target.getMangerTypes() == null ? "-1" : "" +
		 * target.getMangerTypes().getId()); map.put("PropertyDict",
		 * "@com.tianque.domain.property.PropertyTypes@MANGER_TYPES");
		 * map.put("argType", "PropertyDict"); map.put("submitName",
		 * "mangerTypes.id"); compareList.add(map); map = new HashMap();
		 * map.put("notNull", "false"); map.put("label", "出租间数");
		 * map.put("tempValue", "" + temp.getRoomNumber()); map.put("realValue",
		 * "" + target.getRoomNumber()); map.put("argType", "str");
		 * map.put("submitName", "roomNumber"); compareList.add(map); map = new
		 * HashMap(); map.put("notNull", "false"); map.put("label", "出租时间");
		 * map.put("tempValue", "" + DateUtil.formateYMD(temp.getLessorDate()));
		 * map.put("realValue", "" +
		 * DateUtil.formateYMD(target.getLessorDate())); map.put("argType",
		 * "strDate"); map.put("submitName", "lessorDate");
		 * compareList.add(map); map = new HashMap(); map.put("notNull",
		 * "false"); map.put("label", "月租金"); map.put("tempValue", "" +
		 * temp.getRentMonth()); map.put("realValue", "" +
		 * target.getRentMonth()); map.put("argType", "str");
		 * map.put("submitName", "rentMonth"); compareList.add(map); map = new
		 * HashMap(); map.put("notNull", "false"); map.put("label",
		 * " 是否签订治安责任保证书"); map.put("tempValue", "" +
		 * temp.getIsSignGuarantee()); map.put("realValue", "" +
		 * target.getIsSignGuarantee()); map.put("argType", "boolean");
		 * map.put("submitName", "isSignGuarantee"); compareList.add(map); map =
		 * new HashMap(); map.put("notNull", "false"); map.put("label",
		 * "有无安全通道"); map.put("tempValue", "" + temp.getIsSafetyChannel());
		 * map.put("realValue", "" + target.getIsSafetyChannel());
		 * map.put("argType", "boolean"); map.put("submitName",
		 * "isSafetyChannel"); compareList.add(map); map = new HashMap();
		 * map.put("notNull", "false"); map.put("label", "有无消防通道");
		 * map.put("tempValue", "" + temp.getIsFireChannels());
		 * map.put("realValue", "" + target.getIsFireChannels());
		 * map.put("argType", "boolean"); map.put("submitName",
		 * "isFireChannels"); compareList.add(map);
		 */
		return null;
	}

	@Override
	public RentalHouse getTargetById(Long id) {
		return rentalHouseService.getHouseInfoById(id);
	}
