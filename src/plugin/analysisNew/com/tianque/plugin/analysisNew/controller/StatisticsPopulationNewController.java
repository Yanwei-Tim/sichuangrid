...
@Controller("statisticsPopulationNewController")
@Scope("prototype")
public class StatisticsPopulationNewController extends BaseAction {
	...
	public String createStatisticsPopulationList() throws Exception {
		...
		statisticsPopulationNewService.addpopulationStat(year, month, orgId,
				populationType);
		return SUCCESS;
	}
	...
	public String getStatisticsPopulationList() throws Exception {
		...
		if (year == Calendar.getInstance().get(Calendar.YEAR)
				&& month == (Calendar.getInstance().get(Calendar.MONTH) + 1)) {
			populationAreaData = statisticsPopulationNewService
					.getCurrentAreaDate(orgId, populationType, orgLevelDistance);
		} else {
			populationAreaData = statisticsPopulationNewService
					.getAreaDateByDate(orgId, populationType, year, month,
							orgLevelDistance);
		}
	...
	public String getStatisticsPopulationColumn() throws Exception {
		...
		if (year == Calendar.getInstance().get(Calendar.YEAR)
			...
			if (populationType == null || "".equals(populationType)
				...
				personnelColumn = statisticsPopulationNewService
	...
	public String getStatisticsPopulationPie() throws Exception {
		...
		personnelPie = statisticsPopulationNewService.getPopulationPieInfo(
				year, month, orgId, populationType);
		return SUCCESS;
	}
	...
	public String getInhabitantCountAndFamilyCountAndImportantPersonelByOrgIdOrHaiNing()
		...
		// 查询得到数据集合
		inhabitantCount = statisticsPopulationNewService.getCountByOrgId(orgId,
				"householdStaff");
		floatingPopulation = statisticsPopulationNewService.getCountByOrgId(
				orgId, "floatingPopulation");
		importantPersonel = baseinfoStatisticNewService
				.getImportPersonCount(orgId);
		importantPlaceCount = statisticsPlaceNewService
				.getImportantPlaceCountByOrgId(orgId);
		lettingHouseCount = baseinfoStatisticNewService
