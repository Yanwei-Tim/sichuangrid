...
import com.tianque.plugin.analysisNew.dao.IssueStatNewDao;
...
@Transactional
@Service("issueStatNewService")
public class IssueStatNewServiceImpl extends AbstractBaseService implements
		IssueStatNewService {
	@Autowired
	...
	@Autowired
	private IssueStatNewDao issueStatNewDao;
	@Autowired
	...
	public List<IssueDealStat> buildIssueDealStatsByMonth(Long rootOrgId,
		...
		List<IssueDealStat> lastMonthStats = issueStatNewDao
		...
		reportBuilder.setLastMonthDealingHistory(issueStatNewDao
		...
		issueStatNewDao.deleteIssueDealStatsByOrgInternalCodeMonthStartDate(
				rootOrg.getOrgInternalCode(), startDate);
		issueStatNewDao.addIssueDealStats(result);
		...
		for (IssueMonthEndDealingHistory record : dealingLogHistoryss) {
			...
			issueStatNewDao.addMonthDealingLogRecord(record);
		}
		return result;
	}
	...
	public List<IssueDealStat> findAdminstrateOrgIssueDealStatsByMonth(
		...
		IssueDealStat rootData = issueStatNewDao
		...
		if (!hasGenerated) {
			...
			for (Organization org : needStatOrgs) {
				IssueDealStat stat = issueStatNewDao
	...
	private IssueDealStat getLastExistedReportDate(Long orgId) {
		List<IssueDealStat> exsited = issueStatNewDao
