...
function getPositiveInfo(){
	$.ajax({
		url:'/baseInfo/statisticManageNew/getBaseInfoStatisticList.action?orgLevelDistance='+'<@s.property value="#parameters.orgLevelDistance"/>'+'&orgId='+'<@s.property value="#parameters.parentOrgId"/>'+'&year='+'<@s.property value="#parameters.year"/>'+'&month='+'<@s.property value="#parameters.month"/>'+'&type='+'<@s.property value="#parameters.type"/>&domainName=' + ('<@s.property value="#parameters.type"/>' == 'all_youth_population' ? '政治面貌' : ('idleYouth' == '<@s.property value="#parameters.type"/>' ? $("#queryBygroup").val() : '')),
...
$(document).ready(function(){
	...
	getPositiveInfo();
	$("#title_<@s.property value='#parameters.type'/>Gridbox").width($("#tHead_<@s.property value='#parameters.type'/>Gridbox").width() - ("<@s.property value='#parameters.type'/>"=="<@s.property value='@com.tianque.core.util.BaseInfoTables@AIDSPOPULATIONS_KEY'/>" ? 7 : 1));
})
</script>
