...
<script type="text/javascript">
	...
	var 
	 columns = [
			...
			{name:"general",caption:"<@s.property  value="@com.tianque.plugin.analysisNew.util.AnalyseUtilNew@groupMap.get(@com.tianque.core.util.BaseInfoTables@IDLEYOUTH_DISPLAYNAME)[2]"/>",children:[
				{name:"baseinfoStatisticDetailVo[index].helped",caption:"<@s.property  value="@com.tianque.plugin.analysisNew.util.AnalyseUtilNew@groupMap.get(@com.tianque.core.util.BaseInfoTables@IDLEYOUTH_DISPLAYNAME)[0]"/>",width:80,mode:"number",align:'center',format:"#"},
				{name:"baseinfoStatisticDetailVo[index].noHelp",caption:"<@s.property  value="@com.tianque.plugin.analysisNew.util.AnalyseUtilNew@groupMap.get(@com.tianque.core.util.BaseInfoTables@IDLEYOUTH_DISPLAYNAME)[1]"/>",width:80,mode:"number",align:'center',format:"#"},
				{name:"baseinfoStatisticDetailVo[index].helped/baseinfoStatisticDetailVo[index].sum",caption:"<@s.property  value="@com.tianque.plugin.analysisNew.util.AnalyseUtilNew@groupMap.get(@com.tianque.core.util.BaseInfoTables@IDLEYOUTH_DISPLAYNAME)[3]"/>",width:80,align:'center',mode:"number",format:"#.00%"}
			]}
...
function onOrgChanged(){
	$.ajax({
		url:'${path}/baseInfo/statisticManageNew/getBaseInfoStatisticList.action?domainName='+encodeURI($("#queryBygroup").val())+'&orgId='+getCurrentOrgId()+"&year="+$("#year").val()+"&month="+$("#month").val()+"&type=<@s.property value="@com.tianque.service.util.PopulationType@IDLE_YOUTH"/>",
...
$(document).ready(function(){
	...
	$("#createStatistic").click(function(){
		$.ajax({
			url:'/baseInfo/statisticManageNew/createHistoryStatistic.action?orgId='+getCurrentOrgId()+"&year="+$("#year").val()+"&month="+$("#month").val()+"&type=<@s.property value="@com.tianque.service.util.PopulationType@IDLE_YOUTH"/>",
	...
	$(".print").click(function(){
		$("#<@s.property value="@com.tianque.service.util.PopulationType@IDLE_YOUTH"/>PrintDlg").createDialog({
			...
			url:'${path}/statAnalyseNew/baseInfoStat/common/commonStatisticPrint.ftl?parentOrgId='+getCurrentOrgId()+"&year="+$("#year").val()+"&month="+$("#month").val()+"&type=<@s.property value="@com.tianque.service.util.PopulationType@IDLE_YOUTH"/>&moduleName="+document.title,
			buttons: {
			   "打印" : function(){
				print();
		  	   },
