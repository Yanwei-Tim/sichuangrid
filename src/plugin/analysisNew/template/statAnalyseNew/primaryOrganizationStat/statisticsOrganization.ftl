...
function onOrgChanged(){
	$.ajax({
		url:'${path}/baseInfoStat/searchPrimaryOrganizationDataColumnNew/getDataColumn.action?parentOrgId='+getCurrentOrgId(),
...
$(document).ready(function(){
	...
	$(".print").click(function(){
		$("#pieDialogNew").createDialog({
			...
			height:400,
			title:"打印基层组织",
			url:'${path}/statAnalyse/primaryOrganizationStatNew/print.jsp?parentOrgId='+getCurrentOrgId(),
			buttons: {
			   "打印" : function(){
				print();
		  	   },
