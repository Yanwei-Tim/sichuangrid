...
public final class ThreadPool extends ThreadPoolExecutor {
	...
	public static void remove(Long key) {
		...
		} else if (thread instanceof NewExcelDataimportThread) {
			((NewExcelDataimportThread) thread).setIntercept(true);
		}
		map.remove(key);
	}

	public static void newrRemove(Long key) {
		NewExcelDataimportThread thread = (NewExcelDataimportThread) map
				.get(key);
		if (null == thread) {
			return;
		}
