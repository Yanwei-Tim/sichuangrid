...
public abstract class AbstractDataConverter<T> implements DataConvert<T> {
		...
		if (GlobalSetting.NOT_ADD_POPULATION
			...
			try {
				convertActualPopulationToAttentionPopulation(
						actualPopulationProcessorService.getActualPopulationbyOrgIdAndIdCardNo(
								domain.getOrganization().getId(),
								domain.getIdCardNo()), domain);
		...
		try {
			if (GlobalSetting.NOT_ADD_POPULATION
				...
				if (null == actualPopulation) {
					 ...
					 * .parseInt(ExcelImportHelper .getDataColumMap("idCardNo"))
					 * + 1), "身份证为" + domain.getIdCardNo() + "在该网络的实有人口中不存在");
					 */
				...
				result.addAllErrorMessage(valitor.validateBaseInfo(domain));
				valitor.validateCurrentAddress(domain, result, convertionHelper);
			}
		} catch (Exception e) {
	...
	public boolean checkDataExitInCache(String modulType, String modulKey,
		...
		return !cacheService.add(key, 300, modulType);
	}
