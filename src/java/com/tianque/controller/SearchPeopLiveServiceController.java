...
@Controller("searchPeopLiveServiceController")
@Scope("prototype")
@Transactional
public class SearchPeopLiveServiceController extends BaseAction {
	@Autowired
	...
	private SearchIssueVo searchPeopleLiveServiceVo = new SearchIssueVo();

	private Long orgId;

	public String searchPeopLiveService() {
		if (null == searchPeopleLiveServiceVo) {
			...
			if (null != orgId) {
				...
				if (null != org) {
					...
					searchPeopleLiveServiceVo.setOrganization(null);
				}
			} else {
				searchPeopleLiveServiceVo.setOrganization(null);
			}
			gridPage = new GridPage(ControllerHelper.processAllOrgRelativeName(searchServiceDaoNew
					.searchSecuritytroubleByOrgInternalCode(searchPeopleLiveServiceVo, page, rows,
