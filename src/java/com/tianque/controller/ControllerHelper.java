...
public class ControllerHelper {
	...
	public static PageInfo processAllOrgRelativeName(PageInfo pageInfo,
			...
			String[] orgPropertyNames, Long organizationId) {
		if (pageInfo == null || pageInfo.getResult() == null
				|| pageInfo.getResult().size() == 0)
			return pageInfo;
		processAllOrgRelativeName(pageInfo.getResult(),
				organizationDubboService, orgPropertyNames, organizationId);
		return pageInfo;
	}

	public static PageResult processAllOrgRelativeName(PageResult pageResult,
			OrganizationDubboService organizationDubboService,
			String[] orgPropertyNames, Long organizationId) {
		if (pageResult == null || pageResult.getResultList() == null
				|| pageResult.getResultList().size() == 0)
			return pageResult;
		processAllOrgRelativeName(pageResult.getResultList(),
				organizationDubboService, orgPropertyNames, organizationId);
		return pageResult;
	}
		...
		try {
			...
			for (Object object : list) {
				for (Method readOrgMethod : orgMethods) {
					...
					if ("getDealOrg".equals(readOrgMethod.getName())
							&& object instanceof IssueLogNew) {
						IssueLogNew issueLogNew = (IssueLogNew) object;
						if (issueLogNew != null
								&& issueLogNew.getDealOrgId() != null
								&& issueLogNew.getDealOrgId().longValue() < 0
								&& cacheValues.get(issueLogNew.getDealOrgId()) == null) {
							Organization orgTemp = IssueToCMSUtil
									.getLocationOrgNameByLocationId(issueLogNew
											.getDealOrgId());
							issueLogNew.setDealOrg(orgTemp);
							cacheValues.put(issueLogNew.getDealOrgId(),
									orgTemp.getOrgName());
						}
					}
				}
	...
	private static void processSingleOrganizationRelativeName(
			...
			OrganizationDubboService organizationDubboService,
			Long organizationId) throws IllegalArgumentException,
		...
		if (organization != null) {
			...
			if (orgRelativeName == null) {
				Organization org = organizationDubboService
						.getSimpleOrgAllOrganizationById(organization.getId());
				...
				if (org != null && org.getParentOrg() != null) {
					orgRelativeName = ControllerHelper
							.getRelativeOrgNameListByOrgId(
									organization.getId(),
									organizationDubboService, organizationId);
				}
				cacheValues.put(organization.getId(), orgRelativeName);
			}
	...
	private static void processSingleOrganizationName(Method readOrgMethod,
		...
		try {
			...
			throw new SystemUtilException("没有找到组织机构，orgid="
	...
	public static List<IssueLogNew> processAllIssueLogDealDescription(
			List<IssueLogNew> list) {
		if (list == null || list.size() == 0)
			return list;
		try {
			for (IssueLogNew issueLog : list) {
	...
	public static String getRelativeOrgNameByOrgId(Long orgId,
			OrganizationDubboService organizationDubboService) {
		Organization organization = organizationDubboService
				.getSimpleOrgAllOrganizationById(orgId);
		if (organization.getParentOrg() != null) {
	...
	public static String getRelativeOrgNameListByOrgId(Long orgId,
		...
		Organization organization = organizationDubboService
				.getSimpleOrgAllOrganizationById(orgId);
		if (organization.getParentOrg() != null) {
			return getOrganizationRelativeNameList(orgId,
					organizationDubboService, organizationId);
		}
		return organization.getOrgName();
	}
	...
	public static String getOrganizationRelativeNameList(Long orgId,
		...
		if ("test".equals(GlobalValue.environment)) {
			...
			String key = CacheKeyGenerator.generateCacheKeyFromString(
					ControllerHelper.class, actualRoot + "_" + orgId + "_"
							+ organizationId);
			if (orgId != null) {
				if (cacheService.get(key) != null) {
					path = cacheService.get(key).toString();
				} else {
					path = getRelativeNameList(orgId, organizationDubboService,
							actualRoot, organizationId);
					cacheService.set(key, path);
				}
			}
		}
		return path;
	...
	public static String getRelativeName(Long orgId,
		...
		if (!orgId.equals(actualRoot)) {
			while (org.getParentOrg() != null
				...
				org = organizationDubboService
						.getSimpleOrgAllOrganizationById(org.getParentOrg()
								.getId());
