...
public class UserManageController extends SearchBaseAction {
	...
	public String dispatchOperate() throws Exception {
		...
		} else if("accountMoveSelect".equals(mode)){
//			dict=propertyDictService.getPropertyDictById(dict.getId());
			return "accountMoveSelect";
		}
		return SUCCESS;
	}
	...
	@PermissionFilter(ename = "clearMobileUserImsi")
	public String clearMobileUserImsi() throws Exception {
		if ("".equals(userIds) || userIds == null) {
			errorMessage = "id为空，请选择要清空的用户！";
			return ERROR;
		}
		long id = ThreadVariable.getUser().getId();
		Long[] ids = analyzePopulationIds(userIds);
		// 检查清空的用户id是否包含当前登录用户id
		for (int i = 0; i < ids.length; i++) {
			if (id == ids[i]) {
				errorMessage = "清空的用户中包含当前登录用户，不能清空！";
				return ERROR;
			}
		}
		permissionService.clearMobileUserImsi(ids);
		return SUCCESS;
	}
	 ...
	 * 对手机账号进行维护 账号批量转移
	 * */
	@PermissionFilter(ename = "accountMove")
	public String mobileUserMove() throws Exception {
		permissionService.mobileUserMove(userIds,user,roleIdsStr);
		return SUCCESS;
	}
	...
	public PropertyDict getDict() {
		return dict;
	}

	public void setDict(PropertyDict dict) {
		this.dict = dict;
	}
