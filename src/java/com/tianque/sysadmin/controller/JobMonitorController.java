...
public class JobMonitorController extends BaseAction {
	...
	public void manualJobContent() throws Exception {
		...
		} else if (BaseInfoTables.ENAME_PERMISSION_CACHE_KEY.equals(jobType)){
...
//			leaderViewService.statisticsAttentionPopulationAddCountByOrgIdForJob();
	...
	@Action(value = "mobilDictionaryJob")
	public void mobilDictionaryJob() throws Exception {
		Long id = jobMonitorService.addJobMonitor(this.getClass().getName()
				+ "  类型：" + jobType);
		long startTime = System.currentTimeMillis();
		//生成手机数据字典项（全部）
		if (NewBaseInfoTables.ALL_MOBILE_DICTIONARY.equals(jobType)) {
			mobileDictionaryService.addOrUpdateMobileDictionary();
			//生成手机数据字典项（增量）
		} else if (NewBaseInfoTables.INCREMENT_MOBILE_DICTIONARY.equals(jobType)) {
