...
import com.tianque.statAnalyse.issueManage.issueAnalysisChartManage.dao.IssueAnalysisChartDao;
import com.tianque.statAnalyse.issueManage.issueAnalysisChartManage.domain.IssueLineColumnVo;
import com.tianque.statAnalyse.issueManage.issueAnalysisChartManage.domain.IssueLineDataColumnVo;
 ...
 */
@Service("issueAnalysisChartService")
public class IssueAnalysisChartServiceImpl implements IssueAnalysisChartService {
	private static Logger logger = LoggerFactory
			.getLogger(IssueAnalysisChartServiceImpl.class);
	private static final String STATISTICS_TYPE = "issueStatistics";
	@Autowired
	private IssueAnalysisChartDao issueAnalysisChartDao;
	@Autowired
...
//		}
	...
	public void createIssueAnalysisChartData() {
		...
		/** 把事件对接的也统计到里面，只是加在“四川省-->成都市-->青羊区-->草堂街道办事处”这个一个层级 */
		issueDataColumnVoService.createIssueJointAnalysisChartData();
	}
	...
	public IssueLineColumnVo getIssueComparedSameChart(Long orgId,
		...
		if (IssueAnalysisSearchType.SEARCH_BY_MONTH.equals(searchType)) {
			...
			for (int i = 0; i < time.length; i++) {
				...
				setComparedData((double) (thisCount - thatCount),
						(double) thatCount, rateData, i);
				setComparedData((double) thisCount, (double) thatCount,
						speedData, i);
			}
...
//			}
	...
	private void setComparedData(Double differCount, Double thatCount,
	...
	public List<Object[]> getStatisticPieToMobile(Long orgId, int year,
		...
		try {
			String key = MemCacheConstant.GETSTATISTICPIETOMOBILE + "_"
			...
			if (list == null) {
				...
				if (MobileModelTypes.ALL_ISSUE_MODEL.equals(issueType)) {
					list = getAllIssueDomainPieInfo(org.getOrgInternalCode(),
							year, month,
							IssueAnalysisSearchType.QUARTER_TO_MONTH_TO_MOBILE,
							IssueAnalysisSearchType.SEARCH_BY_MONTH);
				} else {
					...
					list = getAllIssueTypePieInfo(org.getOrgInternalCode(),
							year, month,
							IssueAnalysisSearchType.QUARTER_TO_MONTH_TO_MOBILE,
							IssueAnalysisSearchType.SEARCH_BY_MONTH,
			...
			throw new ServiceValidationException(
					"类IssueAnalysisChartServiceImpl的getStatisticPieToMobile方法出现异常，原因：",
					"手机事件的研判分析事件各个大类对应的饼状图错误", e);
		}
	}
	...
	public HighchartColumnVo getStatisticColumnForMobile(Long orgId, int year,
		...
		try {
			String key = MemCacheConstant.GETSTATISTICCOLUMNFORMOBILE + "_"
					+ issueType + "_" + year + "_" + month + "_" + orgId;
			HighchartColumnVo highchartColumnVo = (HighchartColumnVo) cacheService
			...
			if (highchartColumnVo == null) {
				...
				// 上个月
				Date lastMonthDate = CalendarUtil.getLastMonthEnd(year, month);
				int lastMonth = CalendarUtil.getMonth(lastMonthDate);
				int lastYear = CalendarUtil.getYear(lastMonthDate);
				// Map<Integer, Integer> monthMap = new HashMap<Integer,
				// Integer>();
				// monthMap.put(lastYear, lastMonth);
				// monthMap.put(year, month);

				List<Organization> orgList = organizationDubboService
						.findAdminOrgsByParentId(orgId);

				highchartColumnVo = new HighchartColumnVo();
				IssueTypeDomain issueTypeDomain = null;
				if (MobileModelTypes.ALL_ISSUE_MODEL.equals(issueType)) {
					highchartColumnVo.setModuleName("事件处理总况");
				} else {
					String issueTypeDomainName = MobileModelTypes.ISSUE_MODEL_MAP
							.get(issueType);
					issueTypeDomain = issueTypeDomainService
							.getIssueTypeDoaminByDomainName(issueTypeDomainName);
					highchartColumnVo.setModuleName(issueTypeDomainName);
				}

				List<HighchartDataColumnVo> series = new ArrayList<HighchartDataColumnVo>();
				List<String> orgNameList = new ArrayList<String>();
				Map<Integer, Integer> monthMap = new HashMap<Integer, Integer>();
				monthMap.put(lastMonth, lastYear);
				monthMap.put(month, year);
				if (orgList != null && orgList.size() != 0) {
					for (Organization org : orgList) {
						for (Map.Entry<Integer, Integer> entry : monthMap
								.entrySet()) {
							HighchartDataColumnVo highchartDataColumnVo = new HighchartDataColumnVo();
							highchartDataColumnVo.setStack("月份");
							highchartDataColumnVo.setName(String.valueOf(entry
									.getKey()));
							List<Integer> datas = new ArrayList<Integer>();
							// for (Organization org : orgList) {
							int result = 0;
							if (MobileModelTypes.ALL_ISSUE_MODEL
									.equals(issueType)) {
								result = issueAnalysisChartDao
										.getCountColumnByMonthAndOrgIdHistoryCount(
												entry.getValue(),
												entry.getKey(), org.getId(),
												null);
							} else {
								result = issueAnalysisChartDao
										.getCountColumnByMonthAndOrgIdHistoryCount(
					...
					highchartColumnVo.setSeries(series);
					// 设置部门
					highchartColumnVo.setCategories(orgNameList
							.toArray(new String[orgNameList.size()]));
				}
				cacheService.set(MemCacheConstant.LEADERVIEW_NAMESPACE, key,
						highchartColumnVo);
			}
			...
			throw new ServiceValidationException(
					"类IssueAnalysisChartServiceImpl的getStatisticColumnForMobile方法出现异常，原因：",
					"手机事件的研判分析事件各个大类对应的柱状图错误", e);
		}
	}
	...
	public IssueLineColumnVo getIssueSequentialChartForMobile(Long orgId,
		...
		try {
			...
			String key = MemCacheConstant.GETISSUESEQUENTIALCHARTFORMOBILE
					+ "_" + issueType + "_" + year + "_" + month + "_" + orgId;
			IssueLineColumnVo issueLineColumnVo = (IssueLineColumnVo) cacheService
					.get(MemCacheConstant.LEADERVIEW_NAMESPACE, key);

			if (issueLineColumnVo == null) {

				issueLineColumnVo = new IssueLineColumnVo();
				List<IssueLineDataColumnVo> series = new ArrayList<IssueLineDataColumnVo>();
				IssueLineDataColumnVo rateDataColumnVo = new IssueLineDataColumnVo();// 环比增长率
				IssueTypeDomain issueTypeDomain = null;
				if (MobileModelTypes.ALL_ISSUE_MODEL.equals(issueType)) {
					issueLineColumnVo.setModuleName("事件总数环比增长情况");
				} else {
					String issueTypeDomainName = MobileModelTypes.ISSUE_MODEL_MAP
							.get(issueType);
					issueTypeDomain = issueTypeDomainService
							.getIssueTypeDoaminByDomainName(issueTypeDomainName);
					issueLineColumnVo.setModuleName(issueTypeDomainName);
				}
				issueLineColumnVo.setSeries(series);
				series.add(rateDataColumnVo);
				rateDataColumnVo.setName("环比增长率(%)");
				...
				for (int i = 0; i < time.length; i++) {
					...
					month = Integer
							.parseInt(tempTime[1].startsWith("0") ? tempTime[1]
									.substring(1) : tempTime[1]);
					if (MobileModelTypes.ALL_ISSUE_MODEL.equals(issueType)) {
						thisCount = issueAnalysisChartDao
								.getIssueTendencyCountByMonthAndOrgIdHistoryCount(
										year, month, orgId,
										org.getOrgInternalCode(), null);// 本期数
					} else {
						thisCount = issueAnalysisChartDao
								.getIssueTendencyCountByMonthAndOrgIdHistoryCount(
										year, month, orgId,
										org.getOrgInternalCode(),
										issueTypeDomain.getId());// 本期数
					}
					if (month == 1) {// 如果是一月 就跨年
						year = year - 1;
						month = 12;
					...
					if (MobileModelTypes.ALL_ISSUE_MODEL.equals(issueType)) {
						thatCount = issueAnalysisChartDao
								.getIssueTendencyCountByMonthAndOrgIdHistoryCount(
										year, month, orgId,
										org.getOrgInternalCode(), null);// 上期数
					} else {
						thatCount = issueAnalysisChartDao
								.getIssueTendencyCountByMonthAndOrgIdHistoryCount(
										year, month, orgId,
										org.getOrgInternalCode(),
										issueTypeDomain.getId());// 上期数
					}
					setComparedData((double) (thisCount - thatCount),
							(double) thatCount, rateData, i);
					setComparedData((double) thisCount, (double) thatCount,
			...
			throw new ServiceValidationException(
					"类IssueAnalysisChartServiceImpl的getIssueSequentialChartForMobile方法出现异常，原因：",
					"手机事件的研判分析为手机端提供事件环比趋势图错误", e);
		}
	}
}
