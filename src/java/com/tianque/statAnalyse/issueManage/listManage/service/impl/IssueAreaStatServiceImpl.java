...
public class IssueAreaStatServiceImpl implements IssueAreaStatService {
	...
	public List<IssueAreaStat> findIssueAreaStatsByYearAndMonthAndParentOrgId(
		...
		return findIssueAreaStatsByYearAndMonth(startDate, endDate,
				parentOrgId, queryType, !IssueReportStatHelper.ISDATEOVER,type);
	}
	...
	private List<IssueAreaStat> findIssueAreaStatsByYearAndMonth(
			...
			boolean isDateOver,String type) {
		...
		for (Organization organization : organizations) {
			...
			if("issueReport".equals(type) || type == null){
				issueAreaStat = getIssueAreaStatsByYearAndMonthAndOrgCode(
						startDate, endDate, organization, queryType, isDateOver);
			}else{//"issueHistoryReport".equals(type)
				//历史表数据
				if(queryType == QueryType.AREA_DEAL_HISTORY){
					issueAreaStat = issueAreaStatDao.getIssueAreaStatsByYearAndMonthAndOrgId_history(
							startDate, endDate, organization.getId());
				}else if(queryType == QueryType.AREA_CLASSIFICATION_HISTORY){
					issueAreaStat = issueAreaStatDao.getIssueAreaTypeStatsByYearAndMonthAndOrgId_history(
							startDate, endDate, organization.getId());
				}else if(queryType == QueryType.STEP_HISTORY){
					issueAreaStat = issueAreaStatDao.getIssueStepStatsByYearAndMonthAndOrgId_history(
							startDate, endDate, organization.getId());
				}
			}
			// 草堂街道办事处数据统计进来
