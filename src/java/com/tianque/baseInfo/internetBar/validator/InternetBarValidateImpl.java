package com.tianque.baseInfo.internetBar.validator;
 ...
 * @author xiaofeiyang
 */
@Component("internetBarValidator")
public class InternetBarValidateImpl implements DomainValidator<InternetBar> {
	...
	@Override
	public ValidateResult validate(InternetBar domain) {
		ValidateResult validateResult = new ValidateResult();
		if (validateHelper.emptyString(domain.getPlaceName())) {
			validateResult.addErrorMessage(getColumNo("placeName") + "网吧名称必须输入");
		} else if (validateHelper.illegalStringLength(2, 50, domain.getPlaceName())) {
			validateResult.addErrorMessage(getColumNo("placeName") + "网吧名称只能输入2-50个字符");
		}
		if (!validateHelper.emptyString(domain.getPlaceName())
				&& !validateHelper.isConSpeCharacters(domain.getPlaceName())) {
			validateResult.addErrorMessage(getColumNo("placeName") + "网吧不能有特殊字符");
		}
...
//				validateResult.addErrorMessage(getColumNo("organization") + "权限越界，不能将数据导入到该网格下！");
//			}
//		}
		...
		if (!validateHelper.emptyString(domain.getMobile())
				&& validateHelper.illegalStringLength(0, 20, domain.getMobile())) {
			validateResult.addErrorMessage(getColumNo("mobile") + "联系方式最多输入20位");
		}
		if (validateHelper.illegalStringLength(0, 50, domain.getPlaceAddress())) {
			validateResult.addErrorMessage(getColumNo("placeAddress") + "单位地址最多输入50位");
		}
		if (validateHelper.illegalStringLength(0, 20, domain.getManager())) {
			validateResult.addErrorMessage(getColumNo("manager") + "负责人最多输入20位");
		}
		if (validateHelper.illegalStringLength(0, 20, domain.getBarCode())) {
			validateResult.addErrorMessage(getColumNo("barCode") + "网吧编号最多输入20位");
		}
		if (!validateHelper.emptyString(domain.getNetAccessProviders())
				&& !validateHelper.isConSpeCharacters(domain.getNetAccessProviders())) {
			validateResult.addErrorMessage(getColumNo("netAccessProviders") + "宽带接入服务商不能有特殊字符");
		}
		if (!validateHelper.emptyString(domain.getNetAccessProviders())
				&& validateHelper.illegalStringLength(0, 50, domain.getNetAccessProviders())) {
			validateResult.addErrorMessage(getColumNo("netAccessProviders") + "宽带接入服务商最多输入50个字符");
		}
		if (!validateHelper.emptyString(domain.getUseIP())
				&& !validateHelper.isIP(domain.getUseIP())) {
			validateResult.addErrorMessage(getColumNo("useIP") + "现使用IP不能有特殊字符");
		}
		if (!validateHelper.emptyString(domain.getUseIP())
				&& validateHelper.illegalStringLength(0, 50, domain.getUseIP())) {
			validateResult.addErrorMessage(getColumNo("useIP") + "现使用IP最多输入50个字符");
		}
		if (validateHelper.illegalStringLength(0, 20, domain.getInternetAccessType())) {
			validateResult.addErrorMessage(getColumNo("internetAccessType") + "宽带接入方式最多输入20位");
		}
		if (validateHelper.illegalStringLength(0, 20, domain.getNetCultureLicenceNo())) {
			validateResult.addErrorMessage(getColumNo("netCultureLicenceNo") + "网络文化经营许可证号最多输入20位");
		}
		if (validateHelper.illegalStringLength(0, 20, domain.getNetSecurityAuditNo())) {
			validateResult.addErrorMessage(getColumNo("netSecurityAuditNo") + "网络安全审核意见书号最多输入20位");
		}
		if (validateHelper.illegalStringLength(0, 20, domain.getFireAuditDocumentNo())) {
			validateResult.addErrorMessage(getColumNo("fireAuditDocumentNo") + "消防审核意见书号最多输入20位");
		}
		if (!validateHelper.nullObj(domain.getTempNetCardNum())
				&& validateHelper.illegalNumberZZ(domain.getTempNetCardNum().toString())
				&& !(domain.getTempNetCardNum().longValue() < 10000000000l))
			validateResult
					.addErrorMessage(getColumNo("tempNetCardNum") + "临时上网卡数量为1-999999999的正整数");
		if (!validateHelper.nullObj(domain.getTotalComputerNum())
				&& validateHelper.illegalNumberZZ(domain.getTotalComputerNum().toString())
				&& !(domain.getTotalComputerNum().longValue() < 10000000000l))
			validateResult
					.addErrorMessage(getColumNo("totalComputerNum") + "电脑总数为1-9999999999的正整数");
		if (domain.getOperationArea() != null) {
			if (domain.getOperationArea() < 0 || domain.getOperationArea() > 9999999.99) {
				validateResult
						.addErrorMessage(getColumNo("operationArea") + "营业面积为1-9999999.99的正数");
			}
		}
