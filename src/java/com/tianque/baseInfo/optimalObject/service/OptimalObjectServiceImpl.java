...
public class OptimalObjectServiceImpl extends BaseInfoPopulationTemplateImpl
	...
	//隐藏身份证中间4位
		private PageInfo<OptimalObject> hiddenIdCard(PageInfo<OptimalObject> pageInfo){
							//判断权限，有权限不隐藏
							if(permissionDubboService.
									isUserHasPermission(ThreadVariable.getUser().getId(), "isOptimalObjectManagementNotHidCard")){
								return pageInfo;
							}
							List<OptimalObject> list = pageInfo.getResult();
							int index=0;
							for (OptimalObject verification:list) {
	...
	public PageInfo searchOptimalObjects(
		...
		PageInfo pageInfos=  optimalObjectDao.searchOptimalObjects(page, rows,
	...
	public PageInfo fastSearchOptimalObject(
			SearchOptimalObjectVo searchOptimalObjectVo, Integer page,
			Integer rows, String sidx, String sord) {
		PageInfo pageInfos=  optimalObjectDao.fastSearchOptimalObject(searchOptimalObjectVo,
				page, rows, sidx, sord);
		return pageInfos=hiddenIdCard(pageInfos);
	}
