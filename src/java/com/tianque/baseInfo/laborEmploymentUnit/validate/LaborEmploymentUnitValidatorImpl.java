package com.tianque.baseInfo.laborEmploymentUnit.validate;
...
@Component("laborEmploymentUnitValidator")
public class LaborEmploymentUnitValidatorImpl implements
		DomainValidator<LaborEmploymentUnit> {
	...
	@Override
	public ValidateResult validate(LaborEmploymentUnit domain) {
		...
		if (validateHelper.emptyString(domain.getCompanyName())
			...
			validateResult.addErrorMessage("单位名称不能为空");
		}
		if (!validateHelper.emptyString(domain.getCompanyName())
		...
		if (!validateHelper.emptyString(domain.getTelephone())
			...
			validateResult.addErrorMessage("固定电话只能输入数字和-");
		}
