...
@Component("floatingPopulationJob")
public class FloatingPopulationJob implements Job {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		updateFloatingPopulationLogOut();
	}

	private void updateFloatingPopulationLogOut() {
		...
		try {
			...
			for (Organization org : cityOrgs) {
				...
				do {
					...
					floatingPopulationfList = floatingPopulationService.findFloatingPopulationsWhenIsOld(pageSize, org);
			...
			jobMonitorService.updateJobMonitor(id,
					"花了" + (System.currentTimeMillis() - startTime)
							+ "执行流动人口job的自动新增大于60岁的人！成功人数："+successCount, true);
		} catch (Exception e) {
			logger.error("流动人口job的自动新增大于60岁的人执行错误：", e);
			jobMonitorService.updateJobMonitor(id,
					"流动人口job的自动新增大于60岁的人执行错误:" + e.getMessage(), false);
		}
