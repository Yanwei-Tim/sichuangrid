package com.tianque.fourTeams.util;
...
public class FourTeamsIssueLogDealOverTimeHelper {

	public static boolean isDealOverTime(FourTeamsIssueLogNew issueLogNew,
			WorkCalendarService workCalendarService) {
		boolean flag = false;
		if (issueLogNew.getDealState().equals(IssueDealState.UN_DONE)
				|| issueLogNew.getDealState().equals(IssueDealState.UN_READ)) {
			Date normalDealTime = workCalendarService.getFutureDate(issueLogNew
					.getDealTime(), OverTime.YELLOW_CONCEPT);
			if (CalendarUtil.now("yyyy-MM-dd HH:mm:ss").after(normalDealTime))
				flag = true;
		} else if (issueLogNew.getDealState().equals(IssueDealState.DOING)) {
			Date normalDealTime = workCalendarService.getFutureDate(issueLogNew
					.getDealTime(), OverTime.YELLOW_DONE);
			if (CalendarUtil.now("yyyy-MM-dd HH:mm:ss").after(normalDealTime))
				flag = true;
			...
			Date normalDate = workCalendarService.getFutureDate(issueLogNew
					.getForLogEntryTime(), OverTime.YELLOW_DONE);
			if (issueLogNew.getDealTime().after(normalDate))
				flag = true;
		} else if (issueLogNew.getDealState().equals(IssueDealState.READED)) {
			Date normalDate = workCalendarService.getFutureDate(issueLogNew
					.getForLogEntryTime(), OverTime.YELLOW_CONCEPT);
			if (issueLogNew.getDealTime().after(normalDate))
				flag = true;
		}
		return flag;
	...
	public static boolean isOvertime(Date enddate, Date startDate,
		...
		Date lastDealTime = workCalendarService.getFutureDate(startDate,
				dateLineType);
