package com.tianque.platformMessage.factory;
...
@Component("platformMessageFactory")
public class PlatformMessageFactory {
	@Autowired
	 ...
	 * @return pm
	 */
	public PlatformMessage createNeedDoIssuePlatformMessage(IssueNew issue) {
	 ...
	 * @return pm
	 */
	public PlatformMessage createSeriousIssuePlatformMessage(IssueNew issue) {
	 ...
	 * @return pm
	 */
	public PlatformMessage createIssueTipPlatformMessage(IssueNew issue,
			IssueOperate issueOper, IssueStepGroup steps) {
		...
		if (steps != null) {
			if (steps.getKeyStep().getTarget().getId() < 0) {
				tatgetOrg = steps.getKeyStep().getTarget();
			} else {
		...
		String content = "";
		String url = "";
		if (IssueOperate.COMPLETE.equals(issueOper)) {
			...
			content = content.substring(0, content.lastIndexOf("于"))
					+ "，请及时进行验证。";
			url = IssueOperate.COMPLETE.getDesc().equals(issueOper.getDesc()) ? getGlobalValue(GlobalSetting.MYISSUELIST_URL)
					+ "Verification"
					: null;
		} else if (IssueOperate.VERIFICATION.equals(issueOper)) {
			content = getGlobalValue(GlobalSetting.ISSUEPM_CONTENT)
					.replace("【服务单号】", issue.getSerialNumber())
					.replace("【事件名称】", issue.getSubject())
					.replace("【处理部门名称】", currentOrgName)
			...
			content = content.substring(0, content.lastIndexOf("于"))
					+ "，验证完成，请及时评分。";
			url = IssueOperate.VERIFICATION.getDesc().equals(
					issueOper.getDesc()) ? getGlobalValue(GlobalSetting.MYISSUELIST_URL)
					+ "CheckGrade"
					: null;
		} else if (IssueOperate.INSTRUCT.equals(issueOper)) {
			content = getGlobalValue(GlobalSetting.ISSUEPM_CONTENT)
					.replace("【服务单号】", issue.getSerialNumber())
					.replace("【事件名称】", issue.getSubject())
					.replace("【处理部门名称】", currentOrgName)
					.replace("【处理时间】", getTimeStr())
					.replace("【操作类型】", "进行了" + issueOper.toString());
			content = content.substring(0, content.lastIndexOf("于"))
					+ "，请及时查看并处理。";
		} else {
			content = getGlobalValue(GlobalSetting.ISSUEPM_CONTENT)
	...
	public PlatformMessage createEvaluateUrgePlatformMessage(Evaluate evaluate,
		...
		String content = getGlobalValue(
				GlobalSetting.EVALUATE_RUSHTO_PM_CONTENT)
				.replace("【自评部门】", org.getOrgName())
				.replace("【考核标准时间】", evaluate.getYear())
				.replace("【考核标准名称】", evaluate.getTitle());
	...
	public PlatformMessage createUrgeDocumentPlatformMessage(Document document) {
		...
		// 将此签收文件的ID放在消息中
		pm.setFileInfoIds(String.valueOf(document.getId()));
		return pm;
	}
	 ...
	 * @return
	 */
	public PlatformMessage createUserDocumentPlatformMessage(Document document,
			Long userId) {
	...
	public PlatformMessage createSharInfomationPlatformMessage(
		...
		// 获得共享资料的ID
		String fileInfoIds = getFileInfoIds(files);
		PlatformMessage pm = new PlatformMessage(title, content, url,
				ReceiverType.ORG, PlatformMessageSendType.SYSTERM_SNED,
				PlatformMessageType.SHARED_FILE_REMIND);
		pm.setFileInfoIds(fileInfoIds);
		return pm;
	}

	private String getFileInfoIds(List<MyProfile> files) {
		String ids = "";
		if (files != null && files.size() != 0) {
			for (int i = 0; i < files.size(); i++) {
				if (i == files.size() - 1) {
					ids += files.get(i).getId() + "";
				} else {
					ids += files.get(i).getId() + ",";
				}
			}
