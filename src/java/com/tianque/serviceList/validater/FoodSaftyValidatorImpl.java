...
@Component("foodSaftyImportValidator")
public class FoodSaftyValidatorImpl implements
		DomainValidator<FoodSafty> {
	@Autowired
	...
	@Override
	public ValidateResult validate(FoodSafty domain) {
		...
		//参与人员验证
		if (!validateHelper.emptyString(domain.getPersonnel()) && validateHelper.illegalScript(domain.getPersonnel())) {
			// 非法脚本验证
			result.addErrorMessage(getColumNo("personnel") + "参与人员不能输入非法脚本");
		}else{
			// 长度验证
			if (!validateHelper.emptyString(domain.getPersonnel())&&validateHelper.illegalStringLength(MIN_LENGTH, DETAILS_LENGTH,
					domain.getPersonnel())) {
		...
		PropertyDict dict = propertyDictDubboService.getPropertyDictById(domain.getCategory().getId());
		if(dict==null){
			result.addErrorMessage(getColumNo("category")
					+ "类别选择错误");
		}else{
			if(PropertyTypes.RURAL_FAMILY.equals(dict.getDisplayName()) && domain.getDeclaration()==null){
				result.addErrorMessage(getColumNo("declaration")
						+ "选择农村家宴时，必须选择是否申报");
			}
		}
		return result;
	...
	//pc新增修改验证
	public void validatorFoodSafty(FoodSafty domain) {
