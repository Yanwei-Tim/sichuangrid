...
@Component("pyramidSalesManageImportValidator")
public class PyramidSalesManageValidatorImpl implements
		DomainValidator<PyramidSalesManage> {
	@Autowired
	...
	@Override
	public ValidateResult validate(PyramidSalesManage domain) {
		...
		if(!validateHelper.emptyString(domain.getPersonnel())){
			if (validateHelper.illegalScript(domain.getPersonnel())) {
				// 非法脚本验证
		...
		// 传销类型验证
		validatorPyramidCategory(domain.getPyramidCategory(), result);
		return result;
	}

	private void validatorPyramidCategory(PropertyDict category,
			ValidateResult result) {
		if (!typeIsNotNull(category)) {
			result.addErrorMessage(getColumNo("pyramidCategory") + "请选择传销类别");
		}
	}
	...
	//pc新增修改验证
	public void validatorPyramidSalesManage(PyramidSalesManage domain) {
