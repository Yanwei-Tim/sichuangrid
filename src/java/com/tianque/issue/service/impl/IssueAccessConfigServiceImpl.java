...
@Transactional
@Service("issueAccessConfigService")
public class IssueAccessConfigServiceImpl implements IssueAccessConfigService {
	@Autowired
	private IssueAccessConfigDao issueAccessConfigDao;
	...
	private IssueAccessConfig getScores() {
		IssueAccessConfig config = getIssueAccessConfig();
		if (config == null) {
	...
	private void validateParam(IssueAccessConfig bean) {
		if (bean == null) {
